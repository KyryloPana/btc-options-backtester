  ...
  private async syncManifest() {
    try {
      ...
      for (const [rows, status] of [[expired, "expired"], [active, "active"]] as const) for (const row of rows ?? []) {
        ...
        map.set(name, { instrumentName: name, expiryTimestamp: expiry, expiryLabel: expiryLabel(expiry), creationTimestamp: Number.isFinite(Number(row.creation_timestamp)) ? Number(row.creation_timestamp) : undefined, strike, optionType: type, status });
      }
