...
const PREFIX="/__local/research-selections",SAFE_ID=/^[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?$/;
...
async function body(req:IncomingMessage){const chunks:Buffer[]=[];let total=0;for await(const chunk of req){const b=Buffer.from(chunk);total+=b.length;if(total>10_000_000)throw Object.assign(new Error("Selection snapshot exceeds 10 MB."),{status:413});chunks.push(b)}try{return JSON.parse(Buffer.concat(chunks).toString("utf8")||"{}") }catch{throw Object.assign(new Error("Request body is malformed JSON."),{status:400})}}
