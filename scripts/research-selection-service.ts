...
/**
 * Factory for the recompute engine, injected so the route can be exercised in a
 * test without a network and wired to the real Deribit service in the app.
 */
export type RecomputeEngineFactory=(store:ResearchSelectionStore)=>Promise<ResearchRecomputeEngine>|ResearchRecomputeEngine;

export function researchSelectionApiPlugin(directory=resolve(process.cwd(),"data/research-selections"),engineFactory?:RecomputeEngineFactory):Plugin{const service=new ResearchSelectionService(directory);return{name:"local-research-selections",configureServer(server){server.middlewares.use(async(req,res,next)=>{const url=new URL(req.url??"/","http://local");if(url.pathname===CAPABILITIES&&req.method==="GET")return json(res,200,{runtime:"local-application-server",researchSelections:true,version:1});if(!url.pathname.startsWith(PREFIX))return next();try{const rest=url.pathname.slice(PREFIX.length+1).split("/").map(decodeURIComponent);const [id,kind,eventId]=rest;if(!id||!SAFE_ID.test(id))return json(res,404,{error:"Research selection endpoint not found."});if(rest.length===1&&req.method==="GET")return json(res,200,await service.read(id));if(rest.length===1&&req.method==="PUT")return json(res,200,await service.save(id,await body(req)));if(rest.length===2&&kind==="recompute"&&req.method==="POST"){
 // Rebuilds DERIVED research for selections that already exist. Structural
 // identity comes from the saved store, never from the engine, so this route
 // cannot reselect. Persistence happens once, after every engine call and
 // validation succeed.
 if(!engineFactory)return json(res,501,{error:"No recompute engine is configured on this server."});
 const data=await body(req) as {scope?:RefreshScope;expectedUpdatedAt?:string|null};
 const current=await service.read(id);
 const engine=await engineFactory(current);
 const result=await service.recompute(id,data.scope??{kind:"all"},engine,data.expectedUpdatedAt??null);
 return json(res,200,{refreshed:result.refreshed,updatedAtUtc:result.store.updatedAtUtc,
  selectedStructureCount:result.store.events.reduce((n,e)=>n+e.selectedStructures.length,0)});
}
if(rest.length===3&&kind==="events"&&eventId){if(req.method==="PUT")return json(res,200,{store:await service.upsertEvent(id,eventId,await body(req),req.headers["if-match"]?.toString())});if(req.method==="DELETE")return json(res,200,{store:await service.deleteEvent(id,eventId,req.headers["if-match"]?.toString())});return json(res,405,{error:"Method not allowed."});}return json(res,404,{error:"Research selection endpoint not found."});}catch(error){const e=error as Error&{status?:number;details?:unknown};return json(res,e.status??400,{error:e.message,details:e.details})}})}}}
