...
export default function Home() {
  ...
  const retrievedSpreads = useMemo(
    () => desiredSpreads.map(combo => retrieveSpread(combo, effectiveEntryTimestamp, inventory)),
    [desiredSpreads, effectiveEntryTimestamp, inventory],
  );
  ...
  async function loadRequiredContracts(forceIndex = false) {
    ...
    try {
      ...
      setParseStatus(`${payload.diagnostics.filesRead.toLocaleString()} files read on demand · ${nextInventory.length.toLocaleString()} contracts · ${payload.diagnostics.validTrades.toLocaleString()} valid trades${payload.diagnostics.failedFiles.length ? ` · ${payload.diagnostics.failedFiles.length} failed files` : ""}`);
    } catch (error) {
  ...
  async function runBacktest() {
    const runnable = retrievedSpreads.filter(spread => spread.soldContract && spread.boughtContract && spread.expiryTimestamp);
    if (!runnable.length) {
  ...
  function exportResults() {
    const payload = {
      ...
      event: selectedEvent,
      configuration: { dtes, widths, spreadKind, executionMode, comboExecution, amount, pricingModes },
    ...
    <main>
      ...
      <div className="page-shell">
          ...
          <div className="event-layout">
            <aside className="event-list card">
              ...
              <div className="event-scroll">
                {events.map(event => (
                  <button className={`event-row ${event.id === selectedEvent.id ? "selected" : ""}`} onClick={() => { setSelectedEventId(event.id); setAnalysisResults([]); }} key={event.id}>
        ...
        <section className="workspace-section" ref={node => { sectionRefs.current.construction = node; }}>
          <div className="section-heading"><div><span className="step-number">02</span><p className="eyebrow">Mechanical search space</p><h2>Spread construction</h2></div><span className="count-chip">{desiredSpreads.length} desired combinations</span></div>
          <div className="construction-layout">
            <div className="config-card card">
              <div className="config-block"><label className="config-label">Target DTE <small>nearest expiry ≥ target</small></label><CheckboxGroup values={DTE_OPTIONS} selected={dtes} onChange={setDtes} formatter={value => `${value}D`} /></div>
            ...
            <div className="logic-card card">
              ...
              <p className="fine-print">When the extreme is less than $100 from the rounded strike, the engine adds a separate $1k outward-buffer test. Both legs always use the same selected expiry.</p>
            </div>
          </div>
          <div className="table-card card">
            ...
            <div className="table-scroll"><table><thead><tr><th>Structure</th><th>DTE target</th><th>Desired legs</th><th>Expiry chosen</th><th>Actual legs</th><th>Observed at entry</th><th>Status</th></tr></thead><tbody>
              {retrievedSpreads.map(spread => (
                <tr className={selectedSpread?.id === spread.id ? "row-selected" : ""} key={spread.id} onClick={() => setSelectedSpreadId(spread.id)}>
                  <td><strong>{spread.structure}</strong>{spread.buffered && <small className="buffer-tag">buffer</small>}</td><td>{spread.targetDte}D</td><td><span className="mono">S {money(spread.soldStrike)} / B {money(spread.boughtStrike)}</span><small>{money(spread.targetWidth)} width</small></td><td>{spread.expiryLabel ?? "—"}<small>{spread.actualDte ? `${spread.actualDte.toFixed(1)} actual DTE` : "awaiting files"}</small></td><td>{spread.soldContract && spread.boughtContract ? <><span className="mono">{spread.soldContract.instrumentName}</span><small className="mono">{spread.boughtContract.instrumentName}</small></> : "—"}</td><td>{spread.soldExistedAtEntry && spread.boughtExistedAtEntry ? "Both" : spread.soldContract || spread.boughtContract ? "Unproven" : "—"}</td><td><span className={flagClass(spread.retrievalStatus)}>{spread.retrievalStatus}</span></td>
                </tr>
              ))}
              {!retrievedSpreads.length && <tr><td colSpan={7} className="empty-cell">Enter a failed extreme and select at least one DTE and width.</td></tr>}
            </tbody></table></div>
          </div>
        </section>
        ...
        <section className="workspace-section" ref={node => { sectionRefs.current.contracts = node; }}>
          ...
          <div className="action-bar"><div><strong>Ready to value {retrievedSpreads.filter(spread => spread.soldContract && spread.boughtContract).length} combinations</strong><span>4H grid · VPOC · credit capture · fixed time · invalidation · expiry</span></div><button className="primary-button" onClick={runBacktest}>Run full backtest</button></div>
        ...
        <section className="workspace-section" ref={node => { sectionRefs.current.analysis = node; }}>
          ...
          <div className="table-card card"><div className="table-scroll"><table><thead><tr><th>Quality</th><th>Spread</th><th>Expiry</th><th>Width</th><th>Entry raw</th><th>Entry IV</th><th>Best IV PnL</th><th>Worst IV PnL</th><th>Selected exit</th></tr></thead><tbody>
            {filteredResults.map(result => {
              ...
              return <tr className={selectedAnalysis?.spread.id === result.spread.id ? "row-selected" : ""} key={result.spread.id} onClick={() => setSelectedResultId(result.spread.id)}><td><span className={flagClass(result.eventQuality)}>{result.eventQuality}</span></td><td><strong>{result.spread.structure}</strong><small className="mono">{result.spread.soldContract?.strike} / {result.spread.boughtContract?.strike} {result.spread.optionType}</small></td><td>{result.spread.expiryLabel}<small>{result.spread.actualDte?.toFixed(1)}D</small></td><td>{money(result.spread.actualWidth)}</td><td>{btc(result.path[0]?.rawSpreadValue)}</td><td>{btc(result.path[0]?.ivSpreadValue)}</td><td className="positive">{best === -Infinity ? "—" : money(best)}</td><td className="negative">{worst === Infinity ? "—" : money(worst)}</td><td>{preferred?.rule ?? "—"}<small>{money(preferred?.ivPnlUsd ?? preferred?.rawPnlUsd)}</small></td></tr>;
            })}
