...
import {availableStateQuantiles,buildVolatilityReport,quantiles,endpointEvidenceAssessment,entryVolatilityEvidenceAssessment,partitionStructuresByEvidence,pathEvidenceAssessment,skewVisualScale,structureEvidenceAssessment,structureUnavailableReasons,structuresForNominalHorizon} from "../app/lib/volatility/volatility-report.ts";
...
test("component keeps analytical detail collapsed by default and exposes audit labels",async()=>{
 ...
 assert.match(source,/<summary>Usable \/ partial/);assert.match(source,/<summary>Unavailable audit/);assert.match(source,/checked=\{showEndpoints\}/);assert.doesNotMatch(source,/checked=\{true\}/);assert.match(source,/Unavailable path audit/);
});
...
test("endpoint observed IV and delta availability have distinct reasons and units",()=>{const observed={status:"available",source:"deribit_trade_iv",iv_decimal:.4},structure={event_id:"e",candidate_id:"c",legs:[],same_expiry_reference:{status:"unavailable"},differentials:[],post_entry_market_iv:[{endpoint_id:"4h",short:observed,long:observed}],market_iv_path:[]};const report=buildVolatilityReport(dataset({event_volatility_state:[],structure_volatility_state:[structure],valuations:[],outcomes:[]}));assert.equal(report.endpointSummary.endpointBothObserved,1);assert.equal(report.endpointSummary.bothDeltaUsable,0);assert.equal(Object.values(report.endpointSummary.attemptReasons).reduce((a,b)=>a+b,0),1);assert.match(Object.keys(report.endpointSummary.attemptReasons)[0]!,/entry_short.*entry_long/);assert.equal(Object.values(report.endpointSummary.candidateReasons).reduce((a,b)=>a+b,0),report.endpointSummary.unavailableCandidates)});

test("evidence status is independent of economic outcome sign",()=>{const row={id:"x",label:"x",candidateId:"c",timestampUtc:null,entryShortIv:.5,shortIv:.4,deltaShortIv:-.1,entryLongIv:.5,longIv:.6,deltaLongIv:.1,status:"available" as const,reason:null};assert.equal(endpointEvidenceAssessment(row).status,"VALID");assert.doesNotMatch(JSON.stringify(endpointEvidenceAssessment(row)),/pnl|profit|loss/i)});


test("shared skew scale maps materially different distributions to different coordinates",()=>{const scale=skewVisualScale([{p25:-.2,median:-.1,p75:0},{p25:.1,median:.2,p75:.4}]);assert.deepEqual([scale.position(-.2),scale.position(-.1),scale.position(0)],[10,26.666666666666664,43.33333333333333]);assert.ok(scale.position(.2)!>scale.position(0)!);const flat=skewVisualScale([{p25:.1,median:.1,p75:.1}]);assert.equal(flat.position(.1),60)});

test("structure, strike and width focal completeness can differ",()=>{const row={legs:[{leg:"short",status:"available",observation:"observed",iv_decimal:.5}],same_expiry_reference:{status:"available",iv_decimal:.4},differentials:[{differential:"short_minus_reference_iv",status:"available",value:.1}]};assert.equal(structureEvidenceAssessment(row,"strike").status,"VALID");assert.equal(structureEvidenceAssessment(row,"structure").status,"PARTIAL");assert.equal(structureEvidenceAssessment(row,"width").status,"PARTIAL")});

test("endpoint and path assessments preserve PARTIAL",()=>{const endpoint={id:"x",label:"x",candidateId:"c",timestampUtc:null,entryShortIv:.5,shortIv:.4,deltaShortIv:-.1,entryLongIv:null,longIv:null,deltaLongIv:null,status:"available" as const,reason:"endpoint_long_iv_unavailable"};assert.equal(endpointEvidenceAssessment(endpoint).status,"PARTIAL");assert.equal(endpointEvidenceAssessment({...endpoint,shortIv:null,deltaShortIv:null,status:"unavailable"}).status,"UNAVAILABLE");const path={candidateId:"c",shortPoints:1,longPoints:1,timestampCountAnyObserved:1,timestampCountBothObserved:1,missingTimestampObservationRows:0,shortMin:.4,shortMax:.4,shortRange:0,longMin:.5,longMax:.5,longRange:0};assert.equal(pathEvidenceAssessment(path).status,"PARTIAL")});

test("missing path timestamps are audited but never become pseudo timestamps",()=>{const market={status:"available",source:"deribit_trade_iv",iv_decimal:.4},structure={event_id:"e",candidate_id:"c",legs:[],same_expiry_reference:{status:"unavailable"},differentials:[],post_entry_market_iv:[],market_iv_path:[{short:market,long:market}]};const report=buildVolatilityReport(dataset({event_volatility_state:[],structure_volatility_state:[structure],valuations:[],outcomes:[]})),path=report.paths[0]!;assert.equal(path.timestampCountAnyObserved,0);assert.equal(path.missingTimestampObservationRows,1);assert.equal(report.pathSummary.zero,1)});

test("entry status needs core context, not merely three Reference tenors",()=>{const row={reference_iv:["7d","14d","30d"].map(nominal_tenor=>({nominal_tenor,status:"available",iv_decimal:.5})),realized_volatility:[],iv_minus_rv:[]};assert.equal(entryVolatilityEvidenceAssessment(row).status,"PARTIAL")});


test("focal partitions and reasons ignore irrelevant evidence",()=>{const longOnly={candidate_id:"long",legs:[{leg:"long",status:"available",observation:"observed",iv_decimal:.4},{leg:"short",status:"unavailable",unavailable_reason:"short missing"}],same_expiry_reference:{status:"unavailable",unavailable_reason:"reference missing"},differentials:[]},strikeReady={candidate_id:"strike",legs:[{leg:"short",status:"available",observation:"observed",iv_decimal:.5},{leg:"long",status:"unavailable",unavailable_reason:"irrelevant long"}],same_expiry_reference:{status:"available",iv_decimal:.4},differentials:[{differential:"short_minus_reference_iv",status:"available",value:.1}]},widthReady={candidate_id:"width",legs:[{leg:"short",status:"available",observation:"observed",iv_decimal:.5},{leg:"long",status:"available",observation:"observed",iv_decimal:.4}],same_expiry_reference:{status:"unavailable",unavailable_reason:"irrelevant reference"},differentials:[{differential:"short_minus_long_iv",status:"available",value:.1}]};assert.deepEqual(partitionStructuresByEvidence([longOnly,strikeReady],"strike").usable.map(x=>x.candidate_id),["strike"]);assert.equal(structureEvidenceAssessment(strikeReady,"strike").status,"VALID");assert.equal(structureEvidenceAssessment(widthReady,"width").status,"VALID")});

test("endpoint retains simultaneous side-specific failures",()=>{const observed={status:"available",source:"deribit_trade_iv",iv_decimal:.4},missing={status:"unavailable"},structure={event_id:"e",candidate_id:"c",legs:[{leg:"long",status:"unavailable",observation:"observed"}],same_expiry_reference:{status:"unavailable"},differentials:[],post_entry_market_iv:[{endpoint_id:"4h",short:missing,long:observed}],market_iv_path:[]};const endpoint=buildVolatilityReport(dataset({event_volatility_state:[],structure_volatility_state:[structure],valuations:[],outcomes:[]})).endpoints[0]!;assert.equal(endpoint.shortDeltaReason,"endpoint_short_iv_unavailable");assert.equal(endpoint.longDeltaReason,"entry_long_iv_unavailable");assert.match(endpoint.reason!,/endpoint_short.*entry_long/) });

test("DTE presentation exposes raw and canonical per-day slope with separate Ns",async()=>{const source=await (await import("node:fs/promises")).readFile(new URL("../app/components/volatility-report.tsx",import.meta.url),"utf8");assert.match(source,/value_per_day/);assert.match(source,/Per actual day/);assert.match(source,/shortq.n.*sr.length/)});
