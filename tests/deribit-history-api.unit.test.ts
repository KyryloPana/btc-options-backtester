...
test("an empty successful tape is not a retrieval failure",async()=>{const fetcher=async(input:string|URL|Request)=>{const u=new URL(String(input));if(u.pathname.endsWith("get_delivery_prices"))return json({data:[],records_total:0});if(u.pathname.endsWith("get_instruments"))return json(u.searchParams.get("expired")==="true"?instruments:[]);return json({trades:[]});};const {service,cleanup}=await fixture(fetcher as typeof fetch);try{const r=await service.resolve(entry,[{requestId:"r",targetDte:7,minDte:5,maxDte:10,soldStrike:40000,boughtStrike:39000,optionType:"P"}]);assert.equal(r.complete,true);assert.deepEqual(r.failures,[]);assert.deepEqual(r.diagnostics.failedContracts,[]);assert.equal(r.diagnostics.contractsLoaded,2);assert.equal(r.diagnostics.validTrades,0);}finally{await cleanup();}});
