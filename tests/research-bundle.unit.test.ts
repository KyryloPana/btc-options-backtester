...
import {createServer,type RequestListener} from "node:http";import {mkdtemp,writeFile} from "node:fs/promises";import {tmpdir} from "node:os";import {join} from "node:path";import {unzipSync} from "fflate";
...
test("ZIP is independently listable, extractable, and contains parseable bundle files",()=>{const bundle=buildResearchBundle(store,now),zip=createResearchBundleZip(bundle.files);assert.equal(zip.subarray(0,4).toString("ascii"),"PK\u0003\u0004");const extracted=unzipSync(zip),listed=Object.keys(extracted);assert.deepEqual(listed.sort(),RESEARCH_BUNDLE_FILES.map(name=>`research_bundle/${name}`).sort());for(const name of RESEARCH_BUNDLE_FILES){const text=Buffer.from(extracted[`research_bundle/${name}`]).toString("utf8");if(name.endsWith(".jsonl"))for(const line of text.trim().split("\n").filter(Boolean))assert.doesNotThrow(()=>JSON.parse(line),name);else assert.doesNotThrow(()=>JSON.parse(text),name)}});
