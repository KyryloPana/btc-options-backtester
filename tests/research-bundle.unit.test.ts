...
test("structural configuration identity is canonical and required by schema 4.2",()=>{const bundle=buildResearchBundle(store,now),rows=bundle.files["candidates.jsonl"].trim().split("\n").map(JSON.parse);assert.ok(rows.every((row:any)=>typeof row.structural_configuration_id==="string"&&row.structural_configuration_version==="structural-configuration-v1"));const mutated=structuredClone(bundle.files),first=rows[0];delete first.structural_configuration_id;mutated["candidates.jsonl"]=[JSON.stringify(first),...rows.slice(1).map(JSON.stringify)].join("\n")+"\n";assert.equal(validateResearchBundle(mutated).ok,false)});
