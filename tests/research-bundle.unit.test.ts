...
import {RESEARCH_SELECTION_SCHEMA_VERSION,compactEntryEconomics,generationStructuralConfiguration,migrateResearchSelectionStore,type EvidenceTradeDto,type EvidenceUsageDto} from "../app/lib/research-selections.ts";
import {importResearchBundle} from "../app/lib/research-analysis.ts";
import {reconcileCandidateSpread} from "../app/lib/semantic-spread.ts";
...
import {materializeComparativeEconomics} from "../app/lib/research-matrix.ts";
import {buildResearchEconomicsReport} from "../app/lib/research-economics/report.ts";
import {DEFAULT_ANALYSIS_CONFIGURATION} from "../app/lib/analysis-configuration.ts";import {normalizePositionEconomics} from "../app/lib/economics/position.ts";
...
import {cand,now,referenceOnlyFixture,sel,store,shortStrikeControlledFixture,ts} from "./fixtures/research-selection-store.ts";
const execFileAsync=promisify(execFile);
const addFixtureRanks=(event:any)=>{const groups=new Map<string,any[]>();for(const candidate of event.generationSnapshot.candidates){const id=generationStructuralConfiguration(candidate).id!,rows=groups.get(id)??[];rows.push(candidate);groups.set(id,rows)}for(const rows of groups.values())rows.sort((a,b)=>a.candidateId.localeCompare(b.candidateId)).forEach((candidate,index)=>candidate.expiryRank=index+1)};
...
test("validator rejects broken joins and versions",()=>{const b=buildResearchBundle(store,now);assert.equal(validateResearchBundle({...b.files,"candidates.jsonl":b.files["candidates.jsonl"].replace('"event_id":"e1"','"event_id":"gone"')}).ok,false);const run=JSON.parse(b.files["run.json"]);run.schema_version="9.0.0";assert.equal(validateResearchBundle({...b.files,"run.json":JSON.stringify(run)+"\n"}).ok,false)});
test("current bundles export one rank-1 Comparative Economics materialization per event/configuration",()=>{const fixture=referenceOnlyFixture() as any;for(const event of fixture.events){delete event.comparativeEconomicsStatus;addFixtureRanks(event)}const event=fixture.events[0],selected=event.selectedStructures.find((row:any)=>row.candidateId==="deribit~a");event.selectedStructures=event.selectedStructures.filter((row:any)=>row.candidateId!==selected.candidateId);const configurationId=generationStructuralConfiguration(event.generationSnapshot.candidates.find((row:any)=>row.candidateId===selected.candidateId)).id,attemptCandidateIds=event.generationSnapshot.candidates.filter((row:any)=>generationStructuralConfiguration(row).id===configurationId).map((row:any)=>row.candidateId).sort(),generationAttemptIdentities=event.generationSnapshot.candidates.filter((row:any)=>generationStructuralConfiguration(row).id===configurationId).map((row:any)=>JSON.stringify([row.candidateId,row.requestedStrikes,row.targetHorizon,row.strikeMethod])).sort();const notEvaluated={status:"not_evaluated",reason:"Research-only comparative materialization.",entrySnapshot:null,valuationPathSnapshot:[],outcomeSnapshots:[]};event.researchStructures=[{...selected,selectionId:`research~comparative-economics~e1~${encodeURIComponent(configurationId)}`,researchRole:"comparative_economics",structuralConfigurationId:configurationId,generationAttemptIdentities,referenceValuation:{status:"unavailable",reason:"Fixture Reference unavailable.",source:"unavailable",entrySnapshot:null,valuationPathSnapshot:[],outcomeSnapshots:[],provenance:{}},executionScenarios:{maker:notEvaluated,taker:notEvaluated}}];const bundle=buildResearchBundle(fixture,now),opportunities=bundle.files["configuration_opportunities.jsonl"].trim().split("\n").map(JSON.parse),canonical=opportunities.find((row:any)=>row.event_id==="e1"&&row.structural_configuration_id===configurationId),candidates=bundle.files["candidates.jsonl"].trim().split("\n").map(JSON.parse).filter((row:any)=>row.event_id==="e1"&&row.structural_configuration_id===configurationId),comparative=candidates.filter((row:any)=>row.research_role==="comparative_economics");assert.equal(canonical.candidate_id,"deribit~a");assert.deepEqual(canonical.attempt_candidate_ids,attemptCandidateIds);assert.deepEqual(new Set(comparative.map((row:any)=>row.candidate_id)),new Set(["deribit~a"]));assert.ok(comparative.every((row:any)=>row.is_selected===false));assert.equal(validateResearchBundle(bundle.files).ok,true);const duplicate=comparative.map((row:any)=>JSON.stringify({...row,candidate_id:"deribit~b",structure_execution_id:String(row.structure_execution_id).replace("deribit~a","deribit~b")})).join("\n")+"\n",validation=validateResearchBundle({...bundle.files,"candidates.jsonl":bundle.files["candidates.jsonl"]+duplicate});assert.equal(validation.ok,false);assert.ok(validation.errors.some(error=>error.includes("Multiple Comparative Economics materializations claim")))});
test("producer E2E keeps unselected canonical economics research-only through report import",()=>{const fixture=referenceOnlyFixture() as any;for(const event of fixture.events){delete event.comparativeEconomicsStatus;addFixtureRanks(event);const sources=event.selectedStructures.map((source:any)=>{const reference=source.referenceValuation,entry=reference.entrySnapshot,path=reference.valuationPathSnapshot,outcomes=reference.outcomeSnapshots,track={status:"evaluated",reason:null,reasonCode:null,source:"modeled_execution",modelVersion:"modeled-execution-v5-empirical-taker",entrySnapshot:entry,valuationPathSnapshot:path,outcomeSnapshots:outcomes,provenance:{fixture:true}};return{...source,referenceValuation:reference,modeledExecution:{expected:track,conservative:{...track,entrySnapshot:{...entry,netOpeningCashFlowBtc:(entry?.netOpeningCashFlowBtc??.01)*.9}}}}});event.researchStructures=materializeComparativeEconomics(event.eventId,event.generationSnapshot.candidates,sources,1,now).structures}const event=fixture.events[0],unselected=event.researchStructures.find((row:any)=>row.candidateId==="deribit~a");event.selectedStructures=event.selectedStructures.filter((row:any)=>row.candidateId!==unselected.candidateId);assert.ok(unselected);assert.equal(event.selectedStructures.some((row:any)=>row.candidateId===unselected.candidateId),false);const built=buildResearchBundle(fixture,now),marginRows=built.files["margin_scenarios.jsonl"].trim().split("\n").filter(Boolean).map(JSON.parse);assert.ok(marginRows.some((row:any)=>row.candidate_id===unselected.candidateId),"unselected Comparative Economics materialization exports its own verified margin result or exact unavailable reason");assert.equal(validateResearchBundle(built.files).ok,true);const contradictoryRows=built.files["configuration_opportunities.jsonl"].trim().split("\n").map(JSON.parse).map((row:any)=>row.candidate_id===unselected.candidateId?{...row,status:"unavailable",economic_opportunity:{status:"unavailable",reason_code:"fixture_unavailable",reason:"Fixture contradiction."}}:row),contradictory=validateResearchBundle({...built.files,"configuration_opportunities.jsonl":contradictoryRows.map(JSON.stringify).join("\n")+"\n"});assert.equal(contradictory.ok,false);assert.ok(contradictory.errors.some(error=>error.includes("canonical modeled_expected materialization has a priced entry")));const imported=importResearchBundle(createResearchBundleZip(built.files),"comparative-producer.zip");if(imported.status==="invalid")assert.fail(imported.errors.join("\n"));const report=buildResearchEconomicsReport(imported.dataset);const opportunity=report.canonicalOpportunities.find(row=>row.eventId===event.eventId&&row.candidateId===unselected.candidateId);assert.ok(opportunity);assert.equal(opportunity!.q50GenerationStatus,"trade");assert.ok(report.configurations.find(row=>row.configurationId===opportunity!.structuralConfigurationId)?.trackCoverage.pricedOpportunityN);const candidateRows=imported.dataset.tables.candidates.filter(row=>row.event_id===event.eventId&&row.candidate_id===unselected.candidateId);assert.ok(candidateRows.every(row=>row.is_selected===false&&row.research_role==="comparative_economics"&&((row.modeled_execution as any).expected.status==="evaluated")))});
test("4.5 pipeline exports, validates, imports, and consumes independently indexed USD economics",()=>{
 const fixture=structuredClone(store) as any,s=fixture.events[0].selectedStructures[0],entry=s.entrySnapshot.targetTimestamp,exit=s.outcomeSnapshots.find((x:any)=>x.label==="Settlement").valuationTimestamp;
 s.entrySnapshot={...s.entrySnapshot,entryTargetIndex:60_000,grossSpreadBtc:.01,openingFeesBtc:.001,netOpeningCashFlowBtc:.009};
 s.valuationPathSnapshot=s.valuationPathSnapshot.map((p:any)=>({...p,targetIndex:120_000,closingFeesBtc:.0002,rawClosingFeesBtc:.0002}));
 s.outcomeSnapshots=s.outcomeSnapshots.map((o:any)=>({...o,conversionIndex:o.label==="Settlement"?120_000:80_000,feesBtc:o.label==="Settlement"?.00005:.0002}));
 s.marginSnapshot={status:"available",deployment:{model:"segregated_sm",collateralCurrency:"BTC",accountAssumption:"segregated"},openingInitialMarginBtc:.2,openingMaintenanceMarginBtc:.1,peakInitialMarginBtc:.2,peakMaintenanceMarginBtc:.1,path:[{timestamp:entry,indexPrice:60_000,initialMarginBtc:.2,maintenanceMarginBtc:.1},{timestamp:entry+86_400_000,indexPrice:80_000,initialMarginBtc:.19,maintenanceMarginBtc:.095},{timestamp:exit,indexPrice:120_000,initialMarginBtc:.1,maintenanceMarginBtc:.05}]};
 const built=buildResearchBundle(fixture,now),candidates=built.files["candidates.jsonl"].trim().split("\n").map(JSON.parse),valuations=built.files["valuations.jsonl"].trim().split("\n").map(JSON.parse),outcomes=built.files["outcomes.jsonl"].trim().split("\n").map(JSON.parse),margins=built.files["margin_scenarios.jsonl"].trim().split("\n").map(JSON.parse),candidate=candidates.find((x:any)=>x.candidate_id===s.candidateId&&x.execution_scenario==="taker"),valuation=valuations.find((x:any)=>x.target_underlying_index===120_000&&x.closing_fees_native!==null),settlement=outcomes.find((x:any)=>x.candidate_id===s.candidateId&&x.outcome_type==="settlement"),margin=margins.find((x:any)=>x.candidate_id===s.candidateId);
 assert.equal(candidate.opening_fees_usd,60);assert.equal(valuation.closing_fees_usd,24);assert.equal(settlement.delivery_fees_usd,6);assert.deepEqual(margin.margin_inputs.path.map((x:any)=>x.initial_margin_usd),[12_000,15_200,12_000]);assert.equal(margin.peak_initial_margin,0.2);assert.equal(margin.peak_initial_margin_usd,15_200);assert.notEqual(margin.peak_initial_margin_timestamp_utc,margin.peak_initial_margin_usd_timestamp_utc);assert.equal(validateResearchBundle(built.files).ok,true);
 const imported=importResearchBundle(createResearchBundleZip(built.files),"usd-pipeline.zip");if(imported.status==="invalid")assert.fail(imported.errors.join("\n"));const positions=normalizePositionEconomics(imported.dataset,{...DEFAULT_ANALYSIS_CONFIGURATION,executionAssumption:"taker",pricingTrack:"raw_vwap",exitPolicy:"settlement_benchmark"}),position=positions.find(x=>x.candidateId===s.candidateId);assert.ok(position);assert.equal(position!.openingFeesUsd.value,60);assert.equal(position!.totalRealizedFeesUsd.value,null,"an incomplete routed component set is unavailable rather than converted at one common index");assert.equal((imported.dataset.tables.margin_scenarios.find(x=>x.candidate_id===s.candidateId)!.peak_initial_margin_usd),15_200);
 const mutated={...built.files,"candidates.jsonl":built.files["candidates.jsonl"].replace('"opening_fees_usd":60','"opening_fees_usd":61')};assert.equal(validateResearchBundle(mutated).ok,false);
});
...
test("validator adversarial rejection matrix",()=>{const base=buildResearchBundle(store,now).files,mutate=(file:string,fn:(rows:any[])=>void)=>{const copy={...base},rows=file==="run.json"?[JSON.parse(copy[file])]:copy[file].trim().split("\n").map(JSON.parse);fn(rows);copy[file]=file==="run.json"?JSON.stringify(rows[0])+"\n":rows.map(JSON.stringify).join("\n")+"\n";return validateResearchBundle(copy)};
 const cases:[string,ReturnType<typeof validateResearchBundle>][]=[
  ...
  ["unreconciled canonical opening fee USD",mutate("candidates.jsonl",r=>{const x=r.find(x=>x.opening_fees_usd!==null);x.opening_fees_usd+=1})],
  ["unreconciled canonical valuation fee USD",mutate("valuations.jsonl",r=>{const x=r.find(x=>x.closing_fees_usd!==null);x.closing_fees_usd+=1})],
...
test("bear-call structures survive persistence, bundle and import without semantic leg reversal",()=>{
 const bundle=buildResearchBundle(shortStrikeControlledFixture(),now),result=importResearchBundle(createResearchBundleZip(bundle.files),"roundtrip.zip");assert.notEqual(result.status,"invalid");if(result.status==="invalid")return;
 const calls=result.dataset.tables.candidates.filter(c=>c.option_type==="C");assert.ok(calls.length>0);for(const candidate of calls){const semantic=reconcileCandidateSpread({...candidate});assert.equal(semantic.valid,true);const strikes=semantic.row.actual_strikes as Record<string,number>;assert.ok(strikes.short<strikes.long)}
 assert.equal(result.semanticDiagnostics.filter(d=>d.reason.includes("bear call")).length,0);
});
...
test("research bundle endpoint fails actionably when required local volatility configuration is absent",async()=>{const dir=await mkdtemp(join(tmpdir(),"bundle-http-"));await new ResearchSelectionService(dir).save(store.datasetId,store);let middleware:RequestListener|undefined;researchBundleApiPlugin(dir).configureServer?.({middlewares:{use(handler:RequestListener){middleware=handler}}} as any);assert.ok(middleware);const server=createServer(middleware);await new Promise<void>(resolve=>server.listen(0,"127.0.0.1",resolve));try{const address=server.address();assert.ok(address&&typeof address!=="string");const response=await fetch(`http://127.0.0.1:${address.port}/__local/research-bundle/${store.datasetId}`),bytes=Buffer.from(await response.arrayBuffer());assert.equal(response.status,422);assert.equal(response.headers.get("content-type"),"application/json");assert.match(bytes.toString(),/BTC_OPTIONS_LOCAL_DATA_ROOT/)}finally{await new Promise<void>((resolve,reject)=>server.close(error=>error?reject(error):resolve()))}});
...
test("structural configuration identity is canonical and required by schema 4.3+",()=>{const bundle=buildResearchBundle(store,now),rows=bundle.files["candidates.jsonl"].trim().split("\n").map(JSON.parse);assert.ok(rows.every((row:any)=>typeof row.structural_configuration_id==="string"&&row.structural_configuration_version==="structural-configuration-v2"));const mutated=structuredClone(bundle.files),first=rows[0];delete first.structural_configuration_id;mutated["candidates.jsonl"]=[JSON.stringify(first),...rows.slice(1).map(JSON.stringify)].join("\n")+"\n";assert.equal(validateResearchBundle(mutated).ok,false)});

test("current schema rejects corrupted ex-ante configuration identity but ignores realized changes",()=>{const bundle=buildResearchBundle(structuredClone(store),now),candidate=JSON.parse(bundle.files["candidates.jsonl"].split("\n")[0]);candidate.structural_configuration={...candidate.structural_configuration,requestedWidth:candidate.structural_configuration.requestedWidth+1};const corrupted={...bundle.files,"candidates.jsonl":JSON.stringify(candidate)+"\n"+bundle.files["candidates.jsonl"].split("\n").slice(1).join("\n")};assert.equal(validateResearchBundle(corrupted).ok,false);const realized=JSON.parse(bundle.files["candidates.jsonl"].split("\n")[0]);realized.actual_strikes={...realized.actual_strikes,width:999999};realized.actual_dte=999;const changed={...bundle.files,"candidates.jsonl":JSON.stringify(realized)+"\n"+bundle.files["candidates.jsonl"].split("\n").slice(1).join("\n")};assert.equal(validateResearchBundle(changed).ok,true);assert.equal(structuralConfigurationIdentity(realized).id,realized.structural_configuration_id)});

test("collapsed actual structures remain distinct Comparative Economics observations",()=>{
 const fixture=referenceOnlyFixture() as any,event=fixture.events[0],source=event.selectedStructures[0],base=event.generationSnapshot.candidates.find((candidate:any)=>candidate.candidateId===source.candidateId);
 source.candidateSnapshot={...source.candidateSnapshot,targetDte:14};const actual={...base.actualStrikes},candidateId=base.candidateId;
 const width2000={...base,candidateId,selected:false,targetHorizon:14,strikeMethod:"anchor",requestedStrikes:{short:actual.short,long:(actual.short??0)-2000,width:2000},actualStrikes:actual,expiryRank:1,expirySelectionReason:"causal fixture rank 1"};
 const width3000={...base,candidateId,selected:false,targetHorizon:14,strikeMethod:"anchor",requestedStrikes:{short:actual.short,long:(actual.short??0)-3000,width:3000},actualStrikes:actual,expiryRank:1,expirySelectionReason:"causal fixture rank 1"};
 event.generationSnapshot.candidates=[width2000,width3000];event.selectedStructures=[];
 const materialized=materializeComparativeEconomics(event.eventId,event.generationSnapshot.candidates,[source],source.quantity,now).structures;
 assert.equal(materialized.length,2);assert.equal(new Set(materialized.map(row=>row.candidateId)).size,1);assert.equal(new Set(materialized.map(row=>row.structuralConfigurationId)).size,2);assert.ok(materialized.every(row=>row.generationAttemptIdentities?.length===1));
 event.researchStructures=materialized;fixture.schemaVersion=RESEARCH_SELECTION_SCHEMA_VERSION;
 const persisted=migrateResearchSelectionStore(JSON.parse(JSON.stringify(fixture)));assert.equal(persisted.events[0]!.researchStructures?.length,2);
 const bundle=buildResearchBundle(persisted,now),validation=validateResearchBundle(bundle.files);assert.equal(validation.ok,true,validation.errors?.join("\n"));
 const opportunities=bundle.files["configuration_opportunities.jsonl"].trim().split("\n").map(JSON.parse).filter(row=>row.event_id===event.eventId);assert.equal(opportunities.length,2);assert.equal(new Set(opportunities.map(row=>row.candidate_id)).size,1);assert.equal(new Set(opportunities.map(row=>row.opportunity_id)).size,2);
 const imported=importResearchBundle(createResearchBundleZip(bundle.files),"collapsed-widths.zip");if(imported.status==="invalid")assert.fail(imported.errors.join("\n"));const report=buildResearchEconomicsReport(imported.dataset);const reportRows=report.canonicalOpportunities.filter(row=>row.eventId===event.eventId);assert.equal(reportRows.length,2);assert.equal(new Set(reportRows.map(row=>row.structuralConfigurationId)).size,2);
});
