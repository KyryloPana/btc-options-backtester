...
test("timestamp contract keeps raw evidence separate from executable windows",()=>{
 ...
 for(const id of ids){assert.ok(candidates.find(x=>x.candidate_id===id));assert.ok(vals.filter(x=>x.candidate_id===id).every(x=>x.window_role==="executable_observation"));assert.ok(outcomes.some(x=>x.candidate_id===id&&x.outcome_type==="fixed_14d"&&x.window_role==="outside_executable_window"&&x.valuation_timestamp_utc===null));}
