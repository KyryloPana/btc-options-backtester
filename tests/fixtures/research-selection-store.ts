...
/** Exact 95,700 bearish-MR controlled cohort used by the schema-4.1 acceptance tests. */
export function shortStrikeControlledFixture(){
 const fixture=referenceOnlyFixture(),event=fixture.events[0]!,selected=structuredClone(event.selectedStructures[0]!),expiry=ts+14*864e5;
 const configure=(value:typeof selected,id:string,short:number,long:number,credit:number)=>{value.candidateId=id;value.strategyVariantId=id;value.eventId="e-short";value.candidateSnapshot={...(value.candidateSnapshot as Record<string,unknown>),structure:"credit",spreadKind:"credit",optionType:"C",expiryTimestamp:expiry,targetDte:14,actualDte:14,shortStrike:short,longStrike:long,actualWidth:2000,strikeMethod:short===97000?"buffered":"anchor"};value.contractResolution={status:"exact_resolved",reason:null,short:{instrumentName:`BTC-${short}-C`,creationTimestamp:ts-1,expirationTimestamp:expiry,strike:short,optionType:"C",contractSize:1,source:"fixture",retrievedAtUtc:now,authoritative:true},long:{instrumentName:`BTC-${long}-C`,creationTimestamp:ts-1,expirationTimestamp:expiry,strike:long,optionType:"C",contractSize:1,source:"fixture",retrievedAtUtc:now,authoritative:true}};const reference=value.referenceValuation! as Record<string,unknown>,entry=reference.entrySnapshot as Record<string,unknown>;reference.entrySnapshot={...entry,targetTimestamp:ts,valuationTimestamp:ts,entryTargetIndex:95000,grossSpreadBtc:credit,openingFeesBtc:.0001,netOpeningCashFlowBtc:credit-.0001,sold:{priceBtcPerContract:credit+.002},bought:{priceBtcPerContract:.002}};reference.valuationPathSnapshot=[{timestamp:ts,status:"priced",targetIndex:95000,estimatedNetPnlBtc:0,estimatedNetPnlUsd:0},{timestamp:ts+36e5,status:"priced",targetIndex:95500,estimatedNetPnlBtc:short===96000?-.002:-.001,estimatedNetPnlUsd:short===96000?-191:-95.5}];reference.outcomeSnapshots=[{label:"Settlement",status:"estimated",decisionTimestamp:expiry,valuationTimestamp:expiry,estimatedNetPnlBtc:short===96000?.0015:.001,estimatedNetPnlUsd:short===96000?150:100,conversionIndex:100000}];return value};
 const technicalId="deribit~short-technical",bufferedId="deribit~short-buffered",technical=configure(selected,technicalId,96000,98000,.006),researchTechnical=configure(structuredClone(selected),technicalId,96000,98000,.005),researchBuffered=configure(structuredClone(selected),bufferedId,97000,99000,.004);
 for(const research of [researchTechnical,researchBuffered]){research.selectionId=`research~${research.candidateId}`;research.selectedAtUtc=now;research.marginSnapshot=null;delete research.modeledExecution;delete research.delayedExecution;research.executionScenarios={maker:{status:"not_evaluated",reason:"Controlled research only.",entrySnapshot:null,valuationPathSnapshot:[],outcomeSnapshots:[]},taker:{status:"not_evaluated",reason:"Controlled research only.",entrySnapshot:null,valuationPathSnapshot:[],outcomeSnapshots:[]}}}
 const generation=(id:string,short:number,long:number,strikeMethod:"anchor"|"buffered")=>({candidateId:id,venue:"deribit" as const,selected:id===technicalId,status:"priced" as const,availabilityReasons:[],targetHorizon:14,eligibleDteRange:{min:12,max:16},actualExpiryTimestamp:expiry,actualDte:14,requestedStrikes:{short,long,width:2000},actualStrikes:{short,long,width:2000},structure:"credit",optionType:"C",strikeMethod,entryQuality:"green" as const});
 fixture.events=[{...event,eventId:"e-short",sourceRun:{event:{id:"e-short",direction:"short",entryDate:"2026-08-16",entryTimestamp:ts,entryPrice:95000,extremePrice:95700,rangeLow:94000,rangeHigh:96000}},generationSnapshot:{...event.generationSnapshot,candidates:[generation(technicalId,96000,98000,"anchor"),generation(bufferedId,97000,99000,"buffered")],underlyingHourlyPath:[{openTime:ts,closeTime:ts+36e5,open:95000,high:96500,low:94900,close:95500,volume:1}]},selectedStructures:[technical],researchStructures:[{...researchTechnical,researchRole:"short_strike_technical"},{...researchBuffered,researchRole:"short_strike_buffered"}]}];
 return fixture;
}
