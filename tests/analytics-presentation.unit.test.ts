...
test("PRESENTATION: scoped configuration controls edit only genuine report inputs",()=>{
 ...
 assert.doesNotMatch(shell,/onSelectConfiguration/,"Strategy economics cannot select a configuration from historical results");
...
test("PRESENTATION: paired execution monetary deltas honor display currency without fallback",()=>{const source=readFileSync(new URL("../app/components/economic-analysis-report.tsx",import.meta.url),"utf8"),panel=source.slice(source.indexOf('title="Paired Reference → Q50 → Q90"'),source.indexOf('title="Selected configuration economics"'));for(const field of ["GrossCredit","NetCredit","OpeningFees","Pnl","TrackMaximumNetLoss"]){assert.match(panel,new RegExp(`btc=\\{x\\.medianDelta${field}Btc\\}`),`${field} supplies canonical BTC`);assert.match(panel,new RegExp(`usd=\\{x\\.medianDelta${field}Usd\\}`),`${field} supplies canonical USD`)}assert.doesNotMatch(panel,/btc\(x\.medianDelta/);assert.equal(panel.match(/allowFallback=\{false\}/g)?.length,5);assert.match(panel,/USD paired n=/);assert.match(panel,/common N=/)});

test("PRESENTATION: collateral USD MTM is not labelled strategy USD-PnL drawdown",()=>{const source=readFileSync(new URL("../app/components/economic-analysis-report.tsx",import.meta.url),"utf8");assert.match(source,/Native BTC collateral MTM drawdown/);assert.match(source,/includes collateral spot beta and is not strategy USD-PnL drawdown/);});

test("Strategy fee-drag KPI discloses effective metric-position coverage",()=>{const source=readFileSync(new URL("../app/components/economic-analysis-report.tsx",import.meta.url),"utf8");assert.match(source,/selected\.feeDragCoverage\.n/);assert.match(source,/priced positions/)});
