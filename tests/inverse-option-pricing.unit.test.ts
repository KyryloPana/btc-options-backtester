...
import { MILLISECONDS_PER_YEAR, priceInverseOption } from "../app/lib/inverse-option-pricing.ts";
import { parseContractText, type ContractSeries } from "../app/lib/backtester.ts";
import { selectIvAnchor } from "../app/lib/research-valuation.ts";
const now=Date.UTC(2026,0,1), expiry=now+MILLISECONDS_PER_YEAR;
const priced=(optionType:"call"|"put",overrides={})=>priceInverseOption({optionType,indexPrice:100,strike:100,valuationTimestamp:now,expiryTimestamp:expiry,ivDecimal:.2,...overrides});
test("call and put match independent zero-rate Black-Scholes references and BTC conversion",()=>{const call=priced("call"),put=priced("put");assert.equal(call.status,"priced");assert.equal(put.status,"priced");if(call.status==="priced"&&put.status==="priced"){assert.ok(Math.abs(call.priceUsd-7.96556746)<2e-5);assert.ok(Math.abs(put.priceUsd-7.96556746)<2e-5);assert.ok(Math.abs(call.priceBtc-.0796556746)<2e-7);}});
test("zero-rate call-put parity",()=>{const call=priced("call",{strike:90}),put=priced("put",{strike:90});if(call.status==="priced"&&put.status==="priced")assert.ok(Math.abs((call.priceBtc-put.priceBtc)-(100-90)/100)<1e-10);else assert.fail("prices unavailable");});
test("expiry uses inverse intrinsic including puts above one BTC",()=>{const call=priced("call",{indexPrice:120,strike:100,valuationTimestamp:expiry}),put=priced("put",{indexPrice:40,strike:100,valuationTimestamp:expiry});assert.deepEqual(call.status==="priced"&&call.priceBtc,1/6);assert.deepEqual(put.status==="priced"&&put.priceBtc,1.5);});
