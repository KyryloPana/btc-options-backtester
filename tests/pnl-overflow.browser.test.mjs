...
test("generated PnL table and expanded payoff stay inside their block", async () => {
  ...
  try {
      ...
      block.innerHTML = `
        <div class="table-card card"><div class="table-scroll">
          <table class="pnl-results-table"><thead><tr>
            <th>Research</th><th>⌄</th><th>Status</th><th>Spread and strikes</th><th>Expiry</th><th>Width</th>
            ...
            <td><label class="research-select"><input type="checkbox" checked /><small>SAVED</small></label></td><td><button class="expand-button" aria-expanded="true">⌃</button></td><td><div class="status-stack"><span>MODEL ✓</span><span>MKR —</span><span>TKR ✓</span></div></td>
            ...
            <td><label class="research-select"><input type="checkbox" disabled /></label></td><td><button class="expand-button">⌄</button></td><td><div class="status-stack"><span>MODEL —</span><span>MKR —</span><span>TKR —</span></div></td>
            ...
            <td class="positive">$123,456.78</td><td class="negative">−$98,765.43</td><td>Estimated valuation</td>
          </tr><tr class="ledger-detail-row"><td colspan="11"><div class="expanded-structure"><div class="evidence-panel">
            <section class="evidence-block"><h4>Exact contracts</h4><dl><div><dt>Contracts</dt><dd class="mono">BTC-29DEC28-100000-P / BTC-29DEC28-90000-P</dd></div></dl></section>
            <section class="evidence-block primary-evidence"><h4>Model valuation</h4><p>Theoretical, not executable.</p></section><section class="evidence-block"><h4>Maker opportunity</h4></section><section class="evidence-block"><h4>Taker execution</h4></section></div><div class="payoff-panel"><section class="expiry-payoff"><h4>Expiry payoff</h4><div class="payoff-plot"><svg viewBox="0 0 600 230" aria-label="Payoff chart"><path d="M0 180 L300 180 L450 50 L600 50" class="payoff-curve"/></svg></div>
            ...
            </section></div></div></td></tr></tbody></table>
        </div>`;
    });
    ...
    for (const width of viewports) {
      ...
      const result = await page.locator("[data-testid=pnl-block]").evaluate(block => {
        ...
        return {
          ...
          collapsedParagraphs: block.querySelectorAll("tbody > tr:not(.ledger-detail-row) p").length,
          evidenceBlocks: block.querySelectorAll(".ledger-detail-row .evidence-block").length,
        };
      });
      ...
      assert.equal(result.collapsedParagraphs, 0, "collapsed rows must not contain explanations");
      assert.equal(result.evidenceBlocks, 4, "all four evidence tracks belong in the expanded row");
    ...
    await page.locator("[data-testid=pnl-block]").screenshot({ path: "artifacts/pnl-block-640.png" });
  } finally {
    await browser.close();
  }
});
