...
test("SCHEMA 4.1 E2E: selected technical and unselected buffered survive without contaminating selected analytics",()=>{
 ...
 assert.equal(technical.length,4);assert.ok(technical.filter(r=>r.is_selected===true).every(r=>r.research_role===null));assert.ok(technical.filter(r=>r.is_selected===false).every(r=>r.research_role==="short_strike_technical"));assert.ok(buffered.every(r=>r.is_selected===false&&r.research_role==="short_strike_buffered"));
...
test("MATERIALIZATIONS: research Reference and selected execution survive without merging",()=>{
 ...
 const bundle=buildResearchBundle(fixture,now),rows=parse(bundle.files["candidates.jsonl"]).filter(r=>r.candidate_id===selected.candidateId),selectedRows=rows.filter(r=>r.is_selected===true),researchRows=rows.filter(r=>r.research_role==="short_strike_technical");assert.equal(new Set(rows.map(r=>r.materialization_id)).size,2);assert.ok(selectedRows.every(r=>((r.reference_valuation as {entrySnapshot:{netOpeningCashFlowBtc:unknown}}).entrySnapshot.netOpeningCashFlowBtc)===selectedReference&&r.execution_scenario_status==="evaluated"));assert.ok(researchRows.every(r=>((r.reference_valuation as {entrySnapshot:{netOpeningCashFlowBtc:unknown}}).entrySnapshot.netOpeningCashFlowBtc)===researchReference&&r.execution_scenario_status==="not_evaluated"));
});
...
test("VERSIONING: the schema is bumped and old execution-gated bundles are not reinterpreted",()=>{
 ...
 const stale={...bundle.files,"run.json":bundle.files["run.json"].replace('"4.5.0"','"3.6.0"')};
