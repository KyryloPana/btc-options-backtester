...
test("all prior schema versions are recognized as migratable legacy versions",()=>{
 assert.deepEqual([...LEGACY_RESEARCH_SELECTION_SCHEMA_VERSIONS],["1.0.0","1.1.0","1.2.0","1.3.0","1.4.0","1.5.0","1.6.0","1.7.0","1.8.0","1.9.0"]);
...
test("comparative research structures cross the canonical compaction boundary",()=>{
 const runtime=event("comparative-runtime",["strategy"]),base=selected("comparative-runtime","strategy"),path=Array.from({length:181},(_,index)=>({timestamp:1+index*14_400_000,estimatedNetPnlBtc:.01-index/100_000})),trade={instrumentName:"BTC-RUNTIME",tradeId:"runtime-evidence",timestamp:1_700_000_000_000,price:.01,amount:1,direction:"sell"},calibration={scenario:"taker",status:"empirically_calibrated",count:200,minimum:30,sampleFingerprint:"canonical-fingerprint",records:Array.from({length:200},(_,index)=>({index,trade}))};
...
test("FULL-SPACE Comparative Economics event stays below 8 MB after research-only Reference compaction",()=>{
 const production=event("comparative-full-space",["manual-strategy"]),entry=1_800_000_000_000-30*86_400_000;
 const dtes=[7,14,30],methods=["anchor","buffered"],widths=[1000,2000,3000],research=[] as any[],candidates=[] as any[];
 for(const targetDte of dtes)for(const strikeMethod of methods)for(const width of widths){
  const candidateId=`ce-${targetDte}-${strikeMethod}-${width}`,pointN={7:43,14:85,30:181}[targetDte as 7|14|30],expiryTimestamp=entry+targetDte*86_400_000;
  const path=Array.from({length:pointN},(_,index)=>{const timestamp=entry+index*14_400_000,targetIndex=60_000-index*7;const leg=(side:string)=>({instrumentName:`BTC-${side}-${expiryTimestamp}`,economicSide:side,priceBtcPerContract:.02,unslippedPriceBtcPerContract:.0199,source:"same_expiry_linear_interpolation",supportingTimestamps:[timestamp-1000,timestamp-500],observedAmount:12,nearestGapMinutes:.2,model:{anchorTimestamp:timestamp-1000,anchorTradeId:`trade-${index}`,anchorIvApiPercent:55,anchorIvDecimal:.55,anchorIndex:targetIndex,targetIndex,dte:targetDte,forwardRateAssumption:.01,modelPriceBtc:.02,modelName:"inverse-black-scholes",repeatedMarketIvEvidence:"x".repeat(750)}});return{timestamp,status:"priced",targetIndex,indexResolution:{sourceCandleTimestamp:timestamp,lookupMethod:"exact-hour",runtimeCandle:{open:targetIndex,high:targetIndex+100,low:targetIndex-100,close:targetIndex}},rawEstimate:{status:"priced",sold:leg("sold"),bought:leg("bought"),estimateQuality:"green"},modelEstimate:{status:"priced",sold:leg("sold"),bought:leg("bought"),estimateQuality:"green"},closingSpreadValueBtcPerContract:.01,closingSpreadValueBtc:.01,closingFeesBtc:.0001,estimatedNetPnlBtc:.004,rawClosingSpreadValueBtcPerContract:.0101,rawClosingSpreadValueBtc:.0101,rawClosingFeesBtc:.0001,rawEstimatedNetPnlBtc:.0039,estimateQuality:"green"}});
  const base=selected(production.eventId,candidateId),configurationId=`${targetDte}d-${strikeMethod}-${width}`;
  research.push({...base,selectionId:`research~${configurationId}`,researchRole:"comparative_economics",structuralConfigurationId:configurationId,attemptCandidateIds:[candidateId],candidateSnapshot:{structure:"bull_put_credit",optionType:"P",targetDte,actualDte:targetDte-.15,expiryTimestamp,shortStrike:60_000-width,longStrike:60_000-2*width,actualWidth:width},executionScenarios:{maker:notEvaluatedScenario(),taker:notEvaluatedScenario()},referenceValuation:{status:"valued",reason:null,source:"same_expiry_linear_interpolation",entrySnapshot:{status:"priced",valuationTimestamp:entry,targetTimestamp:entry,entryTargetIndex:60_000,grossSpreadBtc:.01,openingFeesBtc:.0001,netOpeningCashFlowBtc:.0099,sold:{instrumentName:"S",priceBtcPerContract:.02},bought:{instrumentName:"L",priceBtcPerContract:.01}},valuationPathSnapshot:path,outcomeSnapshots:["3D","50% credit","70% credit","90% credit","invalidation","thesis","settlement"].map((label,index)=>({label,status:"estimated",decisionTimestamp:entry+index*86_400_000,valuationTimestamp:entry+index*86_400_000,targetIndex:59_000,estimatedNetPnlBtc:.001*index,feesBtc:.0001,estimateQuality:"green"})),provenance:{executionIndependent:true,methodVersion:"reference-v5"}},modeledExecution:{expected:{status:"evaluated",entrySnapshot:{netOpeningCashFlowBtc:.009},outcomeSnapshots:[{label:"settlement",estimatedNetPnlBtc:.003}],provenance:{artifactHash:"q50"}},conservative:{status:"evaluated",entrySnapshot:{netOpeningCashFlowBtc:.008},outcomeSnapshots:[{label:"settlement",estimatedNetPnlBtc:.002}],provenance:{artifactHash:"q90"}}},marginSnapshot:{status:"available",openingInitialMarginUsd:500,peakInitialMarginUsd:750,path:[{timestamp:entry,initialMarginUsd:500},{timestamp:expiryTimestamp,initialMarginUsd:750}]}});
  candidates.push({candidateId,venue:"deribit",selected:false,status:"priced",availabilityReasons:[],targetHorizon:targetDte,eligibleDteRange:{min:targetDte-2,max:targetDte+3},actualExpiryTimestamp:expiryTimestamp,actualDte:targetDte-.15,requestedStrikes:{short:60_000-width,long:60_000-2*width,width},actualStrikes:{short:60_000-width,long:60_000-2*width,width},structure:"bull_put_credit",optionType:"P",strikeMethod,entryQuality:"green",expiryRank:1});
 }
 production.researchStructures=research;production.generationSnapshot.candidates.push(...candidates);
 const before=researchEventPayloadDiagnostics(production),compacted=compactResearchSelectionEvent(production),after=researchEventPayloadDiagnostics(compacted);
 assert.equal(research.length,18);assert.deepEqual(research.map(row=>row.candidateSnapshot.targetDte).sort((a,b)=>a-b),[...Array(6).fill(7),...Array(6).fill(14),...Array(6).fill(30)]);
 assert.ok(before.totalBytes>10_600_000,`production-like fixture must reproduce the 10.6 MB failure, got ${before.totalBytes}`);
 assert.ok(after.totalBytes<8_000_000,`full-space compact payload must retain growth headroom, got ${after.totalBytes}`);assert.ok(after.totalBytes<10_000_000);
 assert.ok(before.comparativeEconomicsResearchStructuresBytes>before.totalBytes*.9);assert.ok(after.researchCandidateBytes.every(row=>row.reference.pathPointCount>0));assert.ok(after.researchCandidateBytes.every(row=>row.referencePathBytes<row.referenceValuationBytes));
 assert.equal(after.researchCandidateBytes[0]!.targetDte,7);assert.equal(after.researchCandidateBytes[0]!.actualDte,6.85);assert.equal(after.top10LargestComponents.length,10);
 assert.doesNotMatch(JSON.stringify(compacted.researchStructures),/"rawEstimate"|"modelEstimate"|repeatedMarketIvEvidence|runtimeCandle/);
 assert.deepEqual(compacted.selectedStructures,production.selectedStructures,"selected Strategy persistence is byte-semantic and bypasses the research DTO");
 // All persisted economic endpoints and states are independent of the verbose
 // per-leg path reconstruction and must reconcile exactly.
 const economics=(value:ResearchSelectionEvent)=>value.researchStructures!.map(row=>{const modeled=row.modeledExecution as any;return{candidateId:row.candidateId,configurationId:row.structuralConfigurationId,referenceEntry:row.referenceValuation!.entrySnapshot,referenceOutcomes:row.referenceValuation!.outcomeSnapshots,modeledExpected:{status:modeled.expected.status,entrySnapshot:modeled.expected.entrySnapshot,outcomeSnapshots:modeled.expected.outcomeSnapshots,provenance:modeled.expected.provenance},modeledConservative:{status:modeled.conservative.status,entrySnapshot:modeled.conservative.entrySnapshot,outcomeSnapshots:modeled.conservative.outcomeSnapshots,provenance:modeled.conservative.provenance},margin:row.marginSnapshot}});
 assert.deepEqual(economics(compacted),economics(production));
 console.info("FULL-SPACE payload audit",JSON.stringify({before,after},null,2));
});
...
test("schema 1.9 compact DTOs bound delayed tape, calibration, and modeled path growth",()=>{
 ...
 const modeledTrack={status:"evaluated",penaltyBps:100,calibration,entrySnapshot:{netOpeningCashFlowBtc:.01},valuationPathSnapshot:path,outcomeSnapshots:[{label:"expiry",estimatedNetPnlBtc:.005}]};
 const modeled={expected:modeledTrack,conservative:modeledTrack,penaltySensitivity:{status:"evaluated",calibration,grid:[0,50,100,250,500].map(penaltyBps=>({penaltyBps,track:modeledTrack}))}};
 const compactModeled=compactModeledExecution(modeled);assert.doesNotMatch(JSON.stringify(compactModeled),/\"records\"|\"valuationPathSnapshot\":/);assert.ok(Buffer.byteLength(JSON.stringify(compactModeled))<Buffer.byteLength(JSON.stringify(modeled))/20);
 const attempt={window:{id:"0-2h",startMs:0,endMs:7200000},scenario:"taker",status:"unavailable",reasonCode:"synchronization-failure",preEntryResolution:"no-qualifying-execution-before-maximum-delay",shortLeg:{tradeCount:100,supportedAmount:1,supportingTrades:trades},longLeg:{tradeCount:100,supportedAmount:1,supportingTrades:trades},postEntryPath:path};const usages=[] as import("../app/lib/research-selections.ts").EvidenceUsageDto[],catalog=new Map();
 const compactDelayed=compactDelayedExecution({version:"v2",policy:{id:"p",version:"v2",windows:[]},maker:{status:"unavailable",attempts:[attempt]},taker:{status:"unavailable",attempts:[attempt]}},"deribit","c1",usages,catalog);assert.doesNotMatch(JSON.stringify(compactDelayed),/supportingTrades|postEntryPath/);assert.equal(catalog.size,0,"failed diagnostic attempts do not recursively persist raw tape");
});

test("empirical modeled provenance survives compact persistence without losing causal audit fields",()=>{const provenance={historicalExecution:false,datasetFingerprint:"dataset-fp",artifactHash:"artifact-hash",causalCutoffExclusive:1_700_000_000_000,bucketPolicyVersion:"taker-action-dte-amount-v1",description:"per-leg empirical P90 taker concession plus explicit Reference-IV uncertainty stress; not joint spread P90",legs:[{bucketFallbackLevel:"action_dte",tradeCount:501,calendarDayCount:11,expiryDayGroupCount:21}]};const compact=compactModeledExecution({expected:{status:"evaluated",provenance},conservative:{status:"evaluated",provenance},penaltySensitivity:{status:"evaluated"}}) as Record<string,unknown>;for(const name of ["expected","conservative"]){const saved=(compact[name] as Record<string,unknown>).provenance;assert.deepEqual(saved,provenance)}});

test("event persistence route returns structured HTTP 413 at the unchanged 10 MB guard",async()=>{
 const dir=await mkdtemp(join(tmpdir(),"research-limit-"));let middleware:RequestListener|undefined;researchSelectionApiPlugin(dir).configureServer?.({middlewares:{use(handler:RequestListener){middleware=handler}}} as never);assert.ok(middleware);const server=createServer(middleware);await new Promise<void>(resolve=>server.listen(0,"127.0.0.1",resolve));try{const address=server.address();assert.ok(address&&typeof address!=="string");const response=await fetch(`http://127.0.0.1:${address.port}/__local/research-selections/default-sample-trades/events/event-a`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({eventId:"event-a",padding:"x".repeat(10_000_000)})});assert.equal(response.status,413);const payload=await response.json() as {error:string;details:{receivedBytes:number;limitBytes:number}};assert.equal(payload.details.limitBytes,10_000_000);assert.ok(payload.details.receivedBytes>payload.details.limitBytes);assert.match(payload.error,/exceeds 10 MB limit/);}finally{server.close();await rm(dir,{recursive:true,force:true});}
});
