...
function normalizedLayers(dataset:AnalysisDataset,researchExitPolicy:PrimaryExitPolicy|null):ResearchLayerPositions{
 // Thesis is used only as a carrier for policy-independent opening fields when
 // no Research exit is selected; all policy-window fields are removed below.
 const carrier=researchExitPolicy??"thesis";
 return Object.fromEntries(layers.map(([name,track])=>[name,normalizePositionEconomics(datasetForAnalyticsTrack(dataset,track),{...DEFAULT_ANALYSIS_CONFIGURATION,exitPolicy:carrier})])) as unknown as ResearchLayerPositions;
}
...
export function projectResearchEconomicObservationsFromPositions(dataset:AnalysisDataset,researchExitPolicy:PrimaryExitPolicy|null,positionsByLayer:ResearchLayerPositions):readonly ResearchEconomicObservation[]{
 ...
 return layers.flatMap(([analyticalTrack])=>opportunities.map(row=>{
  ...
  if(declared.status!=="trade"||!position||ambiguity||!validIdentity){const noTrade=declared.status==="explicit_no_trade",reason=noTrade?declared.reason??"Canonical opportunity is an explicit no-trade.":declared.status==="unavailable"?declared.reason??"Canonical economic opportunity is unavailable.":baseReason!;return deriveUsdMetrics({eventId,candidateId:declaredCandidate,structuralConfigurationId:configurationId,direction:position?.direction??"unknown",dteFamily:configuration.targetDteFamilyDays===null?null:`${configuration.targetDteFamilyDays}D`,actualDteDays:position?.actualDteDays??null,strikeMethod:configuration.strikeMethod,requestedWidth:configuration.requestedWidth,actualWidth:position?.actualWidth??null,structureFamily:configuration.structureFamily,exitPolicy:researchExitPolicy,analyticalTrack,analyticsTrack:position?.analyticsTrack??null,eligible:true,coverageStatus:noTrade?"explicit_no_trade":"unavailable",reasonCode:noTrade?declared.reasonCode:declared.status==="unavailable"?declared.reasonCode:ambiguity?"ambiguous_event_configuration_position":"matching_position_unavailable",grossOpeningCreditUsd:null,openingFeesUsd:null,netOpeningCreditUsd:null,maximumStructuralLossUsd:null,trackMaximumNetLossUsd:null,openingInitialMarginUsd:null,peakInitialMarginUsd:null,pnlUsd:noTrade?0:null,holdingDays:null,totalRealizedFeesUsd:null,capitalDaysUsd:null,metricAvailability:{...absentReasons(reason),pnlUsd:noTrade?null:reason}})}
