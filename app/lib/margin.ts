...
export function reconstructStandardVerticalMargin(input:{optionType:OptionType;amount:number;shortStrike:number;longStrike:number;expiryTimestamp:number;theoreticalMaximumSpreadLossBtc:number;points:readonly MarginPointInput[];entryTimestamp:number;terminalTimestamp:number;deployment?:DeploymentConfig}){
 ...
 const verticalEstablished=Number.isFinite(input.longStrike)&&input.longStrike>0&&(input.optionType==="P"?input.longStrike<input.shortStrike:input.longStrike>input.shortStrike);
 if(!verticalEstablished)return{status:"unavailable" as const,reason:"Canonical protective-long vertical geometry is not established.",engineVersion:STANDARD_MARGIN_ENGINE_VERSION,ruleVersion:STANDARD_MARGIN_RULE_ID,deployment,path:[]};
 ...
 return{status:"available" as const,engineVersion:STANDARD_MARGIN_ENGINE_VERSION,ruleVersion:STANDARD_MARGIN_RULE_ID,method:"historical Standard Margin reconstruction" as const,provenance:STANDARD_MARGIN_SOURCE,deployment,verticalTreatment:"protective long has zero SM requirement and supplies no short-leg offset; its canonical structure is established independently of its optional mark evidence" as const,protectiveLongMarkCoverage:{available:path.filter(p=>p.longMarkPriceBtc!==undefined).length,total:path.length},integrationConvention:"piecewise-constant between canonical valuation points; final point held to terminal timestamp" as const,openingInitialMarginBtc:path[0]!.initialMarginBtc,openingMaintenanceMarginBtc:path[0]!.maintenanceMarginBtc,peakInitialMarginBtc:peakInitial.initialMarginBtc,peakMaintenanceMarginBtc:peakMaintenance.maintenanceMarginBtc,peakInitialTimestamp:peakInitial.timestamp,peakMaintenanceTimestamp:peakMaintenance.timestamp,capitalDaysMarginBtc,path};
}
