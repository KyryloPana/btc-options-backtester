...
export function validateResearchBundle(files:Partial<Record<string,string>>):{ok:boolean;errors:string[]}{
 ...
 if(run.reference_valuation_methodology_version!==CURRENT_RESEARCH_ENGINE_VERSIONS.referenceValuation)errors.push("run.json has contradictory current Reference methodology metadata.");
 if(run.modeled_execution_methodology_version!==CURRENT_RESEARCH_ENGINE_VERSIONS.modeledExecution)errors.push("run.json has contradictory modeled execution methodology metadata.");
 if(run.execution_estimator_version!==EMPIRICAL_TAKER_EXECUTION_VERSION)errors.push("run.json has contradictory execution estimator metadata.");
 if(run.execution_calibration_method_version!==EXECUTION_CALIBRATION_METHOD_VERSION)errors.push("run.json has contradictory calibration methodology metadata.");
 if(!Array.isArray(run.forward_method_versions)||!run.forward_method_versions.includes(EXPIRY_FORWARD_METHOD_VERSION))errors.push("run.json has contradictory forward methodology metadata.");
 ...
 for(const id of candidateIds){
  ...
  if(!audit.admissible&&!(run.legacy_migrated_from==="3.2.0"&&scenarios.every(r=>r.legacy_economic_track_status==="unavailable")))errors.push(`Candidate ${String(id)} has no valid analytical track.${audit.referenceErrors.length?` Reference valuation: ${audit.referenceErrors.join("; ")}.`:""}`);
 }
