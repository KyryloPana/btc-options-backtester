 ...
 */
 ...
 const entryDelaySensitivity:Row[]=selected.flatMap(({e,s})=>{
  ...
  return ([0,4,8,12] as const).flatMap(requested=>(["maker","taker"] as const).map(executionScenario=>{
   ...
   const actual=num(attempt?.entryTimestamp??null),successful=attempt?.status==="delayed observed opportunity"||attempt?.status==="delayed taker execution proxy",available=pre==="none"&&successful&&target!==null&&actual!==null&&actual>=target,status=referenceTs===null?"not_evaluated":pre!=="none"?"pre_entry_resolution":available?"available":"unavailable",settlement=obj(attempt?.settlementOutcome??null),pnlNative=settlement.status==="settlement-benchmark-priced"?num(settlement.netPnlBtc):null,pnlUsd=settlement.status==="settlement-benchmark-priced"?num(settlement.netPnlUsd):null;
   return row({run_id:runId,source_run_id:sourceIds.get(e.eventId),event_id:e.eventId,candidate_id:s.candidateId,venue:s.venue,execution_scenario:executionScenario,requested_delay_hours:requested,target_entry_timestamp_utc:iso(target),actual_entry_timestamp_utc:available?iso(actual):null,actual_delay_hours:available&&referenceTs!==null&&actual!==null?(actual-referenceTs)/36e5:null,status,reason_code:available?"entry_available":status==="pre_entry_resolution"?pre:String(attempt?.reasonCode??"independent_search_unavailable"),reason:available?null:status==="pre_entry_resolution"?`MR thesis resolved before the +${requested}h target.`:String(attempt?.reason??"Independent causal entry search unavailable."),pre_entry_resolution_status:pre,remaining_actual_dte:available&&expiry!==null&&actual!==null?(expiry-actual)/864e5:null,entry_index_price:available?num(economics.underlyingIndex):null,short_instrument:available?short.instrumentName??null:null,long_instrument:available?long.instrumentName??null:null,short_price_native:available?num(economics.shortPriceBtc):null,long_price_native:available?num(economics.longPriceBtc):null,gross_credit_native:available?num(economics.grossCreditBtc):null,opening_fees_native:available?num(economics.openingFeesBtc):null,net_credit_native:available?num(economics.netCreditBtc):null,credit_change_vs_reference:available&&referenceCredit!==null&&num(economics.netCreditBtc)!==null?num(economics.netCreditBtc)!-referenceCredit:null,synchronization_minutes:available&&num(attempt?.synchronizationGapMs)!==null?num(attempt?.synchronizationGapMs)!/6e4:null,evidence_provenance:available?{source:"observed_exact_contract_tape",causal_search_start_utc:iso(target),short_evidence_ids:short.evidenceIds??[],long_evidence_ids:long.evidenceIds??[],maker_queue_fill_claimed:false}:null,outcome_type:pnlNative!==null?"settlement":null,outcome_pnl_native:pnlNative,outcome_pnl_usd:pnlUsd,exit_timestamp_utc:pnlNative!==null?iso(expiry):null});
  }));
...
export function validateResearchBundle(files:Partial<Record<string,string>>):{ok:boolean;errors:string[]}{
 ...
 for(const r of rows("entry_delay_sensitivity.jsonl")){
  ...
  if(r.status==="available"&&(!Number.isFinite(target)||!Number.isFinite(actual)||actual<target))errors.push(`Entry-delay row ${key} is available without causal post-target evidence.`);
