...
export const RESEARCH_BUNDLE_SCHEMA_VERSION="4.0.0" as const;
/** Bundle schema versions this app can still import (see importResearchBundle). */
export const LEGACY_RESEARCH_BUNDLE_SCHEMA_VERSIONS=["1.0.0","2.0.0","2.1.0","2.2.0","2.3.0","3.0.0","3.1.0","3.2.0","3.3.0","3.4.0","3.5.0","3.6.0","3.7.0","3.8.0","3.9.0"] as const;
...
export function buildResearchBundle(value:unknown,generatedAtUtc=new Date().toISOString(),datasetUpdatedAt?:string,context:ResearchBundleContext={}):ResearchBundle{
 ...
 const selectedIds=new Set(selected.map(({s})=>s.candidateId));
 const research=store.events.flatMap(e=>(e.researchStructures??[]).filter(s=>!selectedIds.has(s.candidateId)).map(s=>({e,s}))).sort((a,b)=>a.s.candidateId.localeCompare(b.s.candidateId));
 // Research-only structures are economically materialized by the same Reference engine,
 // but remain outside every selected-strategy/Q50/Q90/portfolio population.
 const analytical=[...selected,...research];
 ...
 */
 ...
 const candidates:Row[]=analytical.flatMap(({e,s})=>(["maker","taker"] as const).map(mode=>{
  ...
  const c=obj(s.candidateSnapshot),entry=evaluated?obj(scenario.entrySnapshot):{},referenceEntry=obj(obj((s.referenceValuation??null) as unknown as JsonValue).entrySnapshot),expiry=num(c.expiryTimestamp),short=obj(entry.sold),long=obj(entry.bought),index=num(entry.entryTargetIndex);
   const candidate=row({run_id:runId,source_run_id:sourceIds.get(e.eventId),event_id:e.eventId,candidate_id:s.candidateId,strategy_variant_id:s.strategyVariantId??s.candidateId,structure_execution_id:`${s.candidateId}~${mode}`,selection_id:s.selectionId,venue:s.venue,is_selected:selectedIds.has(s.candidateId),research_role:(s as {researchRole?:string}).researchRole??"selected_strategy",selected_at_utc:selectedIds.has(s.candidateId)?s.selectedAtUtc:null,contract_resolution:s.contractResolution??null,reference_valuation:s.referenceValuation??null,delayed_execution:s.delayedExecution??null,modeled_execution:s.modeledExecution??null,structure_type:c.structure??c.spreadKind??null,direction:obj(obj(e.sourceRun).event).direction??null,target_horizon_days:c.targetDte??null,eligible_dte_range:e.generationSnapshot.candidates.find(x=>x.candidateId===s.candidateId)?.eligibleDteRange??null,expiry_timestamp_utc:isoMs(expiry,`candidate ${s.candidateId} expiry`),actual_dte:c.actualDte??null,expiry_weekday_utc:expiry===null?null:new Intl.DateTimeFormat("en-US",{weekday:"long",timeZone:"UTC"}).format(expiry),is_friday_expiry:expiry===null?null:new Date(expiry).getUTCDay()===5,expiry_rank:c.expiryRank??null,expiry_selection_reason:c.expirySelectionReason??null,strike_method:c.strikeMethod??e.generationSnapshot.candidates.find(x=>x.candidateId===s.candidateId)?.strikeMethod??null,requested_strikes:e.generationSnapshot.candidates.find(x=>x.candidateId===s.candidateId)?.requestedStrikes??null,actual_strikes:{short:c.shortStrike??null,long:c.longStrike??null,width:c.actualWidth??null},instruments:c.instruments??{short:short.instrumentName??null,long:long.instrumentName??null},option_type:c.optionType??null,quantity:s.quantity,...economic,
