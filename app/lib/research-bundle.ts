...
export function buildResearchBundle(value:unknown,generatedAtUtc=new Date().toISOString(),datasetUpdatedAt?:string,context:ResearchBundleContext={}):ResearchBundle{
 ...
 const researchIds=new Set(research.map(({e,s})=>`${e.eventId}~${s.candidateId}`));
 const analytical=[...research.map(({e,s})=>{const overlay=selectedById.get(`${e.eventId}~${s.candidateId}`);return{e,s:overlay?{...s,selectionId:overlay.selectionId,selectedAtUtc:overlay.selectedAtUtc,selectionProvenance:overlay.selectionProvenance,executionScenarios:overlay.executionScenarios,delayedExecution:overlay.delayedExecution,marginSnapshot:null,evidenceTradeSnapshots:overlay.evidenceTradeSnapshots,evidenceUsages:overlay.evidenceUsages}:s}}),...selected.filter(({e,s})=>!researchIds.has(`${e.eventId}~${s.candidateId}`))];
...
export function validateResearchBundle(files:Partial<Record<string,string>>):{ok:boolean;errors:string[]}{
 ...
 for(const r of rows("candidates.jsonl"))if(availabilitySelection.get(String(r.candidate_id))!==(r.is_selected===true))errors.push(`Candidate ${String(r.candidate_id)} selection state conflicts with generation availability.`);
