...
export function buildResearchBundle(value:unknown,generatedAtUtc=new Date().toISOString(),datasetUpdatedAt?:string,context:ResearchBundleContext={}):ResearchBundle{
 ...
 const selectedAnalytical=analytical.filter(({e,s})=>selectedIds.has(`${e.eventId}~${s.candidateId}`));
 ...
 */
 ...
 const margins=selectedAnalytical.map(({e,s})=>{
...
export function validateResearchBundle(files:Partial<Record<string,string>>):{ok:boolean;errors:string[]}{
 ...
 const selectedCandidateIds=new Set(rows("candidates.jsonl").filter(r=>r.is_selected===true).map(r=>String(r.candidate_id)));
 ...
 for(const id of marginCandidates)if(!selectedCandidateIds.has(id))errors.push(`Research-only candidate ${id} must not enter selected-strategy margin scenarios.`);
