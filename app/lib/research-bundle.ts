...
export function buildResearchBundle(value:unknown,generatedAtUtc=new Date().toISOString(),datasetUpdatedAt?:string,context:ResearchBundleContext={}):ResearchBundle{
 ...
 const selectedIds=new Set(selected.map(({e,s})=>`${e.eventId}~${s.candidateId}`));
 const selectedById=new Map(selected.map(x=>[`${x.e.eventId}~${x.s.candidateId}`,x.s]));
 // Research owns the structural/Reference layer.  Selection contributes only
 // execution overlays, preventing a stale selected Reference snapshot from
 // replacing a freshly materialized controlled-counterfactual snapshot.
 ...
 const researchIds=new Set(research.map(({e,s})=>`${e.eventId}~${s.candidateId}`));const analytical=[...research.map(({e,s})=>{const overlay=selectedById.get(`${e.eventId}~${s.candidateId}`);return{e,s:overlay?{...s,selectionProvenance:overlay.selectionProvenance,executionScenarios:overlay.executionScenarios,delayedExecution:overlay.delayedExecution,marginSnapshot:null,evidenceTradeSnapshots:overlay.evidenceTradeSnapshots,evidenceUsages:overlay.evidenceUsages}:s}}),...selected.filter(({e,s})=>!researchIds.has(`${e.eventId}~${s.candidateId}`))];
 const materializationIdOf=(s:(typeof analytical)[number]["s"])=>{const controlled="researchRole" in s?s:research.find(({e,s:row})=>e.eventId===s.eventId&&row.candidateId===s.candidateId&&row.researchRole!=="comparative_economics")?.s;return controlled?`research~${controlled.researchRole}~${controlled.eventId}~${("structuralConfigurationId" in controlled?String(controlled.structuralConfigurationId):controlled.candidateId)}`:`selected~${s.eventId}~${s.candidateId}`};
 ...
 // economics producer.
 const marginAnalytical=analytical.filter(({e,s})=>selectedIds.has(`${e.eventId}~${s.candidateId}`)||("researchRole" in s&&s.researchRole==="comparative_economics"));
 ...
 */
 ...
 const candidates:Row[]=analytical.flatMap(({e,s})=>(["maker","taker"] as const).map(mode=>{
  ...
  const c=obj(s.candidateSnapshot),entry=evaluated?obj(scenario.entrySnapshot):{},referenceEntry=obj(obj((s.referenceValuation??null) as unknown as JsonValue).entrySnapshot),expiry=num(c.expiryTimestamp),short=obj(entry.sold),long=obj(entry.bought),index=num(entry.entryTargetIndex);
   const isSelected=!("researchRole" in s&&s.researchRole==="comparative_economics")&&selectedIds.has(`${e.eventId}~${s.candidateId}`);
 ...
 */
 ...
 const configurationOpportunities:Row[]=[...availability.reduce<Map<string,Row[]>>((map,r)=>{const key=`${String(r.event_id)}|${String(r.structural_configuration_id)}`,bucket=map.get(key)??[];bucket.push(r);map.set(key,bucket);return map},new Map())].map(([key,attempts])=>{const event=store.events.find(item=>item.eventId===String(attempts[0]?.event_id)),legacyUnavailable=event?.comparativeEconomicsStatus?.status==="unavailable",rankOne=attempts.filter(attempt=>attempt.expiry_rank===1),canonicalAttempt=attempts.length===1?attempts[0]:rankOne.length===1?rankOne[0]:null,materialized=canonicalAttempt&&canonicalAttempt.research_role==="comparative_economics"&&candidates.some(candidate=>candidate.event_id===canonicalAttempt.event_id&&candidate.candidate_id===canonicalAttempt.candidate_id&&candidate.structural_configuration_id===canonicalAttempt.structural_configuration_id)?canonicalAttempt:null,representative=canonicalAttempt??[...attempts].sort((a,b)=>String(a.candidate_id).localeCompare(String(b.candidate_id)))[0]!,economicState=legacyUnavailable?{status:"unavailable",reason_code:"legacy_comparative_materialization_unavailable",reason:event!.comparativeEconomicsStatus!.reason}:canonicalAttempt===null?{status:"unavailable",reason_code:"canonical_materialization_requires_regeneration",reason:"Multiple generation attempts map to this event × configuration, but the persisted snapshot has no unique ex-ante rank-1 candidate."}:materialized===null?{status:"unavailable",reason_code:"modeled_execution_not_attempted",reason:"The canonical ex-ante rank-1 candidate was not materialized for Comparative Economics."}:materialized.economic_opportunity;return row({run_id:runId,source_run_id:representative.source_run_id,venue:representative.venue,opportunity_id:key,event_id:representative.event_id,structural_configuration_id:representative.structural_configuration_id,structural_configuration_version:representative.structural_configuration_version,structural_configuration:representative.structural_configuration,status:obj(economicState as JsonValue).status,candidate_id:materialized?.candidate_id??null,generation_attempt_identities:attempts.map(x=>String(x.generation_attempt_identity)).sort(),attempt_candidate_ids:[...new Set(attempts.map(x=>x.candidate_id))].sort(),economic_opportunity:economicState})}).sort((a,b)=>String(a.opportunity_id).localeCompare(String(b.opportunity_id)));
