...
import { CURRENT_RESEARCH_ENGINE_VERSIONS } from "./research-refresh.ts";
import { EMPIRICAL_TAKER_EXECUTION_VERSION } from "./empirical-taker-execution.ts";
...
import {FIXED_ENTRY_DELAY_HOURS,FIXED_ENTRY_DELAY_SEARCH_WINDOW_MS,fixedEntryDelaySearchEnd,isInsideFixedEntryDelayWindow} from "./fixed-entry-delay.ts";
import {structuralConfigurationIdentity,STRUCTURAL_CONFIGURATION_ID_VERSION} from "./economics/strategy-configuration.ts";

export const RESEARCH_BUNDLE_SCHEMA_VERSION="4.5.0" as const;
/** Bundle schema versions this app can still import (see importResearchBundle). */
export const LEGACY_RESEARCH_BUNDLE_SCHEMA_VERSIONS=["1.0.0","2.0.0","2.1.0","2.2.0","2.3.0","3.0.0","3.1.0","3.2.0","3.3.0","3.4.0","3.5.0","3.6.0","3.7.0","3.8.0","3.9.0","4.0.0","4.1.0","4.2.0","4.3.0","4.4.0"] as const;
export const RESEARCH_BUNDLE_FILES=["run.json","events.jsonl","underlying_path.jsonl","structure_economics.jsonl","candidates.jsonl","valuations.jsonl","outcomes.jsonl","availability.jsonl","configuration_opportunities.jsonl","margin_scenarios.jsonl","evidence_trades.jsonl","futures_comparisons.jsonl","futures_path.jsonl","entry_delay_sensitivity.jsonl","event_volatility_state.jsonl","structure_volatility_state.jsonl"] as const;
...
export const EFFECTIVE_REFERENCE_PRICING_ASSUMPTIONS={model:"inverse Black-Scholes",referenceValuationMethodVersion:CURRENT_RESEARCH_ENGINE_VERSIONS.referenceValuation,forwardBasis:"expiry-specific causal forward",forwardMethodVersion:EXPIRY_FORWARD_METHOD_VERSION,fixedGlobalRate:null,rateBasis:"derived from forward / spot / time when represented",fallback:"unavailable when causal expiry-forward evidence is absent"} as const;
...
const economicOpportunity=(modeled:JsonValue|undefined)=>{const root=obj(modeled??null),expected=obj(root.expected),code=typeof expected.reasonCode==="string"?expected.reasonCode:null,reason=typeof expected.reason==="string"?expected.reason:null;if(expected.status==="evaluated")return{status:"trade",reason_code:null,reason:null};if(code==="empirical_nonpositive_credit_after_fees")return{status:"explicit_no_trade",reason_code:code,reason};return{status:"unavailable",reason_code:code??(expected.source==="modeled_execution_pending"?"modeled_execution_pending":"modeled_execution_unavailable"),reason:reason??"Q50 modeled execution was not evaluated for this generation opportunity."}};
...
export function buildResearchBundle(value:unknown,generatedAtUtc=new Date().toISOString(),datasetUpdatedAt?:string,context:ResearchBundleContext={}):ResearchBundle{
 ...
 // A candidate is an actual economic structure and may participate in several
 // analyses at once.  Each persisted row below is therefore a materialization,
 // not a candidate-level projection: selected Strategy and every controlled
 // research role survive independently even when they share candidateId.
 const research=store.events.flatMap(e=>(e.researchStructures??[]).map(s=>({e,s}))).sort((a,b)=>a.s.candidateId.localeCompare(b.s.candidateId));
 const analytical=[...research,...selected];
 const materializationIdOf=(s:(typeof analytical)[number]["s"])=>"researchRole" in s
  ?s.researchRole==="comparative_economics"
   ?`research~comparative-economics~${s.eventId}~${String("structuralConfigurationId" in s?s.structuralConfigurationId:"")}`
   :`research~${s.researchRole}~${s.eventId}~${s.candidateId}`
  :`selected~${s.eventId}~${s.candidateId}`;
 // Verified segregated Standard Margin is analytical evidence, not Strategy
 // selection state.  Canonical Comparative Economics materializations need
 // their own configuration-specific reconstruction too; narrow research
 // controls remain excluded because they are not part of the full-space
 // economics producer.
 const marginAnalytical=analytical.filter(({s})=>!("researchRole" in s)||s.researchRole==="comparative_economics");
 ...
 */
 const structureEconomics:Row[]=analytical.map(({e,s})=>{
  ...
  return row({
   run_id:runId,source_run_id:sourceIds.get(e.eventId),venue:s.venue,event_id:e.eventId,candidate_id:s.candidateId,materialization_id:materializationIdOf(s),
 ...
 const candidates:Row[]=analytical.flatMap(({e,s})=>(["maker","taker"] as const).map(mode=>{
  ...
  const c=obj(s.candidateSnapshot),entry=evaluated?obj(scenario.entrySnapshot):{},referenceEntry=obj(obj((s.referenceValuation??null) as unknown as JsonValue).entrySnapshot),expiry=num(c.expiryTimestamp),short=obj(entry.sold),long=obj(entry.bought),index=num(entry.entryTargetIndex);
   const isSelected=!("researchRole" in s);
   const claimedConfiguration="structuralConfigurationId" in s?s.structuralConfigurationId:undefined,generated=e.generationSnapshot.candidates.find(x=>x.candidateId===s.candidateId&&(!claimedConfiguration||structuralConfigurationIdentity({target_horizon_days:x.targetHorizon,strike_method:x.strikeMethod,requested_strikes:x.requestedStrikes,structure_type:x.structure}).id===claimedConfiguration));const structuralFields={target_horizon_days:c.targetDte??null,strike_method:c.strikeMethod??generated?.strikeMethod??null,requested_strikes:generated?.requestedStrikes??null,structure_type:c.structure??c.spreadKind??null},configuration=structuralConfigurationIdentity(structuralFields),materializationId=materializationIdOf(s);const candidate=row({run_id:runId,source_run_id:sourceIds.get(e.eventId),event_id:e.eventId,candidate_id:s.candidateId,strategy_variant_id:s.strategyVariantId??s.candidateId,structural_configuration_id:configuration.id,structural_configuration_version:STRUCTURAL_CONFIGURATION_ID_VERSION,structural_configuration:configuration.fields,materialization_id:materializationId,structure_execution_id:`${materializationId}~${mode}`,selection_id:isSelected?s.selectionId:null,venue:s.venue,is_selected:isSelected,research_role:(s as {researchRole?:string}).researchRole??null,selected_at_utc:isSelected?s.selectedAtUtc:null,contract_resolution:s.contractResolution??null,reference_valuation:s.referenceValuation??null,delayed_execution:isSelected?s.delayedExecution??null:null,modeled_execution:s.modeledExecution??null,structure_type:structuralFields.structure_type,direction:obj(obj(e.sourceRun).event).direction??null,target_horizon_days:structuralFields.target_horizon_days,eligible_dte_range:e.generationSnapshot.candidates.find(x=>x.candidateId===s.candidateId)?.eligibleDteRange??null,expiry_timestamp_utc:isoMs(expiry,`candidate ${s.candidateId} expiry`),actual_dte:c.actualDte??null,expiry_weekday_utc:expiry===null?null:new Intl.DateTimeFormat("en-US",{weekday:"long",timeZone:"UTC"}).format(expiry),is_friday_expiry:expiry===null?null:new Date(expiry).getUTCDay()===5,expiry_rank:c.expiryRank??null,expiry_selection_reason:c.expirySelectionReason??null,strike_method:structuralFields.strike_method,requested_strikes:structuralFields.requested_strikes,actual_strikes:{short:c.shortStrike??null,long:c.longStrike??null,width:c.actualWidth??null},instruments:c.instruments??{short:short.instrumentName??null,long:long.instrumentName??null},option_type:c.optionType??null,quantity:s.quantity,...economic,
   ...
   reference_structure_entry_timestamp_utc:isoMs(num(referenceEntry.valuationTimestamp)??num(referenceEntry.targetTimestamp),`candidate ${s.candidateId} reference structural entry`),
   ...
   gross_credit_debit_usd:evaluated&&num(entry.grossSpreadBtc)!==null&&index!==null?num(entry.grossSpreadBtc)!*index:null,
   opening_fees_native:evaluated?entry.openingFeesBtc??null:null,
   opening_fees_usd:evaluated&&num(entry.openingFeesBtc)!==null&&index!==null?num(entry.openingFeesBtc)!*index:null,
   net_opening_cash_flow_native:evaluated?entry.netOpeningCashFlowBtc??null:null,
   net_opening_cash_flow_usd:evaluated&&num(entry.netOpeningCashFlowBtc)!==null&&index!==null?num(entry.netOpeningCashFlowBtc)!*index:null,
  ...
  if(!semantic.valid){
   // An unresolved controlled candidate has no actual geometry to reconcile.
   // Preserve its explicit unavailable state and requested geometry instead of
   // either fabricating zero strikes or dropping it from the research funnel.
   if(!initiallyEvaluated&&c.shortStrike===null&&c.longStrike===null)return row({...candidate,reconciliation_diagnostics:semantic.diagnostics});
   throw new Error(`Research export blocked: ${semantic.diagnostics.map(d=>`${d.candidateId} (${d.eventId}): ${d.reason}`).join("; ")}`);
  }return row({...semantic.row,reconciliation_diagnostics:semantic.diagnostics});
 }));
 ...
 */
  ...
  for(const t of describeCanonicalTracks(s as unknown as Record<string,unknown>)){
   ...
   for(const [index,point0] of t.valuationPath.entries()){
    ...
    independentValuations.push(row({run_id:runId,source_run_id:sourceIds.get(e.eventId),event_id:e.eventId,
     candidate_id:s.candidateId,materialization_id:materializationIdOf(s),execution_scenario:t.executionScenario,...trackMeta,
     valuation_id:`${s.venue}~valuation~${hash([materializationIdOf(s),t.track,timestamp])}`,venue:s.venue,...economic,
   ...
   for(const kind of REQUIRED_OUTCOMES){
    ...
    independentOutcomes.push(row({run_id:runId,source_run_id:sourceIds.get(e.eventId),event_id:e.eventId,
     candidate_id:s.candidateId,materialization_id:materializationIdOf(s),execution_scenario:t.executionScenario,...trackMeta,
     outcome_id:`${s.venue}~outcome~${hash([materializationIdOf(s),t.track,kind])}`,venue:s.venue,outcome_type:kind,
     ...
     closing_fees_native:o?o.feesBtc??null:null,closing_fees_usd:o&&num(o.feesBtc)!==null&&index!==null?num(o.feesBtc)!*index:null,
     delivery_fees_native:kind==="settlement"&&o?o.feesBtc??null:null,delivery_fees_usd:kind==="settlement"&&o&&num(o.feesBtc)!==null&&index!==null?num(o.feesBtc)!*index:null,quality:o?.estimateQuality??"unavailable",
 ...
 const availability:Row[]=store.events.flatMap(e=>e.generationSnapshot.candidates.map(c=>{const selectedStructure=e.selectedStructures.find(s=>s.candidateId===c.candidateId),researchStructure=(e.researchStructures??[]).find(s=>s.candidateId===c.candidateId),configurationId=structuralConfigurationIdentity({target_horizon_days:c.targetHorizon,strike_method:c.strikeMethod,requested_strikes:c.requestedStrikes,structure_type:c.structure}).id,comparativeStructure=(e.researchStructures??[]).find(s=>s.candidateId===c.candidateId&&s.researchRole==="comparative_economics"&&s.structuralConfigurationId===configurationId),structural=comparativeStructure??researchStructure??selectedStructure;return row({run_id:runId,source_run_id:sourceIds.get(e.eventId),event_id:e.eventId,availability_id:`${c.candidateId}~generation~${hash([c.requestedStrikes,c.targetHorizon,c.strikeMethod])}`,generation_attempt_identity:JSON.stringify([c.candidateId,c.requestedStrikes,c.targetHorizon,c.strikeMethod]),candidate_id:c.candidateId,strategy_variant_id:structural?.strategyVariantId??c.candidateId,structural_configuration_id:configurationId,structural_configuration_version:STRUCTURAL_CONFIGURATION_ID_VERSION,structural_configuration:structuralConfigurationIdentity({target_horizon_days:c.targetHorizon,strike_method:c.strikeMethod,requested_strikes:c.requestedStrikes,structure_type:c.structure}).fields,economic_opportunity:economicOpportunity(comparativeStructure?.modeledExecution),venue:c.venue,is_selected:Boolean(selectedStructure),research_role:researchStructure?.researchRole??null,contract_resolution:structural?.contractResolution??null,reference_valuation:structural?.referenceValuation??null,delayed_execution:selectedStructure?.delayedExecution??null,modeled_execution:comparativeStructure?.modeledExecution??null,structure_type:c.structure,target_horizon_days:c.targetHorizon,strike_method:c.strikeMethod,width:c.actualStrikes.width??c.requestedStrikes.width,requested_strikes:c.requestedStrikes,actual_strikes:c.actualStrikes,actual_expiry_timestamp_utc:iso(c.actualExpiryTimestamp),requested_expiry_timestamp_utc:null,instruments:null,status:c.status,reason_codes:[c.status==="priced"?"candidate_priced":"candidate_unavailable"],availability_reasons:c.availabilityReasons,entry_quality:c.entryQuality,expiry_rank:c.expiryRank??null,expiry_selection_reason:c.expirySelectionReason??null,retrieval_status:structural?.contractResolution?.status??"metadata_unavailable",generation_configuration:e.generationSnapshot.configuration})})).sort((a,b)=>String(a.candidate_id).localeCompare(String(b.candidate_id))||String(a.availability_id).localeCompare(String(b.availability_id)));
 const comparativeByConfiguration=new Map<string,(typeof research)[number]["s"]>();
 for(const {e,s} of research){
  if(s.researchRole!=="comparative_economics")continue;
  const key=`${e.eventId}|${s.structuralConfigurationId}`;
  if(comparativeByConfiguration.has(key))throw new Error(`Multiple Comparative Economics materializations claim ${key}.`);
  comparativeByConfiguration.set(key,s);
 }
 const configurationOpportunities:Row[]=[...availability.reduce<Map<string,Row[]>>((map,r)=>{const key=`${String(r.event_id)}|${String(r.structural_configuration_id)}`,bucket=map.get(key)??[];bucket.push(r);map.set(key,bucket);return map},new Map())].map(([key,attempts])=>{
  const event=store.events.find(item=>item.eventId===String(attempts[0]?.event_id)),legacyUnavailable=event?.comparativeEconomicsStatus?.status==="unavailable",rankOne=attempts.filter(attempt=>attempt.expiry_rank===1),canonicalAttempt=attempts.length===1?attempts[0]:rankOne.length===1?rankOne[0]:null,comparative=comparativeByConfiguration.get(key)??null,representative=canonicalAttempt??[...attempts].sort((a,b)=>String(a.candidate_id).localeCompare(String(b.candidate_id)))[0]!;
  if(comparative&&canonicalAttempt&&comparative.candidateId!==canonicalAttempt.candidate_id)throw new Error(`Comparative Economics materialization ${comparative.candidateId} is not the canonical rank-1 candidate ${String(canonicalAttempt.candidate_id)} for ${key}.`);
  const economicState=canonicalAttempt===null?{status:"unavailable",reason_code:"canonical_materialization_requires_regeneration",reason:"Multiple generation attempts map to this event × configuration, but the persisted snapshot has no unique ex-ante rank-1 candidate."}:comparative===null?(legacyUnavailable?{status:"unavailable",reason_code:"legacy_comparative_materialization_unavailable",reason:event!.comparativeEconomicsStatus!.reason}:{status:"unavailable",reason_code:"modeled_execution_not_attempted",reason:"The canonical ex-ante rank-1 candidate was not materialized for Comparative Economics."}):economicOpportunity(comparative.modeledExecution);
  return row({run_id:runId,source_run_id:representative.source_run_id,venue:representative.venue,opportunity_id:key,event_id:representative.event_id,structural_configuration_id:representative.structural_configuration_id,structural_configuration_version:representative.structural_configuration_version,structural_configuration:representative.structural_configuration,status:economicState.status,candidate_id:comparative?.candidateId??null,generation_attempt_identities:attempts.map(x=>String(x.generation_attempt_identity)).sort(),attempt_candidate_ids:[...new Set(attempts.map(x=>x.candidate_id))].sort(),economic_opportunity:economicState});
 }).sort((a,b)=>String(a.opportunity_id).localeCompare(String(b.opportunity_id)));
 const margins=marginAnalytical.map(({e,s})=>{
  ...
  const m=stale?obj(buildResearchMarginSnapshot(s)):persisted,available=m.status==="available"||m.state==="ok",deployment=obj(m.deployment),openingIm=num(m.openingInitialMarginBtc)??num(m.initialMarginBtc),openingMm=num(m.openingMaintenanceMarginBtc)??num(m.maintenanceMarginBtc),rawMarginPath=Array.isArray(m.path)?m.path:[],canonicalMarginPath=rawMarginPath.map(x=>{const p=obj(x as JsonValue),index=num(p.indexPrice)??num(p.index_price),im=num(p.initialMarginBtc),mm=num(p.maintenanceMarginBtc);return {...p,timestamp:num(p.timestamp),timestamp_utc:typeof p.timestamp_utc==="string"?p.timestamp_utc:null,initial_margin_btc:im,initial_margin_usd:im!==null&&index!==null?im*index:null,maintenance_margin_btc:mm,maintenance_margin_usd:mm!==null&&index!==null?mm*index:null,index_price:index}}),btcImPeak=canonicalMarginPath.filter(x=>num(x.initial_margin_btc)!==null).sort((a,b)=>(num(b.initial_margin_btc)??0)-(num(a.initial_margin_btc)??0))[0],btcMmPeak=canonicalMarginPath.filter(x=>num(x.maintenance_margin_btc)!==null).sort((a,b)=>(num(b.maintenance_margin_btc)??0)-(num(a.maintenance_margin_btc)??0))[0],usdImPeak=canonicalMarginPath.filter(x=>num(x.initial_margin_usd)!==null).sort((a,b)=>(num(b.initial_margin_usd)??0)-(num(a.initial_margin_usd)??0))[0],usdMmPeak=canonicalMarginPath.filter(x=>num(x.maintenance_margin_usd)!==null).sort((a,b)=>(num(b.maintenance_margin_usd)??0)-(num(a.maintenance_margin_usd)??0))[0],peakIm=num(m.peakInitialMarginBtc)??num(btcImPeak?.initial_margin_btc),peakMm=num(m.peakMaintenanceMarginBtc)??num(btcMmPeak?.maintenance_margin_btc);return row({run_id:runId,source_run_id:sourceIds.get(e.eventId),event_id:e.eventId,candidate_id:s.candidateId,materialization_id:materializationIdOf(s),margin_scenario_id:`${s.venue}~margin~${hash(materializationIdOf(s))}`,venue:s.venue,margin_status:available?"available":"unavailable",margin_model:deployment.model??null,method_version:m.engineVersion??m.ruleVersion??null,rule_version:m.ruleVersion??null,provenance:m.provenance??null,account_configuration:deployment.accountAssumption??null,collateral_currency:deployment.collateralCurrency??"BTC",settlement_currency:"BTC",incremental_initial_margin:available?openingIm:null,incremental_initial_margin_usd:available&&openingIm!==null&&num(canonicalMarginPath[0]?.index_price)!==null?openingIm*(num(canonicalMarginPath[0]?.index_price)??0):null,incremental_maintenance_margin:available?openingMm:null,incremental_maintenance_margin_usd:available&&openingMm!==null&&num(canonicalMarginPath[0]?.index_price)!==null?openingMm*(num(canonicalMarginPath[0]?.index_price)??0):null,peak_initial_margin:available?peakIm:null,peak_initial_margin_timestamp_utc:btcImPeak?iso(num(btcImPeak.timestamp)):null,peak_initial_margin_usd:available?num(usdImPeak?.initial_margin_usd):null,peak_initial_margin_usd_timestamp_utc:usdImPeak?iso(num(usdImPeak.timestamp)):null,peak_maintenance_margin:available?peakMm:null,peak_maintenance_margin_timestamp_utc:btcMmPeak?iso(num(btcMmPeak.timestamp)):null,peak_maintenance_margin_usd:available?num(usdMmPeak?.maintenance_margin_usd):null,peak_maintenance_margin_usd_timestamp_utc:usdMmPeak?iso(num(usdMmPeak.timestamp)):null,peak_timestamp_utc:available?iso(num(m.peakInitialTimestamp)??num(m.observationTimestamp)):null,capital_days_margin:available?num(m.capitalDaysMarginBtc):null,maximum_structural_loss_native:num(m.maximumStructuralLossBtcAtReferenceIndex)??num(m.theoreticalMaximumSpreadLossBtc),
   ...
   margin_inputs:{path:canonicalMarginPath.length?canonicalMarginPath:null,mark_price_btc:m.markPriceBtc??null,vertical_treatment:m.verticalTreatment??null,protective_long_mark_coverage:m.protectiveLongMarkCoverage??null,integration_convention:m.integrationConvention??null} });});
 ...
 const selectedByCandidate=new Map(analytical.map(({e,s})=>[s.candidateId,{e,s}]));
 ...
 const entryDelaySensitivity:Row[]=selected.flatMap(({e,s})=>{
  const ref=obj(obj((s.referenceValuation??null) as unknown as JsonValue).entrySnapshot),referenceTs=num(ref.valuationTimestamp)??num(ref.targetTimestamp),referenceCredit=num(ref.netOpeningCashFlowBtc),expiry=num(obj(s.candidateSnapshot).expiryTimestamp),delayed=obj(s.delayedExecution??null),timing=resolveEventTiming({sourceRun:e.sourceRun,underlyingHourlyPath:e.generationSnapshot.underlyingHourlyPath});
  return FIXED_ENTRY_DELAY_HOURS.flatMap(requested=>(["maker","taker"] as const).map(executionScenario=>{
   const target=referenceTs===null?null:referenceTs+requested*36e5,track=obj(delayed[executionScenario]??null),attempts=Array.isArray(track.attempts)?track.attempts.map(x=>obj(x)):[],attempt=attempts.find(a=>num(obj(a.window??null).startMs)===requested*36e5),economics=obj(attempt?.economics??null),short=obj(attempt?.shortLeg??null),long=obj(attempt?.longLeg??null);
   const endpoints=[timing.vpocDecisionTimestamp!==null?{kind:"pre_entry_vpoc",at:Number(timing.vpocDecisionTimestamp)}:null,timing.invalidationDecisionTimestamp!==null?{kind:"pre_entry_invalidation",at:Number(timing.invalidationDecisionTimestamp)}:null].filter((x):x is {kind:string;at:number}=>x!==null).sort((a,b)=>a.at-b.at),first=endpoints[0],pre:string=target!==null&&first!==undefined&&first.at<target?(endpoints[1]?.at===first.at?"pre_entry_ambiguous":first.kind):"none";
   const actual=num(attempt?.entryTimestamp??null),successful=attempt?.status==="delayed observed opportunity"||attempt?.status==="delayed taker execution proxy",insideFixedWindow=target!==null&&actual!==null&&isInsideFixedEntryDelayWindow(target,actual),available=pre==="none"&&successful&&insideFixedWindow,status=referenceTs===null?"not_evaluated":pre!=="none"?"pre_entry_resolution":available?"available":"unavailable",settlement=obj(attempt?.settlementOutcome??null),pnlNative=settlement.status==="settlement-benchmark-priced"?num(settlement.netPnlBtc):null,pnlUsd=settlement.status==="settlement-benchmark-priced"?num(settlement.netPnlUsd):null,netCredit=num(economics.netCreditBtc),entryIndex=num(economics.underlyingIndex),creditDeltaNative=available&&referenceCredit!==null&&netCredit!==null?netCredit-referenceCredit:null,creditDeltaUsd=creditDeltaNative!==null&&entryIndex!==null&&entryIndex>0?creditDeltaNative*entryIndex:null;
   return row({run_id:runId,source_run_id:sourceIds.get(e.eventId),event_id:e.eventId,candidate_id:s.candidateId,materialization_id:materializationIdOf(s),venue:s.venue,execution_scenario:executionScenario,requested_delay_hours:requested,requested_search_window_minutes:FIXED_ENTRY_DELAY_SEARCH_WINDOW_MS/6e4,target_entry_timestamp_utc:iso(target),actual_entry_timestamp_utc:available?iso(actual):null,actual_delay_hours:available&&referenceTs!==null&&actual!==null?(actual-referenceTs)/36e5:null,status,reason_code:available?"entry_available":status==="pre_entry_resolution"?pre:String(attempt?.reasonCode??"independent_search_unavailable"),reason:available?null:status==="pre_entry_resolution"?`MR thesis resolved before the +${requested}h target.`:String(attempt?.reason??"Independent causal entry search unavailable."),pre_entry_resolution_status:pre,remaining_actual_dte:available&&expiry!==null&&actual!==null?(expiry-actual)/864e5:null,entry_index_price:available?entryIndex:null,short_instrument:available?short.instrumentName??null:null,long_instrument:available?long.instrumentName??null:null,short_price_native:available?num(economics.shortPriceBtc):null,long_price_native:available?num(economics.longPriceBtc):null,gross_credit_native:available?num(economics.grossCreditBtc):null,opening_fees_native:available?num(economics.openingFeesBtc):null,net_credit_native:available?netCredit:null,credit_change_vs_reference_native:creditDeltaNative,credit_change_vs_reference_usd:creditDeltaUsd,credit_change_vs_reference:creditDeltaNative,synchronization_minutes:available&&num(attempt?.synchronizationGapMs)!==null?num(attempt?.synchronizationGapMs)!/6e4:null,evidence_provenance:available?{source:"observed_exact_contract_tape",causal_search_start_utc:iso(target),causal_search_end_utc:target===null?null:iso(fixedEntryDelaySearchEnd(target)),search_window_minutes:FIXED_ENTRY_DELAY_SEARCH_WINDOW_MS/6e4,short_evidence_ids:short.evidenceIds??[],long_evidence_ids:long.evidenceIds??[],maker_queue_fill_claimed:false}:null,outcome_type:pnlNative!==null?"settlement":null,outcome_pnl_native:pnlNative,outcome_pnl_usd:pnlUsd,exit_timestamp_utc:pnlNative!==null?iso(expiry):null});
  }));
 }).sort((a,b)=>String(a.candidate_id).localeCompare(String(b.candidate_id))||Number(a.requested_delay_hours)-Number(b.requested_delay_hours)||String(a.execution_scenario).localeCompare(String(b.execution_scenario)));
 ...
 const migratedValuationWindowDiagnostics=selected.flatMap(({e,s})=>(["maker","taker"] as const).flatMap(executionScenario=>{const diagnostic=s.executionScenarios[executionScenario].valuationWindowMigration;return diagnostic?[{event_id:e.eventId,candidate_id:s.candidateId,materialization_id:materializationIdOf(s),execution_scenario:executionScenario,...diagnostic}]:[]}));
 const modeledProvenance=selected.flatMap(({s})=>{const modeled=obj((s.modeledExecution??null) as JsonValue);return [obj(obj(modeled.expected).provenance),obj(obj(modeled.conservative).provenance)]});
 const artifactHashes=[...new Set(modeledProvenance.map(p=>p.artifactHash).filter((x):x is string=>typeof x==="string"))];
 const datasetFingerprints=[...new Set(modeledProvenance.map(p=>p.datasetFingerprint).filter((x):x is string=>typeof x==="string"))];
 const includedAnalyticsTracks=[...new Set([...valuations,...independentValuations,...outcomes,...independentOutcomes].map(r=>r.analytics_track).filter((x):x is string=>typeof x==="string"))].sort();
 const run=row({schema_version:RESEARCH_BUNDLE_SCHEMA_VERSION,futures_engine_version:FUTURES_ENGINE_VERSION,valuation_window_diagnostics:{migrated:migratedValuationWindowDiagnostics,builder_omitted_before_entry:omittedValuationsBeforeEntry,builder_omitted_after_expiry:omittedValuationsAfterExpiry},run_id:runId,generated_at_utc:generatedAtUtc,dataset_id:store.datasetId,dataset_version_updated_at:datasetUpdatedAt??store.updatedAtUtc,application_commit_build_id:[...new Set(sourceRuns.map(x=>x.application_build))],pricing_engine_versions:[...new Set(sourceRuns.map(x=>x.configuration.pricingEngineVersion))],quality_rules_versions:[...new Set(sourceRuns.map(x=>x.configuration.qualityRulesVersion))],valuation_methodology_version:CURRENT_RESEARCH_ENGINE_VERSIONS.referenceValuation,reference_valuation_methodology_version:CURRENT_RESEARCH_ENGINE_VERSIONS.referenceValuation,modeled_execution_methodology_version:CURRENT_RESEARCH_ENGINE_VERSIONS.modeledExecution,execution_estimator_version:EMPIRICAL_TAKER_EXECUTION_VERSION,execution_calibration_method_version:EXECUTION_CALIBRATION_METHOD_VERSION,forward_method_versions:[EXPIRY_FORWARD_METHOD_VERSION],execution_estimator_artifact_hashes:artifactHashes,execution_calibration_dataset_fingerprints:datasetFingerprints,included_analytics_tracks:includedAnalyticsTracks,generation_assumptions:EFFECTIVE_REFERENCE_PRICING_ASSUMPTIONS,source_generation_assumptions_raw:sourceRuns.map(x=>x.configuration.modelAssumptions),valuation_intervals:[...new Set(sourceRuns.map(x=>x.configuration.valuationInterval))],timezone:"UTC",included_pricing_tracks:["raw_vwap","iv_normalized"],included_execution_scenarios:["maker","taker"],dte_windows:sourceRuns.map(x=>x.configuration.dteWindows),expiry_selection_modes:[...new Set(sourceRuns.map(x=>x.configuration.expirySelectionMode))],evidence_windows:sourceRuns.map(x=>x.configuration.historicalEvidenceWindows),synchronization_thresholds:sourceRuns.map(x=>x.configuration.synchronizationThresholds),quality_thresholds:sourceRuns.map(x=>x.configuration.qualityThresholds),reference_pricing_convention:{forward:"expiry-specific causal forward",fixed_global_rate:null,implied_rate_basis:"derived from F/S/T when represented"},fee_assumptions:sourceRuns.map(x=>x.configuration.feeAssumptions),settlement_rules:sourceRuns.map(x=>x.configuration.settlementRules),trade_dataset_mr_event_count:num(context.tradeDatasetMrEventCount??null)??null,trade_dataset_mr_event_count_source:num(context.tradeDatasetMrEventCount??null)!==null?"active_trade_dataset":"unavailable",persisted_research_event_count:events.length,events_with_generated_candidates_count:new Set(availability.map(x=>x.event_id)).size,events_with_selected_candidates_count:new Set(selected.map(x=>x.e.eventId)).size,events_with_stored_underlying_paths_count:new Set(paths.map(x=>x.event_id)).size,selected_structure_count:selected.length,selected_structure_execution_row_count:selected.length*2,generated_denominator_count:availability.length,venues:[...new Set(store.events.flatMap(e=>e.generationSnapshot.candidates.map(c=>c.venue))) ],venue_configuration:{deribit:economic,bybit:null,binance:null},effective_configuration_hash:distinctMethodologies[0]??null,methodology_identity:methodologyIdentity(store.events[0]?.generationSnapshot.configuration??{}) as unknown as JsonValue,build_provenance_status:buildProvenanceStatus(sourceRuns.map(x=>x.application_build)),source_runs:sourceRuns,table_availability:{underlying_path:paths.length?"available":"unavailable",structure_economics:structureEconomics.some(r=>r.maximum_structural_loss_status==="available")?"available":"unavailable",candidates:candidates.length?"available":"unavailable",availability:availability.length?"available":"unavailable",configuration_opportunities:configurationOpportunities.length?"available":"unavailable",valuations:[...valuations,...independentValuations].some(v=>v.valuation_status==="priced")?"available":"unavailable",outcomes:[...outcomes,...independentOutcomes].some(o=>o.status==="evaluated")?"available":"unavailable",margin_scenarios:margins.some(m=>m.margin_status==="available")?"available":"unavailable",evidence_trades:evidence.length?"available":"unavailable",futures_comparisons:futures.some(f=>f.availability==="available")?"available":"unavailable",futures_path:futuresPath.length?"available":"unavailable",entry_delay_sensitivity:entryDelaySensitivity.some(r=>r.status==="available")?"available":"unavailable",event_volatility_state:eventVolatility.length?"available":"unavailable",structure_volatility_state:structureVolatility.length?"available":"unavailable"},volatility_method_versions:{event_volatility_state:EVENT_VOLATILITY_STATE_METHOD_VERSION,structure_volatility_state:STRUCTURE_VOLATILITY_STATE_METHOD_VERSION}});
 const files=Object.fromEntries(RESEARCH_BUNDLE_FILES.map(name=>[name,name==="run.json"?JSON.stringify(run)+"\n":lines(({"events.jsonl":events,"underlying_path.jsonl":paths,"structure_economics.jsonl":structureEconomics,"candidates.jsonl":candidates,"valuations.jsonl":[...valuations,...independentValuations],"outcomes.jsonl":[...outcomes,...independentOutcomes],"availability.jsonl":availability,"configuration_opportunities.jsonl":configurationOpportunities,"margin_scenarios.jsonl":margins,"evidence_trades.jsonl":evidence,"futures_comparisons.jsonl":futures,"futures_path.jsonl":futuresPath,"entry_delay_sensitivity.jsonl":entryDelaySensitivity,"event_volatility_state.jsonl":eventVolatility,"structure_volatility_state.jsonl":structureVolatility})[name]??[])])) as ResearchBundle["files"];
...
export function validateResearchBundle(files:Partial<Record<string,string>>):{ok:boolean;errors:string[]}{
 ...
 const migratedFrom=typeof run.legacy_migrated_from==="string"&&(LEGACY_RESEARCH_BUNDLE_SCHEMA_VERSIONS as readonly string[]).includes(run.legacy_migrated_from)?run.legacy_migrated_from:null;
 if(!migratedFrom&&run.reference_valuation_methodology_version!==CURRENT_RESEARCH_ENGINE_VERSIONS.referenceValuation)errors.push("run.json has contradictory current Reference methodology metadata.");
 if(!migratedFrom&&JSON.stringify(run.generation_assumptions)!==JSON.stringify(EFFECTIVE_REFERENCE_PRICING_ASSUMPTIONS))errors.push("run.json effective Reference pricing assumptions contradict the current methodology.");

 if(!migratedFrom&&run.modeled_execution_methodology_version!==CURRENT_RESEARCH_ENGINE_VERSIONS.modeledExecution)errors.push("run.json has contradictory modeled execution methodology metadata.");
 if(!migratedFrom&&run.execution_estimator_version!==EMPIRICAL_TAKER_EXECUTION_VERSION)errors.push("run.json has contradictory execution estimator metadata.");
 if(!migratedFrom&&run.execution_calibration_method_version!==EXECUTION_CALIBRATION_METHOD_VERSION)errors.push("run.json has contradictory calibration methodology metadata.");
 if(!migratedFrom&&(!Array.isArray(run.forward_method_versions)||!run.forward_method_versions.includes(EXPIRY_FORWARD_METHOD_VERSION)))errors.push("run.json has contradictory forward methodology metadata.");
 if(migratedFrom&&!run.legacy_source_methodology)errors.push("Migrated bundle does not preserve its legacy source methodology.");
 const currentCompatibleMigration=migratedFrom==="3.8.0"||migratedFrom==="3.9.0"||migratedFrom==="4.0.0"||migratedFrom==="4.3.0";
 if(currentCompatibleMigration&&run.reference_valuation_methodology_version!==CURRENT_RESEARCH_ENGINE_VERSIONS.referenceValuation)errors.push(`Migrated ${migratedFrom} bundle has incompatible Reference methodology metadata.`);
 if(currentCompatibleMigration&&run.modeled_execution_methodology_version!==CURRENT_RESEARCH_ENGINE_VERSIONS.modeledExecution)errors.push(`Migrated ${migratedFrom} bundle has incompatible modeled methodology metadata.`);
 if(currentCompatibleMigration&&run.execution_estimator_version!==EMPIRICAL_TAKER_EXECUTION_VERSION)errors.push(`Migrated ${migratedFrom} bundle has incompatible execution estimator metadata.`);
 if(currentCompatibleMigration&&run.execution_calibration_method_version!==EXECUTION_CALIBRATION_METHOD_VERSION)errors.push(`Migrated ${migratedFrom} bundle has incompatible calibration methodology metadata.`);
 if(currentCompatibleMigration&&(!Array.isArray(run.forward_method_versions)||!run.forward_method_versions.includes(EXPIRY_FORWARD_METHOD_VERSION)))errors.push(`Migrated ${migratedFrom} bundle has incompatible forward methodology metadata.`);
 ...
 const economicsIdentity=migratedFrom?"candidate_id":"materialization_id",economicsIds=ids("structure_economics.jsonl",economicsIdentity);
 const candidateMaterializationIds=new Set(rows("candidates.jsonl").map(r=>migratedFrom?r.candidate_id:r.materialization_id));for(const id of candidateMaterializationIds)if(!economicsIds.has(id))errors.push(`Materialization ${String(id)} has no scenario-independent structure-economics record.`);
 for(const r of rows("structure_economics.jsonl")){
  ...
  for(const raw of tracks){const t=obj(raw as JsonValue);if(!migratedFrom&&(t.track==="modeled_expected"||t.track==="modeled_conservative")&&t.engine_version!==null&&t.engine_version!==run.modeled_execution_methodology_version)errors.push(`Structure-economics record ${String(r.candidate_id)} track ${String(t.track)} was produced by ${String(t.engine_version)}, not run methodology ${String(run.modeled_execution_methodology_version)}.`);if(migratedFrom&&["reference_fair_value","modeled_expected","modeled_conservative"].includes(String(t.track))&&!["3.8.0","3.9.0","4.0.0","4.3.0"].includes(migratedFrom)&&t.status!=="unavailable")errors.push(`Migrated ${migratedFrom} track ${String(t.track)} must be unavailable until recomputed.`)}
 }
 ...
 for(const r of rows("availability.jsonl")){const identity=structuralConfigurationIdentity(r),sourceIdentity=structuralConfigurationIdentity({...r,structural_configuration:null});if(identity.id===null||identity.id!==r.structural_configuration_id||sourceIdentity.id!==identity.id||r.structural_configuration_version!==STRUCTURAL_CONFIGURATION_ID_VERSION||JSON.stringify(identity.fields)!==JSON.stringify(r.structural_configuration))errors.push(`Availability attempt ${String(r.availability_id)} has an invalid or inconsistent structural configuration identity.`)}const opportunityKeys=new Set<string>();for(const r of rows("configuration_opportunities.jsonl")){const identity=structuralConfigurationIdentity(r),key=`${String(r.event_id)}|${String(r.structural_configuration_id)}`;if(identity.id===null||identity.id!==r.structural_configuration_id||r.structural_configuration_version!==STRUCTURAL_CONFIGURATION_ID_VERSION||JSON.stringify(identity.fields)!==JSON.stringify(r.structural_configuration))errors.push(`Configuration opportunity ${String(r.opportunity_id)} has an invalid or inconsistent identity.`);if(opportunityKeys.has(key))errors.push(`Duplicate event/configuration opportunity ${key}.`);opportunityKeys.add(key);const attempts=rows("availability.jsonl").filter(attempt=>attempt.event_id===r.event_id&&attempt.structural_configuration_id===r.structural_configuration_id),persistedAttempts=Array.isArray(r.generation_attempt_identities)?[...new Set(r.generation_attempt_identities.map(String))].sort():[],actualAttempts=attempts.map(attempt=>String(attempt.generation_attempt_identity)).sort(),rankOne=attempts.filter(attempt=>attempt.expiry_rank===1);if(JSON.stringify(persistedAttempts)!==JSON.stringify(actualAttempts))errors.push(`Configuration opportunity ${String(r.opportunity_id)} does not retain its complete generation-attempt provenance.`);if(typeof r.candidate_id==="string"&&(attempts.length===1?attempts[0]?.candidate_id!==r.candidate_id:rankOne.length!==1||rankOne[0]?.candidate_id!==r.candidate_id))errors.push(`Configuration opportunity ${String(r.opportunity_id)} canonical candidate is not the unique persisted ex-ante rank-1 attempt.`);const state=obj(r.economic_opportunity);if(!["trade","explicit_no_trade","unavailable"].includes(String(state.status)))errors.push(`Configuration opportunity ${String(r.opportunity_id)} lacks a structured economic status.`)}
 ...
 const researchRoles=new Set(["short_strike_technical","short_strike_buffered","comparative_economics"]);
 const comparativeClaims=new Map<string,Set<string>>();for(const r of rows("candidates.jsonl")){if(r.research_role!=="comparative_economics")continue;const key=`${String(r.event_id)}|${String(r.structural_configuration_id)}`,claims=comparativeClaims.get(key)??new Set<string>();claims.add(String(r.candidate_id));comparativeClaims.set(key,claims);if(!opportunityKeys.has(key))errors.push(`Comparative Economics candidate ${String(r.candidate_id)} does not match a canonical event/configuration opportunity.`)}for(const [key,claims] of comparativeClaims){if(claims.size>1)errors.push(`Multiple Comparative Economics materializations claim ${key}: ${[...claims].sort().join(", ")}.`);const opportunity=rows("configuration_opportunities.jsonl").find(row=>`${String(row.event_id)}|${String(row.structural_configuration_id)}`===key),claim=[...claims][0];if(claims.size===1&&opportunity?.candidate_id!==claim)errors.push(`Comparative Economics materialization ${claim} is not the canonical candidate for ${key}.`)}for(const opportunity of rows("configuration_opportunities.jsonl")){const candidateId=typeof opportunity.candidate_id==="string"?opportunity.candidate_id:null,attemptIds=Array.isArray(opportunity.attempt_candidate_ids)?opportunity.attempt_candidate_ids.map(String):[];if(candidateId&&!attemptIds.includes(candidateId))errors.push(`Configuration opportunity ${String(opportunity.opportunity_id)} canonical candidate is absent from attempt provenance.`);const materializations=rows("candidates.jsonl").filter(candidate=>candidate.event_id===opportunity.event_id&&candidate.candidate_id===candidateId&&candidate.structural_configuration_id===opportunity.structural_configuration_id),comparative=materializations.find(candidate=>candidate.research_role==="comparative_economics");if(!migratedFrom&&comparative){const authoritative=economicOpportunity(comparative.modeled_execution);if(JSON.stringify(opportunity.economic_opportunity)!==JSON.stringify(authoritative)||opportunity.status!==authoritative.status)errors.push(`Configuration opportunity ${String(opportunity.opportunity_id)} does not equal its authoritative Comparative Economics modeled_expected state.`)}if(candidateId&&!materializations.length)errors.push(`Configuration opportunity ${String(opportunity.opportunity_id)} canonical materialization does not match its event × structural configuration.`);if(!migratedFrom&&["explicit_no_trade","unavailable"].includes(String(obj(opportunity.economic_opportunity).status))&&materializations.some(candidate=>{const expected=obj(obj(candidate.modeled_execution).expected),entry=obj(expected.entrySnapshot);return expected.status==="evaluated"&&[entry.grossSpreadBtc,entry.netOpeningCashFlowBtc].some(value=>num(value)!==null)}))errors.push(`Configuration opportunity ${String(opportunity.opportunity_id)} is ${String(obj(opportunity.economic_opportunity).status)} but its canonical modeled_expected materialization has a priced entry.`)}
 for(const r of rows("candidates.jsonl")){const identity=structuralConfigurationIdentity(r),sourceIdentity=structuralConfigurationIdentity({...r,structural_configuration:null});if(typeof r.structural_configuration_id!=="string"||r.structural_configuration_version!==STRUCTURAL_CONFIGURATION_ID_VERSION||identity.id===null||identity.id!==r.structural_configuration_id||sourceIdentity.id!==identity.id||JSON.stringify(identity.fields)!==JSON.stringify(r.structural_configuration))errors.push(`Candidate ${String(r.candidate_id)} has an invalid or inconsistent structural configuration identity.`);if(!eventIds.has(r.event_id))errors.push(`Candidate ${r.candidate_id} has a broken event foreign key.`);if(r.is_selected!==true&&!researchRoles.has(String(r.research_role)))errors.push(`Research-only candidate ${r.candidate_id} has no valid research role.`);if(r.is_selected===true&&!r.selection_id)errors.push(`Selected candidate ${r.candidate_id} has no selected provenance.`);if(r.is_selected===false&&r.execution_scenario_status==="evaluated")errors.push(`Research-only candidate ${r.candidate_id} carries fabricated evaluated ${String(r.execution_scenario)} execution.`);if(!availabilityIds.has(r.candidate_id))errors.push(`Candidate ${r.candidate_id} is absent from generation provenance.`)}
 ...
 const availabilityCandidates=new Set(rows("availability.jsonl").map(r=>r.candidate_id));for(const id of candidateIds)if(!availabilityCandidates.has(id))errors.push(`Exported candidate ${String(id)} is absent from generation provenance.`);
 const availabilitySelection=new Map(rows("availability.jsonl").map(r=>[String(r.candidate_id),r.is_selected===true]));
 for(const r of rows("candidates.jsonl"))if(r.is_selected===true&&availabilitySelection.get(String(r.candidate_id))!==true)errors.push(`Selected candidate ${String(r.candidate_id)} conflicts with generation availability.`);
 ...
 // ---------------------------------------------------------------------------
 const economicsByCandidate=new Map(rows("structure_economics.jsonl").map(r=>[String(r.materialization_id),r]));
 ...
 for(const m of rows("margin_scenarios.jsonl")){
  const economics=economicsByCandidate.get(String(m.materialization_id));if(!economics)continue;
 ...
 const selectedCandidateIds=new Set(rows("candidates.jsonl").filter(r=>r.is_selected===true).map(r=>String(r.candidate_id))),comparativeMarginCandidateIds=new Set(rows("candidates.jsonl").filter(r=>r.research_role==="comparative_economics").map(r=>String(r.candidate_id)));
 for(const id of selectedCandidateIds)if(!marginCandidates.has(id))errors.push(`Selected candidate ${id} has no margin scenario row.`);
 for(const id of comparativeMarginCandidateIds)if(!marginCandidates.has(id))errors.push(`Comparative Economics candidate ${id} has no explicit verified-margin or unavailable-margin row.`);
 for(const id of marginCandidates)if(!selectedCandidateIds.has(id)&&!comparativeMarginCandidateIds.has(id))errors.push(`Non-comparative research-only candidate ${id} must not enter margin scenarios.`);
 // --- analytics tracks: rows must reflect what the producing engine persisted ---
 const trackRows=new Map<string,Row>();
 for(const r of rows("structure_economics.jsonl"))for(const t of (Array.isArray(r.tracks)?r.tracks:[]))trackRows.set(`${String(r.materialization_id)}~${String(obj(t as JsonValue).track)}`,obj(t as JsonValue));
 const countBy=(name:string)=>{const counts=new Map<string,number>();for(const r of rows(name)){const key=`${String(r.materialization_id)}~${String(r.analytics_track)}`;counts.set(key,(counts.get(key)??0)+1)}return counts};
 ...
 for(const r of rows("valuations.jsonl"))if(r.valuation_status==="priced"){const key=`${String(r.materialization_id)}~${String(r.analytics_track)}`;pricedValuationCounts.set(key,(pricedValuationCounts.get(key)??0)+1)}
 ...
 for(const r of [...rows("outcomes.jsonl")])outcomeRows.set(`${String(r.materialization_id)}~${String(r.analytics_track)}~${String(r.outcome_type)}`,r);
 ...
 for(const r of rows("outcomes.jsonl")){
  ...
  if(holding===null){if(r.status==="evaluated"&&r.valuation_timestamp_utc!=null)errors.push(`${String(r.materialization_id)} ${String(r.outcome_type)} is evaluated at a known valuation timestamp but reports no holding time.`);continue}
  if(holding<0)errors.push(`${String(r.materialization_id)} ${String(r.outcome_type)} reports a negative holding time.`);
  if(r.status!=="evaluated")errors.push(`${String(r.materialization_id)} ${String(r.outcome_type)} is ${String(r.status)} but reports a holding time.`);
  const descriptor=trackRows.get(`${String(r.materialization_id)}~${String(r.analytics_track)}`);
  ...
  if(Number.isFinite(entry)&&Number.isFinite(close)&&Math.abs(holding-(close-entry)/36e5)>1e-6)
   errors.push(`${String(r.materialization_id)} ${String(r.outcome_type)} holding time is not measured from that track's own entry.`);
  const expiry=Date.parse(String(economicsByCandidate.get(String(r.materialization_id))?.expiry_timestamp_utc));
  if(Number.isFinite(expiry)&&Number.isFinite(close)&&close>expiry)errors.push(`${String(r.materialization_id)} ${String(r.outcome_type)} reports a holding time past expiry.`);
 }
 ...
 for(const r of rows("outcomes.jsonl")){
  const key=`${String(r.materialization_id)}~${String(r.analytics_track)}`,descriptor=trackRows.get(key);
 ...
 for(const r of rows("valuations.jsonl")){
  const track=String(r.analytics_track),key=`${String(r.materialization_id)}~${track}`,descriptor=trackRows.get(key);
  if(!descriptor){errors.push(`Valuation row references undeclared analytics track ${track} for ${String(r.materialization_id)}.`);continue}
 ...
 for(const r of rows("structure_economics.jsonl")){
  const reference=Date.parse(String(obj(trackRows.get(`${String(r.materialization_id)}~reference_fair_value`)??{}).entry_timestamp_utc));
  for(const track of ["delayed_maker","delayed_taker"]){
   const descriptor=trackRows.get(`${String(r.materialization_id)}~${track}`);if(!descriptor)continue;
   const entry=Date.parse(String(descriptor.entry_timestamp_utc));
   if(Number.isFinite(entry)&&Number.isFinite(reference)&&entry<reference)errors.push(`${String(r.materialization_id)} ${track} opens before the reference entry; a delayed fill is never backdated.`);
  }
 }
 ...
 ["futures_path",rows("futures_path.jsonl").length>0],
  ["entry_delay_sensitivity",rows("entry_delay_sensitivity.jsonl").some(r=>r.status==="available")],
 ];
 ...
 const delaySeen=new Set<string>();
 for(const r of rows("entry_delay_sensitivity.jsonl")){
  const key=`${String(r.candidate_id)}~${String(r.requested_delay_hours)}~${String(r.execution_scenario)}`;
  if(delaySeen.has(key))errors.push(`Duplicate fixed-offset entry-delay row ${key}.`);else delaySeen.add(key);
  if(!candidateIds.has(String(r.candidate_id)))errors.push(`Entry-delay row ${key} has a broken candidate foreign key.`);
  if(![0,4,8,12].includes(Number(r.requested_delay_hours)))errors.push(`Entry-delay row ${key} has an unsupported requested offset.`);
  if(num(r.requested_search_window_minutes)!==FIXED_ENTRY_DELAY_SEARCH_WINDOW_MS/6e4)errors.push(`Entry-delay row ${key} does not use the canonical fixed search window.`);
  const target=Date.parse(String(r.target_entry_timestamp_utc)),actual=Date.parse(String(r.actual_entry_timestamp_utc));
  if(r.status==="available"&&(!Number.isFinite(target)||!Number.isFinite(actual)||!isInsideFixedEntryDelayWindow(target,actual)))errors.push(`Entry-delay row ${key} is available without evidence inside its causal fixed window.`);
  if(r.status!=="available"&&r.actual_entry_timestamp_utc!=null)errors.push(`Entry-delay row ${key} is ${String(r.status)} but carries an actual entry.`);
 }
 ...
 for(const id of candidateIds){
  ...
  const explicitUnavailableResearch=base.is_selected===false&&researchRoles.has(String(base.research_role))&&obj(base.reference_valuation).status==="unavailable"&&typeof obj(base.reference_valuation).reason==="string";
  if(!audit.admissible&&!explicitUnavailableResearch&&!(migratedFrom&&["1.0.0","2.0.0","2.1.0","2.2.0","2.3.0","3.0.0","3.1.0","3.2.0"].includes(migratedFrom)&&scenarios.every(r=>r.legacy_economic_track_status==="unavailable")))errors.push(`Candidate ${String(id)} has no valid analytical track.${audit.referenceErrors.length?` Reference valuation: ${audit.referenceErrors.join("; ")}.`:""}`);
 }
 ...
 for(const r of structureVolatilityRows){
  ...
  for(const collection of ["post_entry_market_iv","market_iv_path"]){
   ...
   for(const point of arr(r[collection])){
    ...
    if(collection==="post_entry_market_iv"&&["vpoc","invalidation","credit_capture_50","credit_capture_70","fixed_3d","fixed_5d","fixed_7d"].includes(String(point.endpoint_id))){const outcome=rows("outcomes.jsonl").find(outcome=>outcome.candidate_id===r.candidate_id&&outcome.analytics_track==="reference_fair_value"&&outcome.outcome_type===point.endpoint_id);if(outcome&&outcome.valuation_timestamp_utc!=null&&point.target_timestamp_utc!==outcome.valuation_timestamp_utc)errors.push(`${pointContext} target ${String(point.target_timestamp_utc)} disagrees with canonical Reference outcome ${String(outcome.valuation_timestamp_utc)}.`)}
