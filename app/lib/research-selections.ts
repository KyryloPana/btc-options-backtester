...
export function compactResearchSelectionEvent(event:ResearchSelectionEvent):ResearchSelectionEvent{
 ...
 const compactStructure=<T extends SelectedStructure>(structure:T):T=>{
  const usages=(structure.evidenceUsages??[]).map(usage=>({...usage,evidenceId:evidenceIdRemap.get(usage.evidenceId)??usage.evidenceId}));
  ...
  const compacted={...structure,...(delayedExecution===undefined?{}:{delayedExecution}),...(modeledExecution===undefined?{}:{modeledExecution}),...(!evidenceUsages.length&&structure.evidenceUsages===undefined?{}:{evidenceUsages})} as T;
 ...
 return{...event,selectedStructures,...(researchStructures===undefined?{}:{researchStructures}),...(event.evidenceCatalog===undefined&&!catalog.size?{}:{evidenceCatalog:[...catalog.values()].sort((a,b)=>a.evidenceId.localeCompare(b.evidenceId))})};
}
/**
