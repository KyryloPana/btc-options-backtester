...
export const RESEARCH_SELECTION_SCHEMA_VERSION = "1.10.0" as const;
/** Every schema version this app can still read and migrate forward from. */
export const LEGACY_RESEARCH_SELECTION_SCHEMA_VERSIONS = ["1.0.0", "1.1.0", "1.2.0", "1.3.0", "1.4.0", "1.5.0", "1.6.0", "1.7.0", "1.8.0", "1.9.0"] as const;
...
export interface ResearchOnlyStructure extends SelectedStructure { researchRole:ResearchRole; structuralConfigurationId?:string; generationAttemptIdentities?:string[]; /** @deprecated schema <=1.9 compatibility only */ attemptCandidateIds?:string[] }
...
export function comparativeMaterializationId(eventId:string,structuralConfigurationId:string){return `research~comparative-economics~${part(eventId)}~${part(structuralConfigurationId)}`;}
...
export function validateResearchSelectionStore(value:unknown):{ok:true;store:ResearchSelectionStore}|{ok:false;errors:SelectionValidationError[]} {
  ...
  for(const [i,event] of (Array.isArray(store.events)?store.events:[]).entries()){
    ...
    for(const [j,s] of (Array.isArray(event.researchStructures)?event.researchStructures:[]).entries()){
      ...
      const key=s.researchRole==="comparative_economics"?`comparative:${s.structuralConfigurationId}`:`${s.researchRole}:${s.venue}:${s.candidateId}`;if(researchKeys.has(key))errors.push({path:q,message:"Duplicate research materialization identity."});researchKeys.add(key);
      const generated=Array.isArray(event.generationSnapshot?.candidates)?event.generationSnapshot.candidates:[];
      const configurationRows=s.researchRole==="comparative_economics"&&typeof s.structuralConfigurationId==="string"?generated.filter(candidate=>generationStructuralConfiguration(candidate).id===s.structuralConfigurationId):[];
      const generatedCandidate=(s.researchRole==="comparative_economics"?(configurationRows.find(candidate=>candidate.candidateId===s.candidateId)??(store.schemaVersion!==RESEARCH_SELECTION_SCHEMA_VERSION?generated.find(candidate=>candidate.candidateId===s.candidateId):undefined)):generated.find(c=>c&&typeof c==="object"&&(c as GenerationCandidateSnapshot).candidateId===s.candidateId)) as GenerationCandidateSnapshot|undefined;
      if(!generatedCandidate)errors.push({path:`${q}.candidateId`,message:"Research candidate is absent from generation provenance."});
      if(s.researchRole==="comparative_economics"&&generatedCandidate){const configurationId=s.structuralConfigurationId!;if(comparativeKeys.has(configurationId))errors.push({path:q,message:"Only one Comparative Economics materialization may claim an event × structural configuration."});else comparativeKeys.add(configurationId);const attempts=configurationRows.map(generationAttemptIdentity).sort(),persisted=[...new Set(s.generationAttemptIdentities??[])].sort(),rankOne=configurationRows.filter(candidate=>candidate.expiryRank===1);if(store.schemaVersion===RESEARCH_SELECTION_SCHEMA_VERSION&&s.selectionId!==comparativeMaterializationId(event.eventId,configurationId))errors.push({path:`${q}.selectionId`,message:"Comparative Economics materialization identity must be event × structural configuration."});if(store.schemaVersion===RESEARCH_SELECTION_SCHEMA_VERSION&&JSON.stringify(attempts)!==JSON.stringify(persisted))errors.push({path:`${q}.generationAttemptIdentities`,message:"Comparative Economics materialization must retain every exact generation-attempt identity for its canonical configuration."});if(configurationRows.length===1?configurationRows[0]!.candidateId!==s.candidateId:rankOne.length!==1||rankOne[0]!.candidateId!==s.candidateId)errors.push({path:`${q}.candidateId`,message:"Comparative Economics materialization must be the persisted ex-ante rank-1 candidate inside its exact configuration; row order and outcomes are not admissible selectors."});}
...
export function preserveControlledGenerationProvenance(current:GenerationCandidateSnapshot[],previous:GenerationCandidateSnapshot[],preserved:ResearchOnlyStructure[],controlledAuthoritative:boolean):GenerationCandidateSnapshot[]{
 ...
 const requiredAttempts=new Set(preserved.flatMap(structure=>structure.generationAttemptIdentities??[])),legacyCandidateIds=new Set(preserved.filter(structure=>!structure.generationAttemptIdentities?.length).map(structure=>structure.candidateId)),attempts=new Set(current.map(generationAttemptIdentity)),retained:GenerationCandidateSnapshot[]=[];
 for(const candidate of previous){const attempt=generationAttemptIdentity(candidate);if((requiredAttempts.has(attempt)||legacyCandidateIds.has(candidate.candidateId))&&!attempts.has(attempt)){attempts.add(attempt);retained.push(structuredClone(candidate));}}
...
function withoutRuntimeEvidenceCopies(value:unknown):JsonValue{if(value===undefined||typeof value==="number"&&!Number.isFinite(value))return null;if(value===null||typeof value==="string"||typeof value==="boolean"||typeof value==="number")return value;if(Array.isArray(value))return value.map(withoutRuntimeEvidenceCopies);const result:Record<string,JsonValue>={};for(const [key,child] of Object.entries(asObj(value)))if(key!=="supportingTrades"&&key!=="calibrationRecords"&&key!=="records")result[key]=withoutRuntimeEvidenceCopies(child);return result;}
function compactModeledTrack(input:unknown):JsonValue{const t=asObj(input);return canonicalJson({status:t.status,reasonCode:t.reasonCode,reason:t.reason,source:t.source,modelVersion:t.modelVersion,penaltyBps:t.penaltyBps,assumptionOrCalibration:t.assumptionOrCalibration,calibrationCount:t.calibrationCount,calibration:compactModeledCalibration(t.calibration),entryTimestamp:t.entryTimestamp,entrySnapshot:withoutRuntimeEvidenceCopies(t.entrySnapshot),attemptedEntrySnapshot:withoutRuntimeEvidenceCopies(t.attemptedEntrySnapshot),economicRejection:t.economicRejection,artifactEligibleTradeCount:t.artifactEligibleTradeCount,outcomeSnapshots:withoutRuntimeEvidenceCopies(t.outcomeSnapshots),uncertainty:t.uncertainty,referenceQuality:t.referenceQuality,provenance:t.provenance,pathDerivation:t.status==="evaluated"?{kind:"reference-marks-plus-modeled-opening-ledger",reference:"referenceValuation.valuationPathSnapshot",version:"1"}:null});}
...
/**
 * Comparative Economics consumes the canonical entry and outcome snapshots,
 * not the Reference engine's repeated per-leg market reconstruction at every
 * four-hour mark.  Keep the independently calculated path economics (rather
 * than fabricating them on reload), plus the small amount of point provenance
 * used by bundle validation.  Selected Strategy structures deliberately do
 * not pass through this DTO.
 */
export function compactComparativeReferencePoint(input:unknown):JsonValue{
 const p=asObj(input),resolution=asObj(p.indexResolution);
 const dto={timestamp:p.timestamp,status:p.status,targetIndex:p.targetIndex,indexResolution:Object.keys(resolution).length?Object.fromEntries([["sourceCandleTimestamp",resolution.sourceCandleTimestamp],["lookupMethod",resolution.lookupMethod]].filter(([,value])=>value!==undefined)):undefined,unavailableReason:p.unavailableReason,rawUnavailableReason:p.rawUnavailableReason,missingField:p.missingField,closingSpreadValueBtcPerContract:p.closingSpreadValueBtcPerContract,closingSpreadValueBtc:p.closingSpreadValueBtc,closingFeesBtc:p.closingFeesBtc,estimatedNetPnlBtc:p.estimatedNetPnlBtc,rawClosingSpreadValueBtcPerContract:p.rawClosingSpreadValueBtcPerContract,rawClosingSpreadValueBtc:p.rawClosingSpreadValueBtc,rawClosingFeesBtc:p.rawClosingFeesBtc,rawEstimatedNetPnlBtc:p.rawEstimatedNetPnlBtc,estimateQuality:p.estimateQuality};
 return canonicalJson(Object.fromEntries(Object.entries(dto).filter(([,value])=>value!==undefined)));
}

/** The authoritative runtime -> persisted event boundary used by save, refresh and migration. */
export function compactResearchSelectionEvent(event:ResearchSelectionEvent):ResearchSelectionEvent{
 const evidenceIdRemap=new Map<string,string>(),catalog=new Map<string,EvidenceTradeDto>();
 for(const evidence of event.evidenceCatalog??[]){const normalized=evidenceTradeDto(evidence.venue,evidence as unknown as Record<string,unknown>);evidenceIdRemap.set(evidence.evidenceId,normalized.evidenceId);catalog.set(normalized.evidenceId,normalized)}
 const compactStructure=<T extends SelectedStructure>(structure:T):T=>{
  const comparative=(structure as Partial<ResearchOnlyStructure>).researchRole==="comparative_economics";
  const {attemptCandidateIds:_legacyAttemptCandidateIds,...withoutLegacyAttemptIds}=structure as T&Partial<ResearchOnlyStructure>;void _legacyAttemptCandidateIds;
  // Reference path market-print evidence is not an input to any canonical
  // Comparative Economics table. Entry/outcome and all selected-Strategy
  // usages remain untouched.
  const usages=(structure.evidenceUsages??[]).filter(usage=>!comparative||usage.role!=="valuation").map(usage=>({...usage,evidenceId:evidenceIdRemap.get(usage.evidenceId)??usage.evidenceId}));
  const delayedExecution=structure.delayedExecution===undefined?undefined:compactDelayedExecution(structure.delayedExecution,structure.venue,structure.candidateId,usages,catalog),modeledExecution=structure.modeledExecution===undefined?undefined:compactModeledExecution(structure.modeledExecution);
  const evidenceUsages=[...new Map(usages.map(usage=>[JSON.stringify(usage),usage])).values()].sort((a,b)=>JSON.stringify(a).localeCompare(JSON.stringify(b)));
  const referenceValuation=comparative&&structure.referenceValuation?{...structure.referenceValuation,valuationPathSnapshot:structure.referenceValuation.valuationPathSnapshot.map(compactComparativeReferencePoint)}:structure.referenceValuation;
  const compacted={...(comparative?withoutLegacyAttemptIds:structure),...(referenceValuation===undefined?{}:{referenceValuation}),...(delayedExecution===undefined?{}:{delayedExecution}),...(modeledExecution===undefined?{}:{modeledExecution}),...(!evidenceUsages.length&&structure.evidenceUsages===undefined?{}:{evidenceUsages})} as T;
  if(structure.evidenceTradeSnapshots!==undefined)compacted.evidenceTradeSnapshots=[];
  return compacted;
 };
 const selectedStructures=event.selectedStructures.map(compactStructure),researchStructures=event.researchStructures?.map(compactStructure);
 const referencedEvidenceIds=new Set([...selectedStructures,...(researchStructures??[])].flatMap(structure=>(structure.evidenceUsages??[]).map(usage=>usage.evidenceId)));
 const evidenceCatalog=[...catalog.values()].filter(evidence=>referencedEvidenceIds.has(evidence.evidenceId)).sort((a,b)=>a.evidenceId.localeCompare(b.evidenceId));
 return{...event,selectedStructures,...(researchStructures===undefined?{}:{researchStructures}),...(event.evidenceCatalog===undefined&&!evidenceCatalog.length?{}:{evidenceCatalog})};
}
/**
...
export function migrateResearchSelectionStore(value:unknown):ResearchSelectionStore{const checked=validateResearchSelectionStore(value);if(!checked.ok)throw new Error(checked.errors.map(e=>`${e.path}: ${e.message}`).join(" | "));const store=checked.store;const originatingSchema=String(store.schemaVersion);let changed=false;const canonicalize=(scenario:ExecutionScenarioSnapshot,expiry:number|null):ExecutionScenarioSnapshot=>{if(scenario.status!=="evaluated")return scenario;const entryObject=asObj(scenario.entrySnapshot),entry=finite(entryObject.valuationTimestamp)??finite(entryObject.targetTimestamp);if(entry===null||expiry===null)return scenario;let beforeEntryCount=0,afterExpiryCount=0;const valuationPathSnapshot=scenario.valuationPathSnapshot.filter(point=>{const timestamp=finite(asObj(point).timestamp);if(timestamp===null)return true;if(timestamp<entry){beforeEntryCount++;return false}if(timestamp>expiry){afterExpiryCount++;return false}return true});const removedCount=beforeEntryCount+afterExpiryCount;if(!removedCount)return scenario;changed=true;return{...scenario,valuationPathSnapshot,valuationWindowMigration:{originatingSchema,removedCount,beforeEntryCount,afterExpiryCount}}};if(store.schemaVersion===RESEARCH_SELECTION_SCHEMA_VERSION){const events=store.events.map(event=>compactResearchSelectionEvent({...event,selectedStructures:event.selectedStructures.map(structure=>{const expiry=finite(asObj(structure.candidateSnapshot).expiryTimestamp),maker=canonicalize(structure.executionScenarios.maker,expiry),taker=canonicalize(structure.executionScenarios.taker,expiry);return maker===structure.executionScenarios.maker&&taker===structure.executionScenarios.taker?structure:{...structure,executionScenarios:{maker,taker}}})}));return {...store,events}}const wasLegacy=true;changed=true;const events=store.events.map(event=>{const catalog=new Map<string,EvidenceTradeDto>((event.evidenceCatalog??[]).map(t=>[t.evidenceId,t]));const selectedStructures=event.selectedStructures.map(raw=>{const legacy=raw as unknown as Record<string,unknown>;const usages:EvidenceUsageDto[]=[...(raw.evidenceUsages??[])];for(const t of (legacy.evidenceTradeSnapshots as unknown[]|undefined)??[]){const dto=evidenceTradeDto(raw.venue,asObj(t));catalog.set(dto.evidenceId,dto);usages.push({evidenceId:dto.evidenceId,candidateId:raw.candidateId,role:"legacy-saved-source-trade",valuationTimestamp:finite(asObj(t).timestamp),pricingTrack:null,leg:null,executionScenario:null});}
 ...
 });const materializations=[...selectedStructures,...(event.researchStructures??[])],configurationClaims=new Map<string,Set<string>>();for(const structure of materializations){const generated=event.generationSnapshot.candidates.find(candidate=>candidate.candidateId===structure.candidateId),configurationId=generated?generationStructuralConfiguration(generated).id:null;if(!configurationId)continue;const claims=configurationClaims.get(configurationId)??new Set<string>();claims.add(structure.candidateId);configurationClaims.set(configurationId,claims)}const ambiguousAttempts=[...new Set(event.generationSnapshot.candidates.map(candidate=>generationStructuralConfiguration(candidate).id).filter((id):id is string=>id!==null))].filter(configurationId=>{const attempts=event.generationSnapshot.candidates.filter(candidate=>generationStructuralConfiguration(candidate).id===configurationId);return attempts.length>1&&attempts.filter(candidate=>candidate.expiryRank===1).length!==1}),conflicts=[...new Set([...configurationClaims].filter(([,claims])=>claims.size>1).map(([configurationId])=>configurationId).concat(ambiguousAttempts))].sort();const researchStructures=(event.researchStructures??[]).map(structure=>{if(structure.researchRole!=="comparative_economics")return structure;const claimed=structure.structuralConfigurationId,matching=claimed?event.generationSnapshot.candidates.filter(candidate=>generationStructuralConfiguration(candidate).id===claimed):event.generationSnapshot.candidates.filter(candidate=>candidate.candidateId===structure.candidateId),configurationIds=[...new Set(matching.map(candidate=>generationStructuralConfiguration(candidate).id).filter((id):id is string=>id!==null))],structuralConfigurationId=claimed??(configurationIds.length===1?configurationIds[0]:undefined);if(!structuralConfigurationId)return structure;const attempts=event.generationSnapshot.candidates.filter(candidate=>generationStructuralConfiguration(candidate).id===structuralConfigurationId),{attemptCandidateIds:_legacyAttemptCandidateIds,...withoutLegacyAttemptIds}=structure;void _legacyAttemptCandidateIds;return{...withoutLegacyAttemptIds,structuralConfigurationId,selectionId:comparativeMaterializationId(event.eventId,structuralConfigurationId),generationAttemptIdentities:attempts.map(generationAttemptIdentity).sort()}});return{...event,selectedStructures,researchStructures,...(conflicts.length?{comparativeEconomicsStatus:{status:"unavailable" as const,reasonCode:"legacy_comparative_materialization_unresolved",reason:`Legacy schema ${originatingSchema} cannot reproduce one canonical materialization for ${conflicts.length} event × structural configuration identit${conflicts.length===1?"y":"ies"}; regenerate/reselect before Comparative Economics.`}}:{}),evidenceCatalog:[...catalog.values()].sort((a,b)=>a.evidenceId.localeCompare(b.evidenceId))};});return {...store,schemaVersion:RESEARCH_SELECTION_SCHEMA_VERSION,events:events.map(compactResearchSelectionEvent)};}
export function researchEventPayloadDiagnostics(event:ResearchSelectionEvent){const encoder=new TextEncoder(),bytes=(value:unknown)=>encoder.encode(JSON.stringify(value)).byteLength,track=(value:ExecutionScenarioSnapshot|IndependentTrackSnapshot|undefined)=>({totalBytes:bytes(value??null),entryBytes:bytes(value?.entrySnapshot??null),pathBytes:bytes(value?.valuationPathSnapshot??[]),outcomeBytes:bytes(value?.outcomeSnapshots??[]),pathPointCount:value?.valuationPathSnapshot?.length??0}),structure=(value:SelectedStructure)=>{const candidate=asObj(value.candidateSnapshot);return{candidateId:value.candidateId,candidate_id:value.candidateId,structural_configuration_id:(value as Partial<ResearchOnlyStructure>).structuralConfigurationId??null,targetDte:candidate.targetDte??null,actualDte:candidate.actualDte??null,totalBytes:bytes(value),candidateSnapshotBytes:bytes(value.candidateSnapshot),contractResolutionBytes:bytes(value.contractResolution??null),statusLayersBytes:bytes(value.statusLayers??null),maker:track(value.executionScenarios.maker),taker:track(value.executionScenarios.taker),reference:track(value.referenceValuation),referenceValuationBytes:bytes(value.referenceValuation??null),referenceEntryBytes:bytes(value.referenceValuation?.entrySnapshot??null),referencePathBytes:bytes(value.referenceValuation?.valuationPathSnapshot??[]),referenceOutcomeBytes:bytes(value.referenceValuation?.outcomeSnapshots??[]),referenceBytes:bytes(value.referenceValuation??null),legacyModel:track(value.legacyModelTrack),makerEntrySnapshotBytes:bytes(value.executionScenarios.maker.entrySnapshot),makerValuationPathBytes:bytes(value.executionScenarios.maker.valuationPathSnapshot),makerOutcomesBytes:bytes(value.executionScenarios.maker.outcomeSnapshots),takerEntrySnapshotBytes:bytes(value.executionScenarios.taker.entrySnapshot),takerValuationPathBytes:bytes(value.executionScenarios.taker.valuationPathSnapshot),takerOutcomesBytes:bytes(value.executionScenarios.taker.outcomeSnapshots),delayedExecutionBytes:bytes(value.delayedExecution??null),modeledExecutionBytes:bytes(value.modeledExecution??null),marginBytes:bytes(value.marginSnapshot),evidenceBytes:bytes(value.evidenceTradeSnapshots??[]),evidenceUsageBytes:bytes(value.evidenceUsages??[])}};
 const research=event.researchStructures??[],researchRows=research.map(value=>({...structure(value),researchRole:value.researchRole,structuralConfigurationId:value.structuralConfigurationId??null})),components=[...researchRows.flatMap(row=>([[`${row.candidateId}:referencePath`,row.referencePathBytes],[`${row.candidateId}:modeledExecution`,row.modeledExecutionBytes],[`${row.candidateId}:margin`,row.marginBytes],[`${row.candidateId}:evidenceUsages`,row.evidenceUsageBytes]] as const)),["selectedStructures",bytes(event.selectedStructures)] as const,["evidenceCatalog",bytes(event.evidenceCatalog??[])] as const].sort((a,b)=>b[1]-a[1]).slice(0,10);
 return{sourceRunBytes:bytes(event.sourceRun),sourceEventBytes:bytes(event.sourceRun),generationSnapshotBytes:bytes(event.generationSnapshot),generationConfigurationBytes:bytes(event.generationSnapshot.configuration),candidateSnapshotBytes:bytes(event.generationSnapshot.candidates),underlyingPathBytes:bytes(event.generationSnapshot.underlyingHourlyPath),underlyingHourlyPathBytes:bytes(event.generationSnapshot.underlyingHourlyPath),selectedStructuresBytes:bytes(event.selectedStructures),selectedCandidateBytes:event.selectedStructures.map(structure),researchStructuresBytes:bytes(research),comparativeEconomicsResearchStructuresBytes:bytes(research.filter(value=>value.researchRole==="comparative_economics")),shortStrikeResearchStructuresBytes:bytes(research.filter(value=>value.researchRole!=="comparative_economics")),researchStructureCount:research.length,researchCandidateBytes:researchRows,evidenceCatalogBytes:bytes(event.evidenceCatalog??[]),eventEvidenceCatalogBytes:bytes(event.evidenceCatalog??[]),evidenceUsagesBytes:bytes([...event.selectedStructures,...research].flatMap(value=>value.evidenceUsages??[])),top10LargestComponents:components,totalBytes:bytes(event)};}
...
export function renameResearchSelectionEvent(store:ResearchSelectionStore,oldEventId:string,newEventId:string,now=new Date().toISOString()):ResearchSelectionStore{
 ...
 const events=store.events.map(event=>{
  ...
  const researchStructures=event.researchStructures?.map(structure=>{const candidateId=candidateIdFor(structure.candidateId,structure.venue),{attemptCandidateIds:_legacyAttemptCandidateIds,...withoutLegacyAttemptIds}=structure;void _legacyAttemptCandidateIds;return{...withoutLegacyAttemptIds,eventId:newEventId,candidateId,selectionId:structure.researchRole==="comparative_economics"&&structure.structuralConfigurationId?comparativeMaterializationId(newEventId,structure.structuralConfigurationId):`research~${structure.researchRole}~${candidateId}`,...(structure.generationAttemptIdentities===undefined?{}:{generationAttemptIdentities:structure.generationAttemptIdentities.map(identity=>{const parsed=JSON.parse(identity) as [string,...unknown[]];parsed[0]=candidateIdFor(parsed[0],structure.venue);return JSON.stringify(parsed)})}),...(structure.evidenceUsages===undefined?{}:{evidenceUsages:structure.evidenceUsages.map(usage=>({...usage,candidateId:candidateIdFor(usage.candidateId,structure.venue)}))})} satisfies ResearchOnlyStructure;});
