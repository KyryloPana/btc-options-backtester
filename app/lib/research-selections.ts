...
function compactModeledTrack(input:unknown):JsonValue{const t=asObj(input);return canonicalJson({status:t.status,reason:t.reason,source:t.source,modelVersion:t.modelVersion,penaltyBps:t.penaltyBps,assumptionOrCalibration:t.assumptionOrCalibration,calibrationCount:t.calibrationCount,calibration:compactModeledCalibration(t.calibration),entryTimestamp:t.entryTimestamp,entrySnapshot:t.entrySnapshot,outcomeSnapshots:t.outcomeSnapshots,uncertainty:t.uncertainty,referenceQuality:t.referenceQuality,provenance:t.provenance,pathDerivation:t.status==="evaluated"?{kind:"reference-marks-plus-modeled-opening-ledger",reference:"referenceValuation.valuationPathSnapshot",version:"1"}:null});}
...
export function compactResearchSelectionEvent(event:ResearchSelectionEvent):ResearchSelectionEvent{const catalog=new Map<string,EvidenceTradeDto>((event.evidenceCatalog??[]).map(e=>[e.evidenceId,e]));const selectedStructures=event.selectedStructures.map(s=>{const usages=[...(s.evidenceUsages??[])];const delayedExecution=s.delayedExecution===undefined?undefined:compactDelayedExecution(s.delayedExecution,s.venue,s.candidateId,usages,catalog),modeledExecution=s.modeledExecution===undefined?undefined:compactModeledExecution(s.modeledExecution);return{...s,...(delayedExecution===undefined?{}:{delayedExecution}),...(modeledExecution===undefined?{}:{modeledExecution}),...(s.evidenceTradeSnapshots===undefined?{}:{evidenceTradeSnapshots:[]}),...(!usages.length&&s.evidenceUsages===undefined?{}:{evidenceUsages:[...new Map(usages.map(u=>[JSON.stringify(u),u])).values()].sort((a,b)=>JSON.stringify(a).localeCompare(JSON.stringify(b)))})}});return{...event,selectedStructures,...(event.evidenceCatalog===undefined&&!catalog.size?{}:{evidenceCatalog:[...catalog.values()].sort((a,b)=>a.evidenceId.localeCompare(b.evidenceId))})};}
/**
