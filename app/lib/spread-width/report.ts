...
function bandRow(width:number,rows:readonly WidthStructure[]):WidthBandRow{
 ...
 return {
  ...
  metricN:{grossCredit:count(r=>r.entry.grossCreditUsd),netCredit:count(r=>r.entry.netCreditUsd),structuralLoss:count(r=>r.payoff.maximumStructuralLossUsd.value),feeDrag:count(r=>r.entry.feeDragRoundTrip),breakeven:count(r=>r.payoff.breakEvenIndex.value),longLegCost:count(r=>r.protection.longLegPremiumUsd)},
