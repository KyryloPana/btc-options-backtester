...
export class ExecutionCalibrationIndex{
 ...
 constructor(input:readonly ExecutionCalibrationObservation[]|ExecutionEstimatorArtifact){const raw=Array.isArray(input),artifact=input as ExecutionEstimatorArtifact,observations=input as readonly ExecutionCalibrationObservation[],rows:EmpiricalCalibrationRow[]=raw?compileCalibrationRows(observations):artifact.rows;const base={datasetId:EXECUTION_CALIBRATION_DATASET_ID,methodVersion:EXECUTION_CALIBRATION_METHOD_VERSION,sourceDatasetFingerprint:raw?contentHash(rows.map(r=>r.tradeId)):artifact.sourceDatasetFingerprint,coverageStartMs:rows[0]?.timestampMs??null,coverageEndMs:rows.at(-1)?.timestampMs??null,estimatorVersion:EMPIRICAL_TAKER_EXECUTION_VERSION,bucketPolicyVersion:EXECUTION_BUCKET_POLICY_VERSION,tradeCount:rows.length,uniqueDays:new Set(rows.map(r=>r.calendarDate)).size,uniqueExpiryDayGroups:new Set(rows.map(r=>r.expiryDayGroup)).size,quantileMethod:QUANTILE_METHOD,rows};this.artifact={...base,artifactHash:raw?contentHash(base):artifact.artifactHash}}
 execution(target:{action:TakerAction;dteDays:number;amount:number;forwardMoneyness:number;cutoffMs:number}):BucketEstimate|null{const band=dteBand(target.dteDays);if(!band||Math.abs(target.forwardMoneyness)>0.35)return null;const causal=this.artifact.rows.filter(r=>r.timestampMs<target.cutoffMs),ab=amountBand(target.amount),levels=[...[{name:"action_dte_amount" as const,rows:causal.filter(r=>r.action===target.action&&r.dteBand===band&&r.amountBand===ab)},{name:"action_dte" as const,rows:causal.filter(r=>r.action===target.action&&r.dteBand===band)},{name:"action" as const,rows:causal.filter(r=>r.action===target.action)}],{name:"global" as const,rows:causal}];for(const level of levels){const s=support(level.rows);if(adequate(s,level.name==="global")){return{fallbackLevel:level.name,...s,q50VolPoints:quantileR7(level.rows.map(r=>r.concessionVolPoints),.5)!,q90VolPoints:quantileR7(level.rows.map(r=>r.concessionVolPoints),.9)!,dteBand:band,amountBand:ab}}}return null}
 reference(target:{dteDays:number;cutoffMs:number}):ReferenceEstimate|null{const band=dteBand(target.dteDays);if(!band)return null;const causal=this.artifact.rows.filter(r=>r.timestampMs<target.cutoffMs),levels=[{name:"dte" as const,rows:causal.filter(r=>r.dteBand===band)},{name:"global" as const,rows:causal}];for(const level of levels){const s=support(level.rows);if(adequate(s,level.name==="global"))return{fallbackLevel:level.name,...s,q90VolPoints:quantileR7(level.rows.map(r=>r.referenceErrorVolPoints),.9)!,dteBand:band}}return null}
}
