...
export function MonetaryValue({btc=null,usd=null,unavailableReason="Requested currency unavailable from canonical evidence."}:{btc?:number|null;usd?:number|null;unavailableReason?:string}){const preferred=useDisplayCurrency(),selected=selectDisplayMoney(preferred,{btc,usd});if(!selected)return <span className="metric-unavailable" title={unavailableReason}>Unavailable</span>;return <span className={selected.fallback?"currency-fallback":undefined} title={selected.fallback?unavailableReason:undefined}>{formatDisplayMoney(selected.value,selected.currency)}{selected.fallback&&<small> · {preferred.toUpperCase()} unavailable</small>}</span>}
