...
export function ResearchSectionNav({items}:{items:readonly ResearchSectionLink[]}){
 ...
 return <nav className="research-section-nav" aria-label="Research Analytics sections"><strong>Sections</strong><div>{items.map(item=><button type="button" key={item.id} aria-current={active===item.id?"location":undefined} onClick={()=>{document.getElementById(item.id)?.scrollIntoView({behavior:"smooth",block:"start"});setActive(item.id)}}>{item.label}</button>)}</div></nav>;
}
