...
export function ResearchSectionNav({items,currency,onCurrencyChange}:{items:readonly ResearchSectionLink[];currency:DisplayCurrency;onCurrencyChange:(currency:DisplayCurrency)=>void}){
 ...
 useEffect(()=>{const timer=setTimeout(()=>setActive(items[0]?.id??""),0);if(typeof IntersectionObserver==="undefined")return()=>clearTimeout(timer);const observer=new IntersectionObserver(entries=>{const visible=entries.filter(entry=>entry.isIntersecting).sort((a,b)=>a.boundingClientRect.top-b.boundingClientRect.top)[0];if(visible)setActive(visible.target.id)},{rootMargin:"-56px 0px -65%",threshold:[0,.1]});for(const item of items){const node=document.getElementById(item.id);if(node)observer.observe(node)}return()=>{clearTimeout(timer);observer.disconnect()}},[items]);
 return <nav className="research-section-nav" aria-label="Research Analytics sections"><fieldset className="display-currency-control"><legend>Display</legend>{(["usd","btc"] as const).map(option=><button type="button" key={option} aria-pressed={currency===option} onClick={()=>onCurrencyChange(option)}>{option==="btc"?"Native BTC":"USD"}</button>)}</fieldset><strong>Sections</strong><div>{items.map(item=><button type="button" key={item.id} aria-current={active===item.id?"location":undefined} onClick={()=>{document.getElementById(item.id)?.scrollIntoView({behavior:"smooth",block:"start"});setActive(item.id)}}>{item.label}</button>)}</div></nav>;
}
