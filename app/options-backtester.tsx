...
import { LOCAL_PERSISTENCE_REQUIRED_MESSAGE, persistenceEndpointUnavailable } from "./lib/local-persistence";

type Section = "events" | "construction" | "contracts" | "analysis";
type ResultTrack = "reference" | "immediate-maker" | "immediate-taker" | "delayed-maker" | "delayed-taker" | "modeled-expected" | "modeled-conservative" | "execution-penalty";

const RESULT_TRACKS: ReadonlyArray<readonly [ResultTrack,string]> = [
  ["reference","Reference fair value"], ["immediate-maker","Immediate maker"], ["immediate-taker","Immediate taker"],
  ["delayed-maker","Delayed observed maker"], ["delayed-taker","Delayed observed taker"],
  ["modeled-expected","Modeled expected execution"], ["modeled-conservative","Modeled conservative execution"],
  ["execution-penalty","Execution-penalty sensitivity"],
];
...
export function OptionsBacktester() {
  ...
  const [resultTrack, setResultTrack] = useState<ResultTrack>("reference");
  ...
  async function saveResearchSelections(requestedDraft:ReadonlySet<string>=draftSelectionIds){
    ...
    try{
      ...
      const response=await fetch(`/__local/research-selections/${encodeURIComponent(dataset.datasetId)}/events/${encodeURIComponent(selectedEvent.id)}`,{method:"PUT",headers:{"Content-Type":"application/json","If-Match":selectionStore.updatedAtUtc},body:JSON.stringify(eventRecord)});responseReceived=true;const raw=await response.json().catch(()=>({}));if(!response.ok){const context=response.status===413?"Payload too large after normalization":response.status===409?"Conflict: reload selections and retry":response.status===400?"Validation failed":"Persistence failed";throw new Error(`${context}. ${raw.details?.map?.((e:{path:string;message:string})=>`${e.path}: ${e.message}`).join(" | ")??raw.error??"Research selections were not saved."}`)}const checked=validateResearchSelectionStore(raw.store??raw);if(!checked.ok)throw new Error("Server returned an invalid selection store.");setSelectionStore(checked.store);setDraftSelection({eventId:selectedEvent.id,ids:new Set(draftAtStart)});setSelectionStatus(`Saved ${toAdd.size} new structure${toAdd.size===1?"":"s"}, retained ${toKeep.size}, removed ${toRemove.size}.`);
    }catch(error){setDraftSelection({eventId:selectedEvent.id,ids:draftAtStart});if(persistenceEndpointUnavailable(error,responseReceived)){setSelectionPersistenceAvailable(false);setSelectionStatus(`${LOCAL_PERSISTENCE_REQUIRED_MESSAGE} Unsaved selections remain selected in this session.`);}else setSelectionStatus(`${error instanceof Error?error.message:"Research selection save failed."} Save failed; no changes were persisted.`);}
  }
  ...
  setHideRed(checked);
        ...
        <section className="workspace-section pnl-block" data-testid="pnl-block" ref={node => { sectionRefs.current.analysis = node; }}>
          ...
          <div className="results-workspace-heading"><div><p className="eyebrow">Primary results</p><h3>Economically valued structures</h3></div><label className="track-selector"><span>Viewing track</span><select value={resultTrack} onChange={event=>setResultTrack(event.target.value as ResultTrack)} aria-label="Viewing track">{RESULT_TRACKS.map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label></div>
          <div className="filter-row"><strong className="current-track">Track · {RESULT_TRACKS.find(([value])=>value===resultTrack)?.[1]}</strong><button className="secondary-button unavailable-button" aria-expanded={showUnavailable} onClick={()=>setShowUnavailable(value=>!value)}>{showUnavailable?"Hide data-unavailable":`Show data-unavailable · ${unavailableResults.length}`}</button><span>{filteredResults.length} / {analysisResults.length} visible</span></div>
          ...
          {pricingAssumption === "research-estimate" && <div className="table-card card"><div className="table-scroll"><table className="pnl-results-table"><thead><tr><th>Research</th><th><span className="sr-only">Expand</span></th><th>Status</th><th>Spread and strikes</th><th>Expiry</th><th>Width</th><th>Gross entry</th><th>Net opening</th><th>Best PnL</th><th>Worst PnL</th><th>Outcome</th></tr></thead><tbody>
            {filteredResults.map(result => {
              const selectedScenario=resultTrack==="reference"?{entry:result.researchEntry,path:result.researchPath,outcomes:result.researchOutcomes}:resultTrack==="immediate-maker"&&result.researchLayers.maker.status==="available"?result.researchByMode.maker:resultTrack==="immediate-taker"&&result.researchLayers.taker.status==="available"?result.researchByMode.taker:undefined;
              ...
              const trackStatus=resultTrack==="reference"?`FV ${estimate?"✓":"—"}`:resultTrack==="immediate-maker"?`MKR now ${estimate?"✓":"—"}`:resultTrack==="immediate-taker"?`TKR now ${estimate?"✓":"—"}`:resultTrack.startsWith("delayed")?"Delayed 4h —":resultTrack.startsWith("modeled")?"Model —":"Sensitivity —";
