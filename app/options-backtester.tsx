...
import { candidateIdentity, canonicalJson, renameResearchSelectionEvent, canSelectResearchCandidate, compactCandidateMetadata, compactEntryEconomics, compactMarginResult, compactOutcomeSnapshot, compactValuationPoint, eventReference, reconcileGeneratedSelection, sameSelectionIds, selectionChangeSet, stableCandidateId, stableSelectionId, validateResearchSelectionStore, type EvidenceTradeDto, type EvidenceUsageDto, type ExecutionScenarioSnapshot, type GenerationCandidateSnapshot, type ResearchSelectionEvent, type ResearchSelectionStore, type SelectedStructure } from "./lib/research-selections";
  ...
  async function saveResearchSelections(requestedDraft:ReadonlySet<string>=draftSelectionIds){
    ...
    try{
      ...
      for(const candidateId of toAdd){
        ...
        if(result.researchByMode.maker.entry.status!=="priced"&&result.researchByMode.taker.entry.status!=="priced"){
          const spread=result.spread,maker=result.researchByMode.maker.entry,taker=result.researchByMode.taker.entry;
          throw new Error(`Cannot add ${candidateId} (expiry ${formatExpiryWithFriday(spread.expiryTimestamp,spread.expiryLabel)}, strikes ${spread.resolvedSoldStrike??spread.soldStrike}/${spread.resolvedBoughtStrike??spread.boughtStrike}): maker unavailable (${maker.status==="unavailable"?maker.reason:"not evaluated"}); taker unavailable (${taker.status==="unavailable"?taker.reason:"not evaluated"}).`);
        }
      }
  ...
  async function runBacktest() {
    ...
    try {
      ...
      const next: AnalysisResult[] = observations.map(observation => {
        ...
        return { eventPrice:selectedEvent.entryPrice, observation, spread, path, exits, selectedExit, eventQuality, researchByMode, researchEntry:active.entry, researchPath:active.path, researchOutcomes:active.outcomes, scenarioInput: { event: selectedEvent, candidates: canonicalRetrievedSpreads, candles, config } };
      });
