...
import { buildExecutionCalibrationRecords, buildModeledExecution, MODELED_EXECUTION_VERSION } from "./lib/modeled-execution";
...
export function OptionsBacktester() {
  ...
  setHideRed(checked);
        ...
        <section className="workspace-section pnl-block" data-testid="pnl-block" ref={node => { sectionRefs.current.analysis = node; }}>
          <div className="section-heading"><div><span className="step-number">04</span><p className="eyebrow">Path-aware output</p><h2>PnL, exits & trust filters</h2></div><div className="export-controls"><span className={staleDerivedCount?"dirty-state":"flag flag-green"}>{staleDerivedCount?`Stale derived research · ${staleDerivedCount}`:"Current"}</span><button className="secondary-button" disabled={!analysisResults.length} onClick={exportResults}>Export JSON</button><button className="secondary-button" disabled={!persistedSelectionCount||!selectionPersistenceAvailable} onClick={()=>void exportResearchBundle()}>Export research bundle · {persistedSelectionCount}</button></div></div><p className="fine-print">The research bundle contains saved selections across the current dataset and their complete generated denominator. Checked-but-unsaved rows are never exported. {staleDerivedCount?"Research data stale — recompute recommended. Export preserves the saved snapshots without claiming they were evaluated.":"Saved derived research uses the current engine versions."}</p>
