...
import { MODEL_TOOLTIP, buildEstimatedPath, buildResearchExport, buildResearchOutcomes, evaluateResearchEntryLayers, formatExpiryWithFriday, scaleResearchEstimate, scaleResearchPath, validateResearchAmount, openingUsdEquivalent, type EstimatedOutcome, type EstimatedPathPoint, type PricingAssumption, type ResearchValuation, type ResearchEntryLayers } from "./lib/research-valuation";
...
import { candidateIdentity, canonicalJson, renameResearchSelectionEvent, compactCandidateMetadata, compactEntryEconomics, compactMarginResult, compactOutcomeSnapshot, compactValuationPoint, eventReference, reconcileGeneratedSelection, sameSelectionIds, selectionChangeSet, stableCandidateId, stableSelectionId, validateResearchSelectionStore, type EvidenceTradeDto, type EvidenceUsageDto, type ExecutionScenarioSnapshot, type GenerationCandidateSnapshot, type ResearchSelectionEvent, type ResearchSelectionStore, type SelectedStructure } from "./lib/research-selections";
...
export function OptionsBacktester() {
  ...
  const [resultsFilter, setResultsFilter] = useState<"all" | "model" | "maker" | "taker" | "trusted" | "green">("all");
  ...
  const filteredResults = analysisResults.filter(result => {
    if (resultsFilter === "model") return result.researchLayers.model.status==="priced";
    if (resultsFilter === "maker") return result.researchLayers.maker.status==="available";
    if (resultsFilter === "taker") return result.researchLayers.taker.status==="available";
  ...
  const selectionDirty=!sameSelectionIds(savedSelectionIds,draftSelectionIds);
  ...
  async function saveResearchSelections(requestedDraft:ReadonlySet<string>=draftSelectionIds){
    ...
    try{
      if(!draftAtStart.size){
        const response=await fetch(`/__local/research-selections/${encodeURIComponent(dataset.datasetId)}/events/${encodeURIComponent(selectedEvent.id)}`,{method:"DELETE",headers:{"If-Match":selectionStore.updatedAtUtc}});
        const raw=await response.json().catch(()=>({}));
        if(!response.ok)throw new Error(raw.error??"Research selections could not be cleared.");
        const checked=validateResearchSelectionStore(raw.store??raw);if(!checked.ok)throw new Error("Server returned an invalid selection store.");
        setSelectionStore(checked.store);setDraftSelection({eventId:selectedEvent.id,ids:new Set()});
        setSelectionStatus(`Cleared ${toRemove.size} saved structures for this event. 0 remain selected.`);return;
      }
      ...
      const generationSnapshot=analysisResults.length?{generatedAtUtc:now,configuration:{applicationBuild:(import.meta as ImportMeta & {env?:Record<string,string>}).env?.VITE_APP_COMMIT??null,pricingEngineVersion:"research-estimate-v1",qualityRulesVersion:"entry-liquidity-v1",feeScheduleVersion:"deribit-standard-inverse-v1",dteWindows:canonicalJson(dteTolerances),expirySelectionMode,executionMode,pricingAssumption,pricingTracks:[...pricingModes],historicalEvidenceWindows:canonicalJson({entryMinutes:[...new Set(analysisResults.flatMap(result=>result.researchEntry.status==="priced"?[result.researchEntry.evidenceWindowMinutes]:[]))]}),synchronizationThresholds:canonicalJson({milliseconds:60_000}),qualityThresholds:canonicalJson({source:"entry-liquidity-v1"}),feeAssumptions:canonicalJson({tier:"standard",route:comboExecution?"combo":"legs"}),settlementRules:canonicalJson({source:"deribit-delivery-price",fallback:"unavailable"}),valuationInterval:"4h",modelAssumptions:canonicalJson({model:"Black-Scholes reconstructed IV",rate:0}),generatedAtUtc:now},candidates:[...currentGeneration,...retainedStale],underlyingHourlyPath:candles}:savedEventSelection!.generationSnapshot;
      const eventRecord:ResearchSelectionEvent={eventId:selectedEvent.id,sourceRun:savedEventSelection?.sourceRun??canonicalJson({event:eventReference(selectedEvent),savedAtUtc:now}),generationSnapshot,selectedStructures,evidenceCatalog:[...eventEvidenceCatalog.values()].sort((a,b)=>a.evidenceId.localeCompare(b.evidenceId))};
    ...
    }catch(error){setDraftSelection({eventId:selectedEvent.id,ids:draftAtStart});setSelectionStatus(`${error instanceof Error?error.message:"Research selection save failed."} Save failed; no changes were persisted.`);}
  }
  ...
  async function runBacktest() {
    ...
    try {
      ...
      const next: AnalysisResult[] = observations.map(observation => {
        ...
        return { eventPrice:selectedEvent.entryPrice, observation, spread, path, exits, selectedExit, eventQuality, researchByMode, researchLayers, researchEntry:modelEntry, researchPath:modelPath, researchOutcomes:modelOutcomes, scenarioInput: { event: selectedEvent, candidates: canonicalRetrievedSpreads, candles, config } };
      });
  ...
  setHideRed(checked);
        ...
        <section className="workspace-section pnl-block" data-testid="pnl-block" ref={node => { sectionRefs.current.analysis = node; }}>
          ...
          <div className="filter-row"><div className="segmented">{([['all','All tests'],['model','Model priced'],['maker','Maker available'],['taker','Taker available'],['trusted','Green / yellow'],['green','Green only']] as const).map(([value,label]) => <button className={resultsFilter === value ? "active" : ""} onClick={() => setResultsFilter(value)} key={value}>{label}</button>)}</div><span>{filteredResults.length} / {analysisResults.length} results visible</span></div>
          {pricingAssumption === "research-estimate" && analysisResults.length>0 && <div className="source-strip" aria-label="Independent research status counts"><span><small>Generated variants</small><strong>{analysisResults.length}</strong></span><span><small>Exact-contract resolved</small><strong>{analysisResults.filter(r=>r.researchLayers.structural.status==="resolved").length}</strong></span><span><small>Exact-contract unresolved</small><strong>{analysisResults.filter(r=>r.researchLayers.structural.status==="unresolved").length}</strong></span><span><small>Model priced</small><strong>{analysisResults.filter(r=>r.researchLayers.model.status==="priced").length}</strong></span><span><small>Maker opportunities available</small><strong>{analysisResults.filter(r=>r.researchLayers.maker.status==="available").length}</strong></span><span><small>Taker executions available</small><strong>{analysisResults.filter(r=>r.researchLayers.taker.status==="available").length}</strong></span><span><small>Maker raw / no-trade</small><strong>{analysisResults.filter(r=>r.researchLayers.maker.rawVwapStatus==="priced").length} / {analysisResults.filter(r=>r.researchLayers.maker.rawVwapStatus!=="priced").length}</strong></span><span><small>Taker raw / no-trade</small><strong>{analysisResults.filter(r=>r.researchLayers.taker.rawVwapStatus==="priced").length} / {analysisResults.filter(r=>r.researchLayers.taker.rawVwapStatus!=="priced").length}</strong></span></div>}
          ...
          {pricingAssumption === "research-estimate" && <div className="table-card card"><div className="table-scroll"><table className="pnl-results-table"><thead><tr><th>Research</th><th><span className="sr-only">Expand</span></th><th>Exact contracts</th><th>Model valuation</th><th>Maker opportunity</th><th>Taker execution</th><th>Spread and both strikes</th><th>Expiry</th><th>Width</th><th>Estimated gross entry</th><th>Estimated net opening</th><th>Best estimated unrealized PnL</th><th>Worst Observed PnL</th><th>Estimated selected outcome</th></tr></thead><tbody>
            {filteredResults.map(result => {
              const researchPnls=result.researchPath.map(researchPnlUsd).filter((value):value is number=>value!==undefined),researchBest=Math.max(...researchPnls),pathStats=researchPathStatistics(result.researchPath,result.researchEntry.targetTimestamp),researchWorst=pathStats.worstObservedPnlUsd??Infinity; const estimate=result.researchEntry.status==="priced"?result.researchEntry:undefined;
              ...
              return <Fragment key={result.spread.id}><tr className={selectedAnalysis?.spread.id === result.spread.id ? "row-selected" : ""} onClick={() => selectAnalysisResult(result.spread.id)}><td><label className="research-select" onClick={event=>event.stopPropagation()}><input type="checkbox" checked={draftSelectionIds.has(result.spread.id)} disabled={!draftSelectionIds.has(result.spread.id)&&result.researchLayers.model.status!=="priced"&&result.researchLayers.maker.status!=="available"&&result.researchLayers.taker.status!=="available"} aria-label={`Select ${result.spread.structure} for research`} onChange={event=>toggleResearchSelection(result.spread.id,event.target.checked)}/>{savedSelectionIds.has(result.spread.id)&&draftSelectionIds.has(result.spread.id)&&<small>SAVED</small>}</label></td><td><button className="expand-button" aria-expanded={expanded} aria-controls={`ledger-${result.spread.id}`} aria-label={`${expanded ? "Collapse" : "Expand"} opening ledgers for ${result.spread.structure}`} onClick={event => { event.stopPropagation(); setExpandedResultIds(current => expanded ? current.filter(id => id !== result.spread.id) : [...current, result.spread.id]); }}><span className="expand-chevron" aria-hidden="true" /></button></td><td><strong>{result.researchLayers.structural.status}</strong><small>{result.researchLayers.structural.reason}</small></td><td><strong>{result.researchLayers.model.status}</strong><small>{result.researchLayers.model.reason}</small></td><td><strong>{result.researchLayers.maker.status}</strong><small>{result.researchLayers.maker.reason}</small><details><summary>Evidence</summary><small className="mono">{result.researchLayers.maker.shortDirection} / {result.researchLayers.maker.longDirection} · requested {result.researchLayers.maker.requestedAmount} · qualifying {result.researchLayers.maker.shortQualifyingAmount} / {result.researchLayers.maker.longQualifyingAmount} · {formatUtc(result.researchLayers.maker.orderTimestamp)}–{formatUtc(result.researchLayers.maker.fillWindowEnd)} · synchronized {String(result.researchLayers.maker.synchronized)}</small></details></td><td><strong>{result.researchLayers.taker.status}</strong><small>{result.researchLayers.taker.reason}</small><details><summary>Evidence</summary><small className="mono">{result.researchLayers.taker.shortDirection} / {result.researchLayers.taker.longDirection} · requested {result.researchLayers.taker.requestedAmount} · qualifying {result.researchLayers.taker.shortQualifyingAmount} / {result.researchLayers.taker.longQualifyingAmount} · {formatUtc(result.researchLayers.taker.orderTimestamp)}–{formatUtc(result.researchLayers.taker.fillWindowEnd)} · synchronized {String(result.researchLayers.taker.synchronized)}</small></details></td><td><strong>{result.spread.structure}</strong><small className="mono">{spreadIdentity(result.spread).soldStrike} / {spreadIdentity(result.spread).boughtStrike} {result.spread.optionType}</small></td><td>{formatExpiryWithFriday(result.spread.expiryTimestamp,result.spread.expiryLabel)}</td><td>{money(result.spread.actualWidth)}</td><td>{openingDisplay(estimate?.grossSpreadBtc,estimate?.entryTargetIndex,openingCurrency)}</td><td>{openingDisplay(estimate?.netOpeningCashFlowBtc,estimate?.entryTargetIndex,openingCurrency)}</td><td className="positive">{researchBest===-Infinity?"—":money(researchBest)}</td><td className="negative">{researchWorst===Infinity?"—":money(researchWorst)}</td><td>Estimated valuation</td></tr>{expanded&&estimate&&payoffInput&&<tr className="ledger-detail-row"><td colSpan={14}><div id={`ledger-${result.spread.id}`} className="ledger-pair"><section className="opening-ledger"><h4>Research estimate evidence</h4><p>{estimate.disclaimer}</p><dl><div><dt>Contracts</dt><dd className="mono">{estimate.sold.instrumentName} / {estimate.bought.instrumentName}</dd></div><div><dt>Evidence window</dt><dd>±{estimate.evidenceWindowMinutes} minutes</dd></div><div><dt>Leg prices</dt><dd>{btc(estimate.sold.priceBtcPerContract)} / {btc(estimate.bought.priceBtcPerContract)}</dd></div><div><dt>Source / quality</dt><dd>{estimate.priceSource} · {estimate.estimateQuality}</dd></div><div><dt>Fees</dt><dd>{openingDisplay(estimate.openingFeesBtc,estimate.entryTargetIndex,openingCurrency)}</dd></div></dl></section><ExpiryPayoffChart input={payoffInput}/></div></td></tr>}</Fragment>;
            })}
            {!filteredResults.length && <tr><td colSpan={14} className="empty-cell">Run the backtest after importing eligible contract histories. Red tests remain visible in “All tests.”</td></tr>}
          </tbody></table></div>{staleSelections.size>0&&<section className="card"><h3>Saved but no longer generated</h3><p className="fine-print">Retained stale snapshots remain in exports only while explicitly selected here. IDs are never remapped.</p>{[...staleSelections].map(id=><label className="research-select" key={id}><input type="checkbox" checked={draftSelectionIds.has(id)} onChange={event=>toggleResearchSelection(id,event.target.checked)}/><span className="mono">{id}</span>{draftSelectionIds.has(id)&&<small>SAVED · absent from current generation</small>}</label>)}</section>}<div className="research-save-bar"><strong>{draftSelectionIds.size} structures selected for this event</strong>{selectionDirty&&<span className="dirty-state">Unsaved selection changes</span>}<button className="secondary-button" disabled={!savedSelectionIds.size||!selectionPersistenceAvailable} onClick={()=>void clearSavedSelection()}>Clear saved selection</button><button className="primary-button" disabled={!selectionDirty||!selectionPersistenceAvailable} onClick={()=>void saveResearchSelections()}>Save research state</button></div>{selectionStatus&&<p className="research-export-status" role="status" aria-live="polite">{selectionStatus}</p>}</div>}
