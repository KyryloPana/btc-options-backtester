...
import { CHART_GEOMETRY, CHART_SERIES, RESEARCH_CHART_SERIES, adaptResearchPath, hitExitGroups, nearestPoint, rawResearchObservations, researchOutcomeGroups, researchPnlUsd, splitSeriesAtMissing, timestampAtX, timeX, valuationChartTitle, visibleMatrixSpreads, type ChartMetric, type ChartSeriesKey, type ResearchChartMetric } from "./lib/valuation-chart";
import { MODEL_TOOLTIP, buildEstimatedPath, buildResearchExport, buildResearchOutcomes, estimateResearchSpread, formatExpiryWithFriday, scaleResearchEstimate, scaleResearchPath, validateResearchAmount, openingUsdEquivalent, type EstimatedOutcome, type EstimatedPathPoint, type PricingAssumption, type ResearchValuation } from "./lib/research-valuation";
...
function ExpiryPayoffChart({input}:{input:ExpiryPayoffInput}){
  ...
  const be=breakEven(input,currency),extrema=payoffExtrema(input,currency),width=Math.abs(input.shortStrike-input.longStrike);
  ...
  return <section className="expiry-payoff"><div className="card-title-row"><div><p className="eyebrow">Theoretical payoff at expiration</p><h4>Credit-spread expiration profile</h4></div><div className="segmented" aria-label="Payoff currency"><button className={currency==="usd-cash-flow"?"active":""} onClick={()=>setCurrency("usd-cash-flow")}>USD cash-flow</button><button className={currency==="btc-settlement"?"active":""} onClick={()=>setCurrency("btc-settlement")}>BTC settlement</button></div></div><p className="fine-print">Theoretical expiration payoff based on the estimated entry. It is not proof of execution.</p>
    ...
    <div className="payoff-inspector" aria-live="polite"><span><small>Selected point</small><strong>{selected?`${money(selected.expirationIndex)} · ${currency==="usd-cash-flow"?openingUsd.format(selected.pnl):btc(selected.pnl)}`:"Hover or use arrow keys"}</strong></span><span><small>Maximum profit / loss</small><strong>{currency==="usd-cash-flow"?`${openingUsd.format(extrema.maximumProfit)} / ${openingUsd.format(extrema.maximumLoss)}`:`${btc(extrema.maximumProfit)} / ${btc(extrema.maximumLoss)}`}</strong></span><span><small>Break-even</small><strong>{be?`${money(be.index)} · ${be.method}`:"No crossing"}</strong></span><span><small>Amount</small><strong>{input.amount} contracts</strong></span><span><small>Gross / net entry credit</small><strong>{btc((input.shortEntryPremiumBtc-input.longEntryPremiumBtc)*input.amount)} / {btc((input.shortEntryPremiumBtc-input.longEntryPremiumBtc)*input.amount-input.openingFeesBtc)}</strong></span></div>
  </section>;
}
...
function ResearchDetail({result,amount,onAmountChange,openingCurrency,compareExecutionModes}: {result:AnalysisResult;amount:string;onAmountChange:(value:string)=>void;openingCurrency:"btc"|"usd-entry";compareExecutionModes:boolean}) {
 ...
 const values=points.map(point=>researchPnlUsd(point)!); const best=values.length?Math.max(...values):undefined,worst=values.length?Math.min(...values):undefined;
...
export function OptionsBacktester() {
  ...
  const [selectionToggles,setSelectionToggles]=useState<Map<string,boolean>>(new Map());
  ...
  const retrievedSpreads = useMemo(
    () => buildExpiryCandidates(desiredSpreads, candidateManifests, effectiveEntryTimestamp, selectedEvent.entryPrice, inventory, "taker", expirySelectionMode, pricingAssumption).map(spread=>{const venue="deribit" as const;const identified={...spread,venue};return{...identified,id:stableCandidateId(candidateIdentity(dataset?.datasetId??"unloaded",selectedEvent.id,identified))}}),
    [desiredSpreads, candidateManifests, effectiveEntryTimestamp, selectedEvent.entryPrice, inventory, expirySelectionMode, pricingAssumption, dataset?.datasetId, selectedEvent.id],
  );
  ...
  const availabilityRows=analysisResults.map(result=>({id:result.spread.id,status:result.researchEntry.status,estimateQuality:result.researchEntry.status==="priced"?result.researchEntry.estimateQuality:undefined,reason:result.researchEntry.status==="unavailable"?result.researchEntry.reason:undefined}));
  const pricedResultIds=new Set(pricedStructures(availabilityRows).map(row=>row.id));
  const selectedAnalysis = analysisResults.find(result => result.spread.id === selectedResultId && (pricingAssumption!=="research-estimate"||pricedResultIds.has(result.spread.id)));
  ...
  const filteredResults = analysisResults.filter(result => {
    if(pricingAssumption==="research-estimate"&&result.researchEntry.status!=="priced")return false;
  ...
  const coverage=structureCoverage(availabilityRows);
  ...
  const activeSelectionIds=reconcileSelectionIds(savedSelectionIds,selectionToggles);
  const selectionDirty=selectionToggles.size>0;
  ...
  function clearEventAnalysis() { setAnalysisResults([]); setCandidateManifests([]); setInventory([]); setSelectedSpreadId(undefined); setSelectedResultId(undefined); setExpandedResultIds([]); setScenario(undefined); }

  async function loadResearchSelections(datasetId:string){try{const response=await fetch(`/__local/research-selections/${encodeURIComponent(datasetId)}`);const raw=await response.json();if(!response.ok)throw new Error(raw.error??"Research selections could not be loaded.");const checked=validateResearchSelectionStore(raw);if(!checked.ok)throw new Error(checked.errors.map(e=>`${e.path}: ${e.message}`).join(" | "));setSelectionStore(checked.store);setSelectionPersistenceAvailable(true);setSelectionStatus("");}catch(error){setSelectionStore(undefined);setSelectionPersistenceAvailable(false);setSelectionStatus(`${error instanceof Error?error.message:"Persistent research selections unavailable."} Static hosting cannot save research selections; run the local application server.`);}setSelectionToggles(new Map());}
  ...
  async function loadDataset(id:string, guard=true) { if (guard && !canLeaveDirty(datasetDirty||selectionDirty,()=>window.confirm("Discard unsaved dataset or research-selection changes and switch datasets?"))) return; const response=await fetch(`/__local/trade-datasets/${encodeURIComponent(id)}`); const payload=await response.json(); if(!response.ok) throw new Error(payload.error??"Dataset could not be loaded."); const checked=validateTradeDataset(payload); if(!checked.ok) throw new Error(checked.errors.map(e=>`${e.tradeId??"dataset"} · ${e.path}: ${e.message}`).join(" | ")); const loaded=checked.dataset; setDataset(loaded); setEvents(loaded.trades); setSelectedEventId(loaded.trades[0]?.id??""); setDatasetDirty(false); clearEventAnalysis(); await loadResearchSelections(id); window.localStorage.setItem("options-lab-trade-dataset",id); setDatasetStatus(""); }

  function toggleResearchSelection(candidateId:string,selected:boolean){setSelectionToggles(current=>{const next=new Map(current);if(savedSelectionIds.has(candidateId)===selected)next.delete(candidateId);else next.set(candidateId,selected);return next;});}
  function switchEvent(eventId:string){if(eventId===selectedEvent.id)return;if(!canLeaveDirty(selectionDirty,()=>window.confirm("Discard unsaved research-selection changes and switch events?")))return;setSelectionToggles(new Map());setSelectedEventId(eventId);clearEventAnalysis();}
  ...
  function buildScenarioSnapshot(candidateId:string,mode:ExecutionMode,scenario:ResearchScenario,usages:EvidenceUsageDto[],catalog:Map<string,EvidenceTradeDto>):ExecutionScenarioSnapshot{
    if(scenario.entry.status!=="priced")return{status:"not_evaluated",reason:scenario.entry.reason,entrySnapshot:null,valuationPathSnapshot:[],outcomeSnapshots:[]};
  ...
  async function saveResearchSelections(){if(!dataset||!selectionStore||!selectionPersistenceAvailable||(!selectionDirty&&!analysisResults.length))return;try{const now=new Date().toISOString(),finalIds=reconcileSelectionIds(savedSelectionIds,selectionToggles),eventEvidenceCatalog=new Map<string,EvidenceTradeDto>();const byId=new Map(analysisResults.map(result=>[result.spread.id,result]));const prior=new Map((savedEventSelection?.selectedStructures??[]).map(item=>[item.candidateId,item]));const selectedStructures:SelectedStructure[]=[...finalIds].sort().map(candidateId=>{const result=byId.get(candidateId);if(!result)return prior.get(candidateId)!;if(result.researchByMode.maker.entry.status!=="priced"&&result.researchByMode.taker.entry.status!=="priced")throw new Error("Neither maker opportunity nor taker execution could be evaluated for this structure.");const spread=result.spread,usages:EvidenceUsageDto[]=[];const executionScenarios={maker:buildScenarioSnapshot(candidateId,"maker",result.researchByMode.maker,usages,eventEvidenceCatalog),taker:buildScenarioSnapshot(candidateId,"taker",result.researchByMode.taker,usages,eventEvidenceCatalog)};return{selectionId:stableSelectionId(selectedEvent.id,"deribit",candidateId),eventId:selectedEvent.id,candidateId,venue:"deribit",selectedAtUtc:now,quantity:(result.researchByMode.maker.entry.status==="priced"?result.researchByMode.maker.entry:result.researchByMode.taker.entry as {amount:number}).amount,candidateSnapshot:compactCandidateMetadata({venue:"deribit",contractStyle:"inverse BTC option",structure:spread.structure,spreadKind:spread.spreadKind,optionType:spread.optionType,targetDte:spread.targetDte,expiryTimestamp:spread.expiryTimestamp??null,actualDte:spread.actualDte??null,shortStrike:spread.resolvedSoldStrike??spread.soldStrike,longStrike:spread.resolvedBoughtStrike??spread.boughtStrike,actualWidth:spread.actualWidth??null,instruments:{short:spread.soldContract?.instrumentName??null,long:spread.boughtContract?.instrumentName??null},quality:result.eventQuality,qualityReasonCodes:[]}),executionScenarios,marginSnapshot:compactMarginResult(result.observation.marginResult??{status:"unavailable",reason:"No margin result was produced."}),evidenceTradeSnapshots:[],evidenceUsages:usages};});
    const generationCandidates:GenerationCandidateSnapshot[]=retrievedSpreads.map(spread=>{const result=byId.get(spread.id),priced=result?.researchEntry.status==="priced";return{candidateId:spread.id,venue:"deribit",selected:finalIds.has(spread.id),status:priced?"priced":"unavailable",availabilityReasons:priced?[]:[result?.researchEntry.status==="unavailable"?result.researchEntry.reason:spread.retrievalNote],targetHorizon:spread.targetDte,eligibleDteRange:{min:spread.dteMin??null,max:spread.dteMax??null},actualExpiryTimestamp:spread.expiryTimestamp??null,actualDte:spread.actualDte??null,requestedStrikes:{short:spread.soldStrike,long:spread.boughtStrike,width:spread.targetWidth},actualStrikes:{short:spread.resolvedSoldStrike??null,long:spread.resolvedBoughtStrike??null,width:spread.actualWidth??null},structure:spread.structure,optionType:spread.optionType,strikeMethod:spread.buffered?"buffered":"anchor",entryQuality:result?.eventQuality??spread.entryLiquidityQuality??null};});
    const candles=analysisResults[0]?.scenarioInput.candles??[];const generationSnapshot={generatedAtUtc:now,configuration:{applicationBuild:(import.meta as ImportMeta & {env?:Record<string,string>}).env?.VITE_APP_COMMIT??null,pricingEngineVersion:"research-estimate-v1",qualityRulesVersion:"entry-liquidity-v1",feeScheduleVersion:"deribit-standard-inverse-v1",dteWindows:canonicalJson(dteTolerances),expirySelectionMode,executionMode,pricingAssumption,pricingTracks:[...pricingModes],historicalEvidenceWindows:canonicalJson({entryMinutes:[...new Set(analysisResults.flatMap(result=>result.researchEntry.status==="priced"?[result.researchEntry.evidenceWindowMinutes]:[]))]}),synchronizationThresholds:canonicalJson({milliseconds:60_000}),qualityThresholds:canonicalJson({source:"entry-liquidity-v1"}),feeAssumptions:canonicalJson({tier:"standard",route:comboExecution?"combo":"legs"}),settlementRules:canonicalJson({source:"deribit-delivery-price",fallback:"unavailable"}),valuationInterval:"4h",modelAssumptions:canonicalJson({model:"Black-Scholes reconstructed IV",rate:0}),generatedAtUtc:now},candidates:generationCandidates,underlyingHourlyPath:candles};const eventRecord:ResearchSelectionEvent={eventId:selectedEvent.id,sourceRun:canonicalJson({event:eventReference(selectedEvent),savedAtUtc:now}),generationSnapshot,selectedStructures,evidenceCatalog:[...eventEvidenceCatalog.values()].sort((a,b)=>a.evidenceId.localeCompare(b.evidenceId))};const response=await fetch(`/__local/research-selections/${encodeURIComponent(dataset.datasetId)}/events/${encodeURIComponent(selectedEvent.id)}`,{method:"PUT",headers:{"Content-Type":"application/json","If-Match":selectionStore.updatedAtUtc},body:JSON.stringify(eventRecord)});const raw=await response.json().catch(()=>({}));if(!response.ok){const context=response.status===413?"Payload too large after normalization":response.status===409?"Conflict: reload selections and retry":response.status===400?"Validation failed":"Persistence failed";throw new Error(`${context}. ${raw.details?.map?.((e:{path:string;message:string})=>`${e.path}: ${e.message}`).join(" | ")??raw.error??"Research selections were not saved."}`)}const checked=validateResearchSelectionStore(raw.store??raw);if(!checked.ok)throw new Error("Server returned an invalid selection store.");setSelectionStore(checked.store);setSelectionToggles(new Map());setSelectionStatus(`Saved ${eventRecord.selectedStructures.length} structures for ${selectedEvent.id} (${new TextEncoder().encode(JSON.stringify(eventRecord)).byteLength} bytes).`);}catch(error){setSelectionStatus(error instanceof Error?error.message:"Research selection save failed; selections remain unsaved and recoverable.");}}
  ...
  async function runBacktest() {
    ...
    try {
      ...
      const next: AnalysisResult[] = observations.map(observation => {
        ...
        const evaluateScenario=(mode:ExecutionMode):ResearchScenario=>{
          const entry=estimateResearchSpread({spread,targetTimestamp:entryTimestamp,targetIndex:selectedEvent.entryPrice,amount,slippageBps:observation.strategyConfiguration.slippageBps,executionRoute:observation.strategyConfiguration.executionRoute,executionMode:mode});
          const path=entry.status==="priced"?buildEstimatedPath({spread,timestamps:grid,candles,entry,slippageBps:observation.strategyConfiguration.slippageBps,executionRoute:observation.strategyConfiguration.executionRoute}):[];
          const outcomes=entry.status==="priced"?buildResearchOutcomes({event:selectedEvent,spread,entry,path,candles}):[];
          return {entry,path,outcomes};
        };
        const researchByMode={maker:evaluateScenario("maker"),taker:evaluateScenario("taker")};
        const active=researchByMode[executionMode];
        const eventQuality: QualityFlag = active.entry.status === "priced" ? active.entry.estimateQuality : "red";
        const config = observation.strategyConfiguration;
        return { eventPrice:selectedEvent.entryPrice, observation, spread, path, exits, selectedExit, eventQuality, researchByMode, researchEntry:active.entry, researchPath:active.path, researchOutcomes:active.outcomes, scenarioInput: { event: selectedEvent, candidates: retrievedSpreads, candles, config } };
      });
  ...
  setHideRed(checked);
        ...
        <section className="workspace-section pnl-block" data-testid="pnl-block" ref={node => { sectionRefs.current.analysis = node; }}>
          <div className="section-heading"><div><span className="step-number">04</span><p className="eyebrow">Path-aware output</p><h2>PnL, exits & trust filters</h2></div><div className="export-controls"><button className="secondary-button" disabled={!analysisResults.length} onClick={exportResults}>Export JSON</button><button className="secondary-button" disabled={!persistedSelectionCount||!selectionPersistenceAvailable} onClick={()=>void exportResearchBundle()}>Export research bundle · {persistedSelectionCount}</button></div></div><p className="fine-print">The research bundle contains saved selections across the current dataset and their complete generated denominator. Checked-but-unsaved rows are never exported.</p>
          ...
          {pricingAssumption==="research-estimate"&&analysisResults.length>0&&<div className="coverage-line"><strong>{coverage.priced?`Priced structures: ${coverage.priced} / ${coverage.total} · Unavailable excluded: ${coverage.unavailable}`:`No structures could be priced. 0 / ${coverage.total} available.`}</strong>{coverage.unavailable>0&&<details><summary>Unavailable reasons ({coverage.unavailable})</summary>{Object.entries(coverage.reasons).map(([reason,count])=><span key={reason}>{reason}: <b>{count}</b></span>)}</details>}</div>}
          ...
          {pricingAssumption === "research-estimate" && <div className="table-card card"><div className="table-scroll"><table className="pnl-results-table"><thead><tr><th>Research</th><th><span className="sr-only">Expand</span></th><th>Estimate quality</th><th>Spread and both strikes</th><th>Expiry</th><th>Width</th><th>Estimated gross entry</th><th>Estimated net opening</th><th>Best estimated unrealized PnL</th><th>Max estimated adverse PnL</th><th>Estimated selected outcome</th></tr></thead><tbody>
            {filteredResults.map(result => {
              ...
              return <Fragment key={result.spread.id}><tr className={selectedAnalysis?.spread.id === result.spread.id ? "row-selected" : ""} onClick={() => selectAnalysisResult(result.spread.id)}><td><label className="research-select" onClick={event=>event.stopPropagation()}><input type="checkbox" checked={activeSelectionIds.has(result.spread.id)} disabled={!canSelectResearchCandidate(result.researchEntry.status)} aria-label={`Select ${result.spread.structure} for research`} onChange={event=>toggleResearchSelection(result.spread.id,event.target.checked)}/>{savedSelectionIds.has(result.spread.id)&&!selectionToggles.has(result.spread.id)&&<small>SAVED</small>}</label></td><td><button className="expand-button" aria-expanded={expanded} aria-controls={`ledger-${result.spread.id}`} aria-label={`${expanded ? "Collapse" : "Expand"} opening ledgers for ${result.spread.structure}`} onClick={event => { event.stopPropagation(); setExpandedResultIds(current => expanded ? current.filter(id => id !== result.spread.id) : [...current, result.spread.id]); }}><span className="expand-chevron" aria-hidden="true" /></button></td><td><span className={flagClass(estimate?.estimateQuality ?? "red")}>{estimate?.estimateQuality}</span><small>Confidence label, not execution proof</small></td><td><strong>{result.spread.structure}</strong><small className="mono">{spreadIdentity(result.spread).soldStrike} / {spreadIdentity(result.spread).boughtStrike} {result.spread.optionType}</small></td><td>{formatExpiryWithFriday(result.spread.expiryTimestamp,result.spread.expiryLabel)}</td><td>{money(result.spread.actualWidth)}</td><td>{openingDisplay(estimate?.grossSpreadBtc,estimate?.entryTargetIndex,openingCurrency)}</td><td>{openingDisplay(estimate?.netOpeningCashFlowBtc,estimate?.entryTargetIndex,openingCurrency)}</td><td className="positive">{researchBest===-Infinity?"—":money(researchBest)}</td><td className="negative">{researchWorst===Infinity?"—":money(researchWorst)}</td><td>Estimated valuation</td></tr>{expanded&&estimate&&payoffInput&&<tr className="ledger-detail-row"><td colSpan={11}><div id={`ledger-${result.spread.id}`} className="ledger-pair"><section className="opening-ledger"><h4>Research estimate evidence</h4><p>{estimate.disclaimer}</p><dl><div><dt>Contracts</dt><dd className="mono">{estimate.sold.instrumentName} / {estimate.bought.instrumentName}</dd></div><div><dt>Evidence window</dt><dd>±{estimate.evidenceWindowMinutes} minutes</dd></div><div><dt>Leg prices</dt><dd>{btc(estimate.sold.priceBtcPerContract)} / {btc(estimate.bought.priceBtcPerContract)}</dd></div><div><dt>Source / quality</dt><dd>{estimate.priceSource} · {estimate.estimateQuality}</dd></div><div><dt>Fees</dt><dd>{openingDisplay(estimate.openingFeesBtc,estimate.entryTargetIndex,openingCurrency)}</dd></div></dl></section><ExpiryPayoffChart input={payoffInput}/></div></td></tr>}</Fragment>;
            })}
            {!filteredResults.length && <tr><td colSpan={11} className="empty-cell">Run the backtest after importing eligible contract histories. Red tests remain visible in “All tests.”</td></tr>}
          </tbody></table></div><div className="research-save-bar"><strong>{activeSelectionIds.size} structures selected for this event</strong>{selectionDirty&&<span className="dirty-state">Unsaved selection changes</span>}<button className="primary-button" disabled={(!selectionDirty&&!analysisResults.length)||!selectionPersistenceAvailable} onClick={()=>void saveResearchSelections()}>Save research state</button></div>{selectionStatus&&<p className="research-export-status" role="status" aria-live="polite">{selectionStatus}</p>}{[...savedSelectionIds].filter(id=>!analysisResults.some(result=>result.spread.id===id)).map(id=><p className="fine-print" key={id}>SAVED · stale/unmatched candidate {id}</p>)}</div>}
