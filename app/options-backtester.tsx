...
export function OptionsBacktester() {
  ...
  setHideRed(checked);
        ...
        <section className="workspace-section pnl-block" data-testid="pnl-block" ref={node => { sectionRefs.current.analysis = node; }}>
          ...
          </tbody></table></div><div className="research-save-bar"><strong>{activeSelectionIds.size} structures selected for this event</strong>{selectionDirty&&<span className="dirty-state">Unsaved selection changes</span>}<button className="primary-button" disabled={!selectionDirty||!selectionPersistenceAvailable} onClick={()=>void saveResearchSelections()}>Save selected structures</button></div>{selectionStatus&&<p className="research-export-status" role="status" aria-live="polite">{selectionStatus}</p>}{[...savedSelectionIds].filter(id=>!analysisResults.some(result=>result.spread.id===id)).map(id=><p className="fine-print" key={id}>SAVED · stale/unmatched candidate {id}</p>)}</div>}
