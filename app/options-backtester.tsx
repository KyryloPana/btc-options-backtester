...
export function OptionsBacktester() {
  ...
  async function saveResearchSelections(requestedDraft:ReadonlySet<string>=draftSelectionIds,requestedExplicitDeselections:ReadonlySet<string>=explicitDeselections.eventId===selectedEvent.id?explicitDeselections.ids:new Set()){
    ...
    try{
      ...
      const comparativeIds=new Set(comparativeSource.structures.map(structure=>structure.candidateId));
      const researchStructures:ResearchOnlyStructure[]=[...shortStrikeResearchStructures.filter(structure=>!comparativeIds.has(structure.candidateId)&&structure.researchRole!=="comparative_economics"),...comparativeSource.structures].sort((a,b)=>a.candidateId.localeCompare(b.candidateId));
