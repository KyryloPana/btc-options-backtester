...
export function OptionsBacktester() {
  ...
  async function saveResearchSelections(requestedDraft:ReadonlySet<string>=draftSelectionIds,requestedExplicitDeselections:ReadonlySet<string>=explicitDeselections.eventId===selectedEvent.id?explicitDeselections.ids:new Set()){
    ...
    try{
      ...
      if(requestBytes>=RESEARCH_SELECTION_REQUEST_LIMIT_BYTES)throw new Error(`Research save blocked: compact event payload is ${(requestBytes/1_000_000).toFixed(1)} MB; ${(RESEARCH_SELECTION_REQUEST_LIMIT_BYTES/1_000_000).toFixed(1)} MB persistence limit. Payload normalization regression suspected.`);
