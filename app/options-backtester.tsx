...
import { candidateIdentity, canonicalJson, renameResearchSelectionEvent, compactCandidateMetadata, compactEntryEconomics, compactResearchSelectionEvent, compactMarginResult, compactOutcomeSnapshot, compactValuationPoint, eventReference, planCandidatePersistence, preserveControlledGenerationProvenance, reconcileGeneratedSelection, researchEventPayloadDiagnostics, stableCandidateId, stableSelectionId, unavailableResearchStructure, validateResearchSelectionStore, type EvidenceTradeDto, type EvidenceUsageDto, type ExecutionScenarioSnapshot, type GenerationCandidateSnapshot, type GenerationSnapshot, type ResearchOnlyStructure, type ResearchSelectionEvent, type ResearchSelectionStore, type SelectedStructure } from "./lib/research-selections";
...
import { EXPIRY_FORWARD_METHOD_VERSION } from "./lib/volatility/expiry-forward";
...
import { RESEARCH_SELECTION_REQUEST_LIMIT_BYTES } from "./lib/research-persistence-limits";
import { attemptControlled, controlledPersistence } from "./lib/controlled-research";
import { materializeComparativeEconomics } from "./lib/research-matrix";
...
export function OptionsBacktester() {
  ...
  const [explicitDeselections,setExplicitDeselections]=useState<{eventId:string;ids:Set<string>}>({eventId:"",ids:new Set()});
  ...
  function toggleResearchSelection(candidateId:string,selected:boolean){setDraftSelection(current=>{const next=current.eventId===selectedEvent.id?new Set(current.ids):new Set(savedSelectionIds);if(selected)next.add(candidateId);else next.delete(candidateId);return{eventId:selectedEvent.id,ids:next};});setExplicitDeselections(current=>{const next=current.eventId===selectedEvent.id?new Set(current.ids):new Set<string>();if(!selected&&savedSelectionIds.has(candidateId))next.add(candidateId);else next.delete(candidateId);return{eventId:selectedEvent.id,ids:next};});}
  function switchEvent(eventId:string){if(eventId===selectedEvent.id)return;if(!canLeaveDirty(researchStateDirty,()=>window.confirm("Discard unsaved research-selection changes and switch events?")))return;const nextSaved=selectionStore?.events.find(event=>event.eventId===eventId)?.selectedStructures.map(item=>item.candidateId)??[];setDraftSelection({eventId,ids:new Set(nextSaved)});setExplicitDeselections({eventId,ids:new Set()});setSelectedEventId(eventId);clearEventAnalysis();}
  ...
  async function saveResearchSelections(requestedDraft:ReadonlySet<string>=draftSelectionIds,requestedExplicitDeselections:ReadonlySet<string>=explicitDeselections.eventId===selectedEvent.id?explicitDeselections.ids:new Set()){
    if(!dataset||!selectionStore||!selectionPersistenceAvailable)return;
    const savedAtStart=new Set(savedSelectionIds),draftAtStart=new Set(requestedDraft),byId=new Map(analysisResults.map(result=>[result.spread.id,result]));
    const materiallyValid=[...byId].filter(([,result])=>result.researchLayers.structural.status==="resolved"&&(result.researchLayers.model.status==="priced"||result.researchLayers.maker.status==="available"||result.researchLayers.taker.status==="available")).map(([id])=>id);
    const transition=planCandidatePersistence({saved:savedAtStart,draft:draftAtStart,materiallyValid,explicitlyDeselected:requestedExplicitDeselections,generation:contractLoad,currentGenerationKey:currentContractGenerationKey});
    const {toAdd,toRefresh,toKeepUnchanged,toRemoveExplicitly,toRemoveImplicitly,blockedImplicitRemoval,blockedAdd}=transition,toRemove=new Set([...toRemoveExplicitly,...toRemoveImplicitly]);
    if(blockedAdd.size){setSelectionStatus([...blockedAdd].map(([id,reason])=>`Cannot save candidate ${id}: ${reason}.`).join(" "));return;}
    if(!toAdd.size&&!toRefresh.size&&!toRemove.size&&!generationDirty&&!blockedImplicitRemoval.size)return;
    try{
      ...
      const retainedStale=(savedEventSelection?.generationSnapshot.candidates??[]).filter(candidate=>toKeepUnchanged.has(candidate.candidateId)&&!generatedIds.has(candidate.candidateId)).map(candidate=>({...candidate,selected:true}));
      const generationSnapshot=completedGeneration?.eventId===selectedEvent.id?{...completedGeneration.snapshot,candidates:preserveControlledGenerationProvenance([...currentGeneration,...retainedStale],savedEventSelection?.generationSnapshot.candidates??[],researchStructures,controlledResearchLoad.authoritative)}:savedEventSelection!.generationSnapshot;
      const runtimeEventRecord:ResearchSelectionEvent={eventId:selectedEvent.id,sourceRun:savedEventSelection?.sourceRun??canonicalJson({event:eventReference(selectedEvent),savedAtUtc:now}),generationSnapshot,selectedStructures,researchStructures,evidenceCatalog:[...eventEvidenceCatalog.values()].sort((a,b)=>a.evidenceId.localeCompare(b.evidenceId))};
      ...
      if(requestBytes>=RESEARCH_SELECTION_REQUEST_LIMIT_BYTES){console.error("[research-persistence] oversized compact event",researchEventPayloadDiagnostics(eventRecord));throw new Error(`Research save blocked: compact event payload is ${(requestBytes/1_000_000).toFixed(1)} MB; ${(RESEARCH_SELECTION_REQUEST_LIMIT_BYTES/1_000_000).toFixed(1)} MB persistence limit. Payload normalization regression suspected. See console payload decomposition.`)}
      let response:Response;try{response=await fetch(`/__local/research-selections/${encodeURIComponent(dataset.datasetId)}/events/${encodeURIComponent(selectedEvent.id)}`,{method:"PUT",headers:{"Content-Type":"application/json","If-Match":selectionStore.updatedAtUtc},body:requestBody})}catch(error){throw new Error(`Research persistence request failed for ${(requestBytes/1_000_000).toFixed(1)} MB compact payload: ${error instanceof Error?error.message:"Failed to fetch"}`)}const raw=await response.json().catch(()=>({}));if(!response.ok){const context=response.status===413?`Research save failed · ${(requestBytes/1_000_000).toFixed(1)} MB request exceeds 10.0 MB local persistence limit`:response.status===409?"Conflict: reload selections and retry":response.status===400?"Validation failed":"Persistence failed";throw new Error(`${context}. ${raw.details?.map?.((e:{path:string;message:string})=>`${e.path}: ${e.message}`).join(" | ")??raw.error??"Research selections were not saved."}`)}const checked=validateResearchSelectionStore(raw.store??raw);if(!checked.ok)throw new Error("Server returned an invalid selection store.");setSelectionStore(checked.store);setDraftSelection({eventId:selectedEvent.id,ids:new Set(selectedStructures.map(item=>item.candidateId))});setExplicitDeselections({eventId:selectedEvent.id,ids:new Set()});const failureNote=contractLoad.failedContracts?` ${contractLoad.failedContracts} retrieval failure${contractLoad.failedContracts===1?" makes":"s make"} candidate absence non-authoritative.`:"";setSelectionStatus(`Saved ${toAdd.size} new structure${toAdd.size===1?"":"s"}; refreshed ${toRefresh.size}; preserved ${toKeepUnchanged.size} unchanged; removed ${toRemove.size}.${blockedImplicitRemoval.size?` ${blockedImplicitRemoval.size} implicit removal${blockedImplicitRemoval.size===1?" was":"s were"} blocked.`:""}${failureNote}`);
  ...
  async function clearSavedSelection(){if(!savedSelectionIds.size)return;if(!window.confirm(`Clear ${savedSelectionIds.size} saved structures for ${selectedEvent.id}? The source backtest event will not be deleted.`))return;const empty=new Set<string>(),explicit=new Set(savedSelectionIds);setDraftSelection({eventId:selectedEvent.id,ids:empty});setExplicitDeselections({eventId:selectedEvent.id,ids:explicit});await saveResearchSelections(empty,explicit);}
