...
export function OptionsBacktester() {
  ...
  const [resultsFilter, setResultsFilter] = useState<"model" | "maker" | "taker" | "raw">("model");
  ...
  const filteredResults = analysisResults.filter(result => {
    ...
    if (resultsFilter === "raw") return result.researchLayers.maker.status==="available"||result.researchLayers.taker.status==="available";
    return result.researchLayers.model.status==="priced";
  });
  ...
  setHideRed(checked);
        ...
        <section className="workspace-section pnl-block" data-testid="pnl-block" ref={node => { sectionRefs.current.analysis = node; }}>
          ...
          <div className="filter-row"><div className="segmented" aria-label="Result track">{([['model','Model valued'],['maker','Maker available'],['taker','Taker available'],['raw','Raw available']] as const).map(([value,label]) => <button className={resultsFilter === value ? "active" : ""} onClick={() => setResultsFilter(value)} key={value}>{label}</button>)}</div><label className="unavailable-toggle"><input type="checkbox" checked={showUnavailable} onChange={event=>setShowUnavailable(event.target.checked)}/>Show unavailable · {unavailableResults.length}</label><span>{filteredResults.length} / {analysisResults.length} visible</span></div>
