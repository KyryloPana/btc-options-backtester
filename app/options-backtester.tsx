...
export function OptionsBacktester() {
  ...
  async function saveResearchSelections(requestedDraft:ReadonlySet<string>=draftSelectionIds){
    ...
    try{
      ...
      const researchStructures:ResearchOnlyStructure[]=materialized.map(s=>({...s,selectionId:`research~short-strike~${s.candidateId}`,selectedAtUtc:now,modeledExecution:undefined,delayedExecution:undefined,executionScenarios:{maker:{status:"not_evaluated",reason:"Research-only Reference counterfactual; strict maker execution is a separate robustness layer.",entrySnapshot:null,valuationPathSnapshot:[],outcomeSnapshots:[]},taker:{status:"not_evaluated",reason:"Research-only Reference counterfactual; strict taker execution is a separate robustness layer.",entrySnapshot:null,valuationPathSnapshot:[],outcomeSnapshots:[]}},researchRole:((completedGeneration?.snapshot.candidates.find(c=>c.candidateId===s.candidateId)?.strikeMethod)==="buffered"?"short_strike_buffered":"short_strike_technical")}));
