...
export function OptionsBacktester() {
  ...
  async function saveResearchSelections(){if(!dataset||!selectionStore||!selectionPersistenceAvailable||(!selectionDirty&&!analysisResults.length))return;try{const now=new Date().toISOString(),finalIds=reconcileSelectionIds(savedSelectionIds,selectionToggles),eventEvidenceCatalog=new Map<string,EvidenceTradeDto>();const byId=new Map(analysisResults.map(result=>[result.spread.id,result]));const prior=new Map((savedEventSelection?.selectedStructures??[]).map(item=>[item.candidateId,item]));const selectedStructures:SelectedStructure[]=[...finalIds].sort().map(candidateId=>{const result=byId.get(candidateId);if(!result)return prior.get(candidateId)!;if(result.researchEntry.status!=="priced")throw new Error("Unavailable structures cannot be saved.");const spread=result.spread,usages:EvidenceUsageDto[]=[];return{selectionId:stableSelectionId(selectedEvent.id,"deribit",candidateId),eventId:selectedEvent.id,candidateId,venue:"deribit",selectedAtUtc:now,quantity:result.researchEntry.amount,candidateSnapshot:compactCandidateMetadata({venue:"deribit",contractStyle:"inverse BTC option",structure:spread.structure,spreadKind:spread.spreadKind,optionType:spread.optionType,targetDte:spread.targetDte,expiryTimestamp:spread.expiryTimestamp??null,actualDte:spread.actualDte??null,shortStrike:spread.resolvedSoldStrike??spread.soldStrike,longStrike:spread.resolvedBoughtStrike??spread.boughtStrike,actualWidth:spread.actualWidth??null,instruments:{short:spread.soldContract?.instrumentName??null,long:spread.boughtContract?.instrumentName??null},quality:result.eventQuality,qualityReasonCodes:[]}),entrySnapshot:compactEntryEconomics("deribit",candidateId,result.researchEntry,usages,eventEvidenceCatalog),valuationPathSnapshot:result.researchPath.map(point=>compactValuationPoint("deribit",candidateId,point,usages,eventEvidenceCatalog)),outcomeSnapshots:result.researchOutcomes.map(outcome=>compactOutcomeSnapshot("deribit",candidateId,outcome,usages,eventEvidenceCatalog)),marginSnapshot:compactMarginResult(result.observation.marginResult??{status:"unavailable",reason:"No margin result was produced."}),evidenceTradeSnapshots:[],evidenceUsages:usages};});
