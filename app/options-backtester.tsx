...
import { RESEARCH_SELECTION_SCHEMA_VERSION, candidateIdentity, canonicalJson, canSelectResearchCandidate, eventReference, reconcileSelectionIds, stableCandidateId, stableSelectionId, validateResearchSelectionStore, type GenerationCandidateSnapshot, type ResearchSelectionEvent, type ResearchSelectionStore, type SelectedStructure } from "./lib/research-selections";
...
function ResearchDetail({result,amount,onAmountChange,openingCurrency}: {result:AnalysisResult;amount:string;onAmountChange:(value:string)=>void;openingCurrency:"btc"|"usd-entry"}) {
...
export function OptionsBacktester() {
  ...
  async function saveResearchSelections(){if(!dataset||!selectionStore||!selectionDirty||!selectionPersistenceAvailable)return;const now=new Date().toISOString(),finalIds=reconcileSelectionIds(savedSelectionIds,selectionToggles);const byId=new Map(analysisResults.map(result=>[result.spread.id,result]));const prior=new Map((savedEventSelection?.selectedStructures??[]).map(item=>[item.candidateId,item]));const selectedStructures:SelectedStructure[]=[...finalIds].sort().map(candidateId=>{const result=byId.get(candidateId);if(!result)return prior.get(candidateId)!;if(result.researchEntry.status!=="priced")throw new Error("Unavailable structures cannot be saved.");const spread=result.spread;return{selectionId:stableSelectionId(selectedEvent.id,"deribit",candidateId),eventId:selectedEvent.id,candidateId,venue:"deribit",selectedAtUtc:now,quantity:result.researchEntry.amount,candidateSnapshot:canonicalJson({venue:"deribit",contractStyle:"inverse BTC option",structure:spread.structure,spreadKind:spread.spreadKind,optionType:spread.optionType,targetDte:spread.targetDte,expiryTimestamp:spread.expiryTimestamp??null,actualDte:spread.actualDte??null,shortStrike:spread.resolvedSoldStrike??spread.soldStrike,longStrike:spread.resolvedBoughtStrike??spread.boughtStrike,actualWidth:spread.actualWidth??null,instruments:{short:spread.soldContract?.instrumentName??null,long:spread.boughtContract?.instrumentName??null},quality:result.eventQuality,qualityReasonCodes:[result.researchEntry.qualityReason]}),entrySnapshot:canonicalJson(result.researchEntry),valuationPathSnapshot:result.researchPath.map(point=>canonicalJson(point)),outcomeSnapshots:result.researchOutcomes.map(outcome=>canonicalJson(outcome)),marginSnapshot:canonicalJson(result.observation.marginResult??{status:"unavailable",reason:"No margin result was produced."}),evidenceTradeSnapshots:canonicalJson([...(spread.soldContract?.trades??[]),...(spread.boughtContract?.trades??[])]) as never};});
  ...
  async function exportResearchBundle(){if(!dataset||!persistedSelectionCount||!selectionPersistenceAvailable)return;setSelectionStatus("Validating saved research bundle…");try{const response=await fetch(`/__local/research-bundle/${encodeURIComponent(dataset.datasetId)}`);if(!response.ok){const body=await response.json();throw new Error(body.error??"Research bundle export failed.")}const blob=await response.blob(),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download=response.headers.get("Content-Disposition")?.match(/filename="([^"]+)"/)?.[1]??`research-bundle-${dataset.datasetId}.zip`;link.click();URL.revokeObjectURL(url);setSelectionStatus(`Exported ${persistedSelectionCount} persisted selected structures across the current dataset.`)}catch(error){setSelectionStatus(`${error instanceof Error?error.message:"Research bundle export failed."} Static hosting cannot export local research bundles; run the local application server.`)}}
  ...
  setHideRed(checked);
        ...
        <section className="workspace-section pnl-block" data-testid="pnl-block" ref={node => { sectionRefs.current.analysis = node; }}>
          ...
          </tbody></table></div><div className="research-save-bar"><strong>{activeSelectionIds.size} structures selected for this event</strong>{selectionDirty&&<span className="dirty-state">Unsaved selection changes</span>}<button className="primary-button" disabled={!selectionDirty||!selectionPersistenceAvailable} onClick={()=>void saveResearchSelections()}>Save selected structures</button></div>{selectionStatus&&<p className="fine-print">{selectionStatus}</p>}{[...savedSelectionIds].filter(id=>!analysisResults.some(result=>result.spread.id===id)).map(id=><p className="fine-print" key={id}>SAVED · stale/unmatched candidate {id}</p>)}</div>}

          {selectedAnalysis && pricingAssumption === "research-estimate" && <ResearchDetail result={selectedAnalysis} amount={scenarioAmount} onAmountChange={setScenarioAmount} openingCurrency={openingCurrency}/>}
