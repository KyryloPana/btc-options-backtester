...
function ExpiryPayoffChart({input}:{input:ExpiryPayoffInput}){
  const [currency,setCurrency]=useState<PayoffCurrency>("usd-cash-flow"),[selectedIndex,setSelectedIndex]=useState<number>();
  ...
  const inspect=(clientX:number,rect:DOMRect)=>setSelectedIndex(minimum+Math.max(0,Math.min(1,(clientX-rect.left)/rect.width))*(maximum-minimum));
  const markers=[{label:"Short",value:input.shortStrike,className:"short"},{label:"Long",value:input.longStrike,className:"long"},{label:"Break-even",value:be?.index,className:"breakeven"},{label:"Entry",value:input.entryIndex,className:"entry"}];
  return <section className="expiry-payoff"><div className="card-title-row"><div><p className="eyebrow">Theoretical payoff at expiration</p><h4>Credit-spread expiration profile</h4></div><div className="segmented payoff-currency-controls" aria-label="Payoff currency"><button className={currency==="usd-cash-flow"?"active":""} onClick={()=>{setCurrency("usd-cash-flow");setSelectedIndex(undefined)}}>USD cash-flow</button><button className={currency==="btc-settlement"?"active":""} onClick={()=>{setCurrency("btc-settlement");setSelectedIndex(undefined)}}>BTC settlement</button></div></div><p className="fine-print">Theoretical expiration payoff based on the estimated entry. It is not proof of execution.</p>
    <div className="payoff-plot"><svg viewBox={`0 0 ${W} ${H}`} role="application" tabIndex={0} aria-label="Theoretical expiration payoff. Use arrow keys and Escape to inspect points." onPointerMove={e=>inspect(e.clientX,e.currentTarget.getBoundingClientRect())} onPointerDown={e=>inspect(e.clientX,e.currentTarget.getBoundingClientRect())} onKeyDown={e=>{if(e.key==="Escape"){setSelectedIndex(undefined);return}if(!["ArrowLeft","ArrowRight","Home","End"].includes(e.key))return;e.preventDefault();const step=(maximum-minimum)/180;setSelectedIndex(current=>e.key==="Home"?minimum:e.key==="End"?maximum:Math.max(minimum,Math.min(maximum,(current??minimum)+(e.key==="ArrowLeft"?-step:step))))}}><defs><linearGradient id="payoffArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#4e9a75" stopOpacity=".28"/><stop offset="1" stopColor="#a95656" stopOpacity=".22"/></linearGradient></defs><line className="payoff-zero" x1={L} x2={W-R} y1={y(0)} y2={y(0)}/>{markers.map(m=>m.value!==undefined&&<g className={`payoff-marker ${m.className}`} key={m.label}><line x1={x(m.value)} x2={x(m.value)} y1={T} y2={H-B}/><text x={x(m.value)} y={H-B+15} textAnchor="middle">{m.label}</text></g>)}<path className="payoff-area" d={`${curve} L${x(maximum)},${y(0)} L${x(minimum)},${y(0)} Z`}/><path className="payoff-curve" d={curve}/>{selected&&<g className="payoff-cursor"><line x1={x(selected.expirationIndex)} x2={x(selected.expirationIndex)} y1={T} y2={H-B}/><circle cx={x(selected.expirationIndex)} cy={y(selected.pnl)} r="5"/></g>}<text className="axis-label" x={(L+W-R)/2} y={H-4} textAnchor="middle">BTC index at expiration</text><text className="axis-label" transform={`translate(14 ${(T+H-B)/2}) rotate(-90)`} textAnchor="middle">{currency==="usd-cash-flow"?"USD cash-flow PnL":"BTC settlement PnL"}</text></svg></div>
