...
import { aggregateObservations, buildAndRunObservationRequests, buildObservationExport, type StrategyObservation } from "./lib/observation-ledger";
...
export default function Home() {
  ...
  async function runBacktest() {
    ...
    try {
      ...
      const next = observations.map(observation => {
        ...
        return { observation, spread, path, exits, selectedExit, eventQuality };
      });
  ...
  function exportResults() {
    ...
    if (!payload.valid) { setAnalysisStatus(`Export blocked: ${payload.validationErrors.length} ledger invariants failed.`); return; }
        ...
        <section className="workspace-section" ref={node => { sectionRefs.current.analysis = node; }}>
          ...
          {selectedAnalysis && <div className="analysis-detail">
            <div className="path-card card">
              ...
              <div className="path-metrics"><span><small>Best diagnostic unrealized<InfoTooltip term="extrema" label="Explain Best diagnostic unrealized and Max adverse" /></small><strong className="positive">{money(Math.max(...selectedAnalysis.path.map(point => point.diagnosticIvUnrealizedPnlUsd ?? point.diagnosticRawUnrealizedPnlUsd ?? -Infinity)))}</strong></span><span><small>Max adverse</small><strong className="negative">{money(Math.min(...selectedAnalysis.path.map(point => point.diagnosticIvUnrealizedPnlUsd ?? point.diagnosticRawUnrealizedPnlUsd ?? Infinity)))}</strong></span><span><small>Grid points</small><strong>{selectedAnalysis.path.length}</strong></span></div>
            ...
            <aside className="exit-card card"><p className="eyebrow">Exit engine</p><h3>Independent outcomes</h3><p className="quality-reason"><strong>Actual entry evidence:</strong> {selectedAnalysis.observation.entryExecution?.reason ?? "No executed entry"}</p><div className="exit-list">{selectedAnalysis.exits.map(exit => <div className={`exit-row ${exit.status}`} key={exit.rule}><span>{exit.qualityFlag ? <QualityDot flag={exit.qualityFlag} /> : <span className="quality-dot muted" />}{exit.rule}<small>{exit.timestamp ? formatUtc(exit.timestamp) : exit.status.replace("-", " ")} · {exit.qualityReason}</small></span><strong>{exit.status}</strong></div>)}</div><div className="ledger-note"><strong>Terminal realized result</strong><p>{money((selectedAnalysis.observation.executedNetPnl ?? selectedAnalysis.observation.settlementNetPnl)?.usd)} · sourced only from causal close fills or settlement.</p></div><div className="ledger-note"><strong>Trust means reliability</strong><p>Red, Yellow, and Green describe pricing evidence only—not profitability. Displayed trust combines entry evidence with the selected exit’s own evidence.</p></div></aside>
          </div>}
        </section>
