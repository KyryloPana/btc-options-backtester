...
import { displayedProfit, entryEvidenceExplanation, observationStatus, spreadIdentity } from "./lib/observation-presentation";
  ...
  async function runBacktest() {
    ...
    try {
      ...
      const next: AnalysisResult[] = observations.map(observation => {
        ...
        const eventQuality: QualityFlag = observation.eventOutcome === "executed" || observation.eventOutcome === "settled" ? "green" : "red";
        const config = observation.strategyConfiguration;
        return { observation, spread, path, exits, selectedExit, eventQuality, scenarioInput: { event: selectedEvent, candidates: retrievedSpreads, candles, config } };
      });
        ...
        <section className="workspace-section" ref={node => { sectionRefs.current.analysis = node; }}>
          ...
          <div className="table-card card"><div className="table-scroll"><table><thead><tr><th><span className="sr-only">Expand</span></th><th>Selected-exit trust</th><th>Spread</th><th>Expiry</th><th>Width</th><th>Actual gross entry</th><th>Actual net opening</th><th>Best diagnostic IV unrealized<InfoTooltip term="pnlMethods" label="Explain Raw versus IV-normalized PnL" /></th><th>Worst diagnostic IV unrealized</th><th>Selected exit</th></tr></thead><tbody>
            {filteredResults.map(result => {
              ...
              return <Fragment key={result.spread.id}><tr className={selectedAnalysis?.spread.id === result.spread.id ? "row-selected" : ""} onClick={() => selectAnalysisResult(result.spread.id)}><td><button className="expand-button" aria-expanded={expanded} aria-controls={`ledger-${result.spread.id}`} aria-label={`${expanded ? "Collapse" : "Expand"} opening ledgers for ${result.spread.structure}`} onClick={event => { event.stopPropagation(); setExpandedResultIds(current => expanded ? current.filter(id => id !== result.spread.id) : [...current, result.spread.id]); }}><span className="expand-chevron" aria-hidden="true" /></button></td><td><span className={flagClass(result.spread.entryLiquidityQuality ?? "red")}>{result.spread.entryLiquidityQuality ?? "red"} liquidity</span><small>Entry {result.observation.entryExecution?.status ?? "not run"} · exit {result.selectedExit?.status ?? "not applicable"} · outcome {result.eventQuality}</small></td><td><strong>{result.spread.structure}</strong><small className="mono">{spreadIdentity(result.spread).soldStrike ?? "—"} / {spreadIdentity(result.spread).boughtStrike ?? "—"} {result.spread.optionType}</small></td><td>{result.spread.expiryLabel}<small>Target ~{result.spread.targetDte}D · actual {result.spread.actualDte?.toFixed(1)}D · {result.spread.expiryRank ? `rank #${result.spread.expiryRank}` : "unranked"}</small></td><td>{money(result.spread.actualWidth)}</td><td>{btc(result.observation.entryCashFlow?.grossBtc)}</td><td>{btc(result.observation.entryCashFlow?.netBtc)}</td><td className="positive">{best === -Infinity ? "—" : money(best)}</td><td className="negative">{worst === Infinity ? "—" : money(worst)}</td><td>{observationStatus(result.observation)}<small>{money(displayedProfit(result.observation))}</small></td></tr>{expanded && <tr className="ledger-detail-row"><td colSpan={10}><div id={`ledger-${result.spread.id}`} className="ledger-pair">{result.entryLedgers ? <><Ledger ledger={result.entryLedgers.raw}/><Ledger ledger={result.entryLedgers.iv}/></> : <p>{entryEvidenceExplanation(result.observation)}</p>}</div></td></tr>}</Fragment>;
            })}
